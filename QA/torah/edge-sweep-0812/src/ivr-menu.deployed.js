// חולץ מחבילת ה-ESZIP הפרוסה של ivr-menu (פרויקט bieebmnmkffwbqlsfozh, 12/08).
...
const DEFAULT_MENU = {
  ...
  options: [
    ...
    {
      key: "2",
      label: "עדכון שיעור קיים",
      action: "goto",
      target: "update_lesson"
    },
    {
      key: "3",
      label: "הקמת שיעור חדש",
      action: "goto",
      target: "request_lesson"
    },
    {
      key: "4",
      label: "הצטרפות כמגיד שיעור",
      action: "goto",
      target: "register_teacher"
    },
    {
      key: "0",
      label: "שיחה עם נציג",
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data } = await supabase.from("ivr_menu_config").select("menu_tree").order("updated_at", {
      ascending: false
    }).limit(1).single();
