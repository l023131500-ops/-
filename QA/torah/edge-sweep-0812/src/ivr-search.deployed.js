// חולץ מחבילת ה-ESZIP הפרוסה של ivr-search (פרויקט bieebmnmkffwbqlsfozh, 12/08).
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const params = req.method === "GET" ? Object.fromEntries(new URL(req.url).searchParams) : await req.json();
    ...
    if (params.audience) query = query.contains("target_audience", [
      params.audience
    ]);
    ...
    const textResults = (data || []).map((l, i)=>{
      const schedule = l.is_recurring && l.schedule_days?.length ? l.schedule_days.map((d)=>`${d.day} בשעה ${d.time}`).join(", ") : l.specific_date || "";
