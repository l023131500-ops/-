...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!body.tenant_id || !body.amount || !body.donor?.name || !body.donor?.phone) {
      return json({
        ok: false,
        error: "missing required fields (tenant_id, amount, donor.name, donor.phone)"
      }, 400);
    }
    // 1. Load tenant's Nedarim config (mosad_id + api_valid per tenant)
    const { data: cfg } = await supabase.from("nedarim_configs").select("mosad_id, api_valid, charge_message, success_redirect_url, failure_redirect_url").eq("tenant_id", body.tenant_id).maybeSingle();
    ...
    const mosadId = cfg?.mosad_id || Deno.env.get("NEDARIM_MOSAD_ID") || undefined;
    const apiValid = cfg?.api_valid || Deno.env.get("NEDARIM_API_VALID") || undefined;
    if (!mosadId) {
      return json({
        ok: false,
        error: "Nedarim Mosad ID not configured for this tenant. Set nedarim_configs.mosad_id for the tenant."
      }, 400);
    }
    if (!apiValid) {
      return json({
        ok: false,
        error: "Nedarim ApiValid (iframe activation token) not configured. Set nedarim_configs.api_valid for the tenant."
      }, 400);
    }
    ...
    const paymentType = body.payment_type === "HK" ? "HK" : body.payment_type === "CreateToken" ? "CreateToken" : "Ragil";
    ...
    const tashlumim = paymentType === "HK" ? body.recurring_months ? String(body.recurring_months) : "" : String(body.payments || 1);
    ...
    const { data: txn } = await supabase.from("nedarim_transactions").insert({
