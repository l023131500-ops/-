// זריעה למדידת הלבנה של הרשימה — «כמה ידיים עברו» בשורת רשימת העבודה.
//
// זהה בכוונה לזריעה של 0089 (hands-count-0089-0816/seed.mjs) פרט לטלפונים
// ולשמות: מה שנמדד כאן הוא אותם ארבעה מצבים בדיוק, והפעם מהדפדפן. ארבע פניות
// ולא אחת, מפני שהשורה ברשימה היא פנייה, וכל מצב שהסימן יכול להיות בו צריך
// פנייה משלו כדי להיראות בטעינה אחת:
...
// הקריאה בארגומנט אחד, ונעשה בצד ה-SQL ולא כאן: השער ממלא את מזהה המנהל
// מהטוקן תמיד, ואין דרך לקבל את הצורה הזו מעל HTTP.
//
...
const seeded = {};
const plan = [
  ["A", "0501230991", "בדיקה UI שלוש ידיים", "עליה יעברו שני מנהלים בשלושה מעברים"],
  ["B", "0501230992", "בדיקה UI לחיצה חוזרת", "עליה ילחצו פעמיים על אותו סטטוס ומעבר אחד"],
  ["C", "0501230993", "בדיקה UI מעבר בלי זהות", "עליה ייכתב מעבר בצורת v7 — בלי זהות"],
  ["D", "0501230994", "בדיקה UI בלי מעבר", "לא ייגעו בה כלל"],
];
...
const one = await login("qa0089ui-first@more30.test", "Qa0089ui-first-hand!");
const two = await login("qa0089ui-second@more30.test", "Qa0089ui-second-hand!");
