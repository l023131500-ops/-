// פריסה: האם הסימן «נמצאה לפי הנימוק» — הסיבה שבגללה השורה נמצאה — הגיע אל
// הכתובת החיה.
//
// הצד השני של המדידה של e73fc63, ולא ניסוח שני שלה: שם הסימן נמדד משני שרתים
...
// היכן — נמדד ולא הונח:
//
// NEW (7)  — 0 בייצור, ≥1 במקור. אלה שקונים את הפריסה. שלושת הראשונים הם
//            המוצר עצמו (הפונקציה, הנוסח שבשורה והשדה שהיא קוראת), ושלושת
//            הבאים הם ה-title והתיעוד שמחזיקים אותם. ⚠️ «והמסך אינו רשאי
//            להמציא אותו» נבדק ונפסל ב-classify: הוא שבור על פני שתי שורות
//            הערה במקור ולכן src=0 — מסומן שמחזיר 0 בשני הצדדים אינו בקרה,
//            הוא מדידה שלא נעשתה.
// DIFF (6) — קיימים בייצור ובמספר שונה. שלושה מהם הם השימוש האמיתי:
//            Object.hasOwn 10→12 (הכרעה (0)), meta.append 4→5 (השורה שמוסיפה
//            את הסימן) ו-s.className = "stale" 17→18 (הסימן עצמו). שלושת
//            האחרים אינם קוד חדש אלא נוכחות של טקסט — 0091 4→5, q-phone 3→5
//            ו-0093 2→3 גדלו מפני שהתיעוד החדש מזכיר אותם בשמם.
// OLD (46) — בקרה: אותו מספר בדיוק בשתי הגרסאות, ולכן הם מראים שמסומני NEW
//            נבדקו מול קובץ שהתקבל ולא מול תשובה ריקה. תשעת הראשונים הם
//            מסומני ה-NEW של 2840dd7 — מה שהלבנה הקודמת פרסה הוא הבקרה של זו.
//            decidedBit נשאר 4 אף שהוא הסימן שהסימן החדש נדחף מיד לפניו, ולכן
//            הוא הבקרה הצמודה ביותר שיש.
//
