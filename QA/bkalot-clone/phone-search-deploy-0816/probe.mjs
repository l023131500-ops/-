// פריסה: האם שורת «חופש לפי» — מה שהמסך אומר כשמונח החיפוש נורמל אל ספרות —
// הגיעה אל הכתובת החיה.
//
// הצד השני של המדידה של 02fc51c, ולא ניסוח שני שלה: שם השורה נמדדה מקוד שבמקור
...
//        q_phone ו-0093 יושבים כאן ולא ב-DIFF מפני שנמדדו 0 בייצור: 0093 היא
//        מיגרציה, ומה שהיא הוסיפה לתשובה לא נאמר עד עכשיו באף שורה של המסך.
// DIFF — קיימים בשתי הגרסאות ובמספר שונה: border-inline-start ו-role="status"
//        יושבים בייצור מזמן (‎.filters ו-#count), והלבנה החדשה משתמשת בשניהם שוב
//        — ולכן הם גדלים ואינם נולדים.
...
const NEW = [
  ...
  "qnote",
  "qPhone",
  "חיפוש הטלפון רץ על הספרות בלבד",
  "חופש לפי",
  "הקלדתי עם מקפים",
  "אין פנייה כזו",
  "q_phone",
...
const DIFF = ["border-inline-start", "role=\"status\""];
const OLD = [
  "list-error",
  "decider_required",
  "note_too_long",
