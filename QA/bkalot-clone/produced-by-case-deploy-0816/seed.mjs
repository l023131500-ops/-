// זריעה לפעימת הפריסה של «הופק על ידי» בשורת המסמך שברשימת הפנייה.
//
// פנייה אחת דרך נתיב הקליטה הציבורי מעל HTTP (bkalot-clone-intake, מפתח anon
// כמו מהטופס) ולא בהזרקה ל-cases. עליה יופק מסמך מהמסך החי שבייצור, בלחיצת
// «הפק», ולא ב-SQL — מפני שמה שנמדד כאן הוא מה שהשער v8 מעביר בייצור.
//
...
const res = await fetch(INTAKE, {
  ...
  body: JSON.stringify({
    ...
    full_name: "בדיקה מי הפיק בשורה בייצור", phone: "0501230816", email: "test@more30.com",
    note: "יופק עליה מסמך מהמסך החי, כדי לראות מי הפיק אותו בשורת הרשימה שבייצור", consent: "true",
  }),
});
