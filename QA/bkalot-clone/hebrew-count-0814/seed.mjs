// זריעה לפעימת «1 זכויות / 1 מסמכים» ברשימת העבודה.
//
...
// אחת תרד לזכות אחת ותקבל מסמך אחד, והשנייה היא הבקרה שנשארת ברבים — אחרת
// ההבדל שנמדד אחר כך יכול להיות הבדל בין שתי פניות ולא בין שני מצבי ניסוח.
//
...
const caseOne = await mk("בדיקה 0082 יחיד", "מדידת ספירה בעברית — תרד לזכות אחת ולמסמך אחד", "0501230095");
const caseMany = await mk("בדיקה 0082 רבים", "מדידת ספירה בעברית — בקרה שנשארת ברבים", "0501230096");
