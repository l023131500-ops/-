// פריסה: האם תיבת הנימוק — הצד שממנו מקלידים — הגיעה אל הכתובת החיה.
//
// הצד השני של המדידה של b1fcca7, ולא ניסוח שני שלה: שם התיבה נמדדה מקוד שבמקור
// מול אותו edge ואותו מסד חי, וכאן אותם מסומנים בדיוק צריכים להתהפך מ-0 בייצור
// אל שוויון עם המקור.
//
// NEW  — קיימים במקור ואינם קיימים בייצור לפני. אלה שקונים את הפריסה.
// DIFF — קיימים בשתי הגרסאות ובמספר שונה: btrim ו-note_too_long הגיעו לייצור
//        ב-9c96135 (הראשון בהערה על td.why, השני כשם שגיאה), והתיבה מזכירה את
//        שניהם שוב — ולכן הם גדלים ואינם נולדים.
// OLD  — בקרה: קיימים בשתי הגרסאות ובאותו מספר בדיוק, ולכן הם מראים שמסומני NEW
//        נבדקו מול קובץ שהתקבל ולא מול תשובה ריקה.
//
// הסיווג נקבע ב-classify.mjs לפני הריצה הראשונה ונמדד ולא הונח. בריצה הזו אף
// מסומן לא סווג מחדש — תשעת מסומני הבקרה נמדדו שווים בדיוק, בשונה מ-8a577bc,
// מ-24f8a7d ומ-9c96135 שבהם בקרה נפלה ל-DIFF. שתי הריצות רצות מהקובץ הזה בדיוק
// ובלי שינוי ביניהן — תקלת 0087, שבה שתי ריצות רצו משתי גרסאות של אותו כלי.
//
...
const DIFF = ["btrim", "note_too_long"];
const OLD = [
  "td.why",
  "fillHistoryNoteCell",
  "רצף ההכרעות",
