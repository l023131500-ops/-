// זריעה לפעימת ה-UI של «מי הכריע ומתי».
//
// שתי פניות, שתיהן דרך נתיב הקליטה הציבורי מעל HTTP (bkalot-clone-intake, מפתח
// anon כמו מהטופס) ולא בהזרקה ל-cases: מה שנמדד אחר כך הוא מה שהמסך מקבל על
// שורה שנוצרה כמו שנוצרת שורה אמיתית.
//
...
const a = await intake({
  ...
  full_name: "בדיקה 0078 א",
  phone: "0501230090",
  email: "test@more30.com",
  note: "מדידת UI — מי הכריע ומתי",
  consent: "true",
});
