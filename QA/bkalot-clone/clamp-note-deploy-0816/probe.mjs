// פריסה: האם ההודעה שמסבירה את החיתוך — מה שמונע ממספר העמוד לקפוץ בלי שנאמר
// למה — הגיעה אל הכתובת החיה.
//
// הצד השני של המדידה של e2ea093, ולא ניסוח שני שלה: שם ההודעה נמדדה משני
// שרתים סטטיים מקומיים (8180 הגרסה שבייצור, 8181 המתוקנת), וכאן אותם מסומנים
// בדיוק צריכים להתהפך מ-0 בייצור אל שוויון עם המקור.
//
// הסיווג נקבע ב-classify.mjs לפני שהקובץ הזה נכתב, ומספריו הם מה שקבע מי יושב
// היכן — נמדד ולא הונח:
//
// NEW (8)  — 0 בייצור, ≥1 במקור. אלה שקונים את הפריסה.
// DIFF (6) — קיימים בשתי הגרסאות ובמספר שונה. שניים מהם מפתיעים ונאמרים:
//            qPhone 6→7 ו-load(true) 1→3 גדלו מפני שהתיעוד החדש מזכיר אותם
//            בשמם ("הוא יושב כאן ליד qPhone", "ולא על load(true)") — כלומר הם
//            אינם מודדים קוד חדש אלא נוכחות של הטקסט. reclamped 3→4 הוא
//            השימוש האמיתי (if (!reclamped) clampNote = null), ו-hidden = true
//            21→22 ו-role="status" 2→3 הם האלמנט החדש עצמו.
// OLD (26) — בקרה: אותו מספר בדיוק בשתי הגרסאות, ולכן הם מראים שמסומני NEW
//            נבדקו מול קובץ שהתקבל ולא מול תשובה ריקה. שמונת הראשונים הם
//            מסומני ה-NEW וה-DIFF של 3322ca5 — מה שהלבנה הקודמת פרסה הוא
//            הבקרה של זו. q-phone נשאר 3 אף שנוסף אלמנט <p class="qnote">
//            שני לידו, ולכן הוא הבקרה הצמודה ביותר שיש.
//
