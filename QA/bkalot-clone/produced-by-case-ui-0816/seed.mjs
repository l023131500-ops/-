// זריעה לפעימת «שורת המסמך ברשימת הפנייה אינה אומרת מי הפיק» (UI).
//
...
const res = await fetch(INTAKE, {
  ...
  body: JSON.stringify({
    ...
    full_name: "בדיקה מי הפיק בשורה", phone: "0501230802", email: "test@more30.com",
    note: "יופק עליה מסמך מהמסך, כדי לראות מי הפיק אותו בשורת הרשימה", consent: "true",
  }),
});
