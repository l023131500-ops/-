// זריעה למדידת המונה מהכתובת החיה: פנייה אחת, דרך נתיב הציבור מעל HTTP עם מפתח
// anon כמו מהטופס ולא הזרקת שורה. פנייה אחת מספיקה בדיוק: היא נותנת את שלושת
// המצבים שהמונה נמדד בהם — בלי סינון (תור), סינון שהיא עונה עליו, וסינון שאיש
// אינו עונה עליו — והמצב השלישי הוא זה שבו המונה אמר «אין פניות» על תור מלא.
//
...
const res = await fetch(FN + "/bkalot-clone-intake", {
  ...
  body: JSON.stringify({
    ...
    full_name: "אברהם ישראלי", phone: "0501230012",
    email: "test@more30.com", note: "פנייה שהמונה בייצור נמדד עליה", consent: "true",
  }),
});
