...
const cb = `?cachebust=count-filtered-${phase}`;
const URLS = [
...
// ⚠️ הטענה שאין לה תחליף: הקוד הישן יצא. לפני — ≥1 בייצור; אחרי — 0.
verdicts.removed_present_in_live = out.markers.removed.every((m) => m.dist > 0 && m.live.every((v) => v > 0));
verdicts.removed_gone_from_live = out.markers.removed.every((m) => m.live.every((v) => v === 0));
...
writeFileSync(`${ROOT}/QA/bkalot-clone/count-filtered-deploy-0816/${phase}.json`, JSON.stringify(out, null, 2));
