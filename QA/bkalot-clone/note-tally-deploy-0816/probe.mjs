...
for (const group of ["new", "diff", "control"]) {
...
// ⚠️ נוסף אחרי ריצת ה-before ולפני הפריסה, ונאמר ולא נבלע: ארבעה מסומנים
// שסווגו ב-classify.mjs כבקרה גדלים בפועל (רצף ההכרעות 4→5, fillHistoryNoteCell
// 2→4, «זה הנימוק שהתאים» 2→3, openCase 10→11) — הבלוק החדש מצטט את המילים
// האלה בהערותיו, וזו שגיאת סיווג שלי ולא ממצא. classify.mjs לא נערך: תיקון
// סיווג אחרי שרואים את התוצאה הוא בדיוק תקלת 0087. במקומו נוספה כאן טענה
// חזקה יותר משלוש הקודמות גם יחד — כל 25 המסומנים, בלי קשר לקבוצתם, חייבים
// להחזיר בייצור בדיוק את ספירת המקור. היא נכשלת לפני הפריסה ואמורה לעבור
// אחריה. ולצדה: dist==live לפני הפריסה הוא מה שמוכיח שהשרשרת נמדדת ולא מונחת.
const ALL = [...out.markers.new, ...out.markers.diff, ...out.markers.control];
...
writeFileSync(`${ROOT}/QA/bkalot-clone/note-tally-deploy-0816/${phase}.json`, JSON.stringify(out, null, 2));
