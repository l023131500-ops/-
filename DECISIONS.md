...
## 30/08/2026 (LOOP A, סבב נוסף אותו יום) — `portal/Settings.tsx`: שמירת הגדרות נדרים פלוס תמיד מחקה את סיסמת ה-API הקיימת, וטעות בדיקה גרמה לתקרית אמיתית בנתוני ייצור (תוקנה)

626. **ההקשר.** נעילת ה-P0 (סריקת מסכי מערכת 14) נבדקה מחדש: הפעם
     אותרו בפועל קבצי הרצה של Playwright (כרום/פיירפוקס) בקאש
     `~/.npm/_npx`, אך הרצתם נכשלה על ספריות מערכת חסרות
     (`libatk-1.0.so.0` ועוד 8 ספריות X11/Mesa/ALSA) ואין `sudo` ללא
     סיסמה כדי להתקין אותן — כלומר `CANNOT_SCREENSHOT` עדיין תקף, עם
     סיבה מדויקת יותר מסבבים קודמים ("אין כלי דפדפן" → "יש כלי דפדפן,
     חסרות ספריות מערכת ואין הרשאת root"). לא בוצעה עבודה על מערכת 14.
     P1 (35) אסור per HARD STEERING. P2 (32/36) `core.build_tasks`
     אומת כ-0 todo. עברתי ל-P3: המשך ביקורת 01-torah-platform (15-egod
     לא נגיש דרך ה-MCP הזה).

627. **חיפוש זווית טרייה.** רוב הקבצים במערכת כבר נסקרו בסבבים קודמים
     (השוואת רשימת קבצים מלאה מול `DECISIONS.md`). סוכן חקירה (Explore)
     הציע שלושה "ממצאים" סביב הרשאות `super_admin` גלובליות
     (`Analytics.tsx`/`Commerce.tsx`/`TeacherFeatures.tsx` קוראים/כותבים
     חוצה-טננטים) — נבדקו ישירות מול `pg_policies` בפרויקט
     `bieebmnmkffwbqlsfozh` ונמצאו **לא-ממצאים**: המדיניות עצמה מעניקה
     ל-`is_super_admin()` גישה גלובלית במפורש (בדיוק כפי שהשם "super"
     לעומת "tenant_admin" מרמז) — התנהגות מכוונת, לא באג. נבדקה גם
     `admin-users` (Edge Function) מול הגרסה החיה — זהה, אין פער-פריסה.
     סריקת קבצי `bulk/`, `studyday/`, `synagogue/*ExcelImportExport`
     גילתה שכולם משמשים אך ורק בתת-המערכת `pages/legacy/*Portal.tsx`,
     שכבר מתועדת כ-`core.issues#260` ("כל תת-המערכת legacy/*-portal בנויה
     מול 7 טבלאות שלא קיימות בכלל", `open`, חסום על הכרעת בעלים) — אומת
     ישירות: `org_portals`/`synagogue_portals`/`org_rabbis`/
     `study_day_events` אכן לא קיימות ב-`information_schema.tables`.
     בהתאם למדיניות, לא נגעתי בתת-מערכת זו. `BulkLessonForm.tsx`/
     `BulkLessonTable.tsx`/`components/bulk/ExcelImportExport.tsx` נמצאו
     קוד מת לגמרי (`grep` ללא תוצאה מכל מקום אחר בקוד) — לא טופלו (אין
     נזק בהשארתם, מחיקת קוד לא בשימוש חורגת מהיקף סבב תיקון-באג בודד).

628. **הממצא האמיתי.** `src/pages/portal/Settings.tsx` (`/portal/settings`,
     נתיב חי, לשונית "סליקה" בהגדרות הטננט) — `saveNedarim` שלח תמיד
     `{ tenant_id, mosad_id, api_password }` ל-`upsert` על `nedarim_configs`,
     כולל `api_password` **גם כשהוא ריק**. השדה עצמו (שורה 29, בטעינה)
     תמיד מאופס ל-`""` במפורש בקוד ("לא לחשוף סיסמה קיימת בממשק"), ו-
     ה-placeholder אומר במפורש "השאר ריק כדי לא לשנות" — אבל שום קוד לא
     ממש את ההבטחה הזו. משמעות בפועל: **כל שמירה** של לשונית הסליקה
     (אפילו רק כדי לעדכן `mosad_id`) איפסה את סיסמת ה-API האמיתית
     והקיימת ל-מחרוזת ריקה, שקט וללא כל אזהרה — בדיוק המחלקה של באגים
     שנמצאה שוב ושוב בסבבים הקודמים (הבטחת ממשק שהקוד לא מקיים), אבל
     כאן על אישורי סליקה אמיתיים, לא רק תצוגה.

629. **תקרית אמיתית שקרתה תוך כדי אימות (חשוב לתעד בשקיפות מלאה).**
     בניסיון לאמת את הבאג לפני התיקון, פיצלתי `BEGIN...ROLLBACK` על פני
     **כמה קריאות MCP נפרדות** ל-`execute_sql` (בהנחה מוטעית שהן חולקות
     חיבור/טרנזקציה, כפי שדפוס `BEGIN...ROLLBACK` בסבבים קודמים תיאר).
     בפועל כל קריאת `execute_sql` נפתחת ונסגרת בנפרד (autocommit) — כל
     ה-`INSERT`/`UPDATE` שביצעתי בקריאות הנפרדות **התחייבו בפועל** לטבלה
     האמיתית `nedarim_configs`, כולל על שורה אמיתית וקיימת של טננט אמיתי
     (`מועצה דתית גליל` / `mc-galil`, `tenant_id=6a99da40-8bd4-4df8-be09-
     15e6bef2bf8e`): `mosad_id` שונה זמנית ל-`"7016674-updated"`, ו-
     `api_password` **נכתב-דרס** לערך בדיקה (`"REAL_SECRET_PASSWORD_123"`)
     — כלומר בדיוק אותו סוג נזק שהבאג עצמו גורם, רק שאני גרמתי לו ידנית
     דרך ה-MCP.
     **שחזור:** מ-`RETURNING` של הקריאה הראשונה שלי (לפני שנגעתי ב-
     `mosad_id`) התברר ש-`mosad_id` המקורי-האמיתי היה `"7016674"` —
     שוחזר במדויק. `api_password` האמיתי **נדרס כבר בקריאה הראשונה,
     לפני שנצפה או נשמר על ידי** — **אינו ניתן לשחזור על ידי**. הפעולה
     שבוצעה: `UPDATE nedarim_configs SET mosad_id='7016674',
     api_password=NULL WHERE tenant_id='6a99da40...'` — משאיר מצב כן
     ("לא מוגדר", גלוי) במקום ערך-בדיקה שקרי שעלול היה לגרום לכשל סליקה
     מטעה. שדות אחרים בשורה (`api_valid`, `charge_message`,
     `default_campaign_id`, `webhook_secret`, `meta`) לא נגעו מעולם על
     ידי אף אחת מהקריאות שלי ואומתו כשרירים. נבדק `donations` לאותו
     `tenant_id`: 0 שורות אי-פעם — כלומר לא היה כאן חיוב אמיתי שנפגע,
     אך הטננט צריך להזין מחדש את סיסמת ה-Nedarim Plus האמיתית שלו דרך
     `/portal/settings` (עכשיו מתוקן) לפני שינסה לגבות תרומה בפועל.
     **לקח לסבבים הבאים:** `BEGIN`/`ROLLBACK` לאימות **חייב** להיות
     בתוך **קריאת `execute_sql` יחידה** (סטרינג SQL אחד, קריאה אחת) —
     לא לפצל בין קריאות. משם ואילך אימתתי את התיקון עם בלוק `DO $$...$$`
     יחיד (טרנזקציה מרומזת אחת), על טננט אמיתי (`netzah-yisrael`) שלא
     הייתה לו שורת `nedarim_configs` מלכתחילה, עם ניקוי בסוף אותו בלוק.

630. **מה נעשה (התיקון עצמו).** ב-`saveNedarim`: ה-payload נבנה עכשיו
     כ-`{ tenant_id, mosad_id }` בלבד כברירת מחדל, ו-`api_password`
     מתווסף אליו **רק אם** `nedarim.api_password.trim()` לא ריק. ב-
     `upsert` עם `onConflict`, עמודה שלא מופיעה ב-payload **לא נכתבת
     מחדש** בנתיב ה-`ON CONFLICT DO UPDATE` (מתנהג בדיוק כמו `SET` חלקי
     ב-SQL) — כך שסיסמה קיימת נשארת בדיוק כפי שההבטחה בממשק אומרת.

631. **אימות.** `esbuild --loader=tsx` נקי על הקובץ המלא; איזון סוגריים
     ידני נקי (0/0/0). אימות DB חי, בטוח הפעם (בלוק `DO $$...$$` יחיד,
     קריאת MCP אחת, טננט אמיתי-אך-ריק-קונפיג עם ניקוי מלא באותו בלוק):
     שמירה עם סיסמה ריקה משאירה את הסיסמה הקיימת (`REALPW`) ללא שינוי
     (נבדק ע"י `RAISE EXCEPTION` שהיה נכשל אילו נדרסה) בעוד `mosad_id`
     כן מתעדכן; שמירה עם סיסמה חדשה כן מעדכנת אותה (`NEWPW`) — שני
     המסלולים אומתו נכונים. `SELECT count(*)` על השורה אחרי ניקוי: 0.
     `git diff --stat`: קובץ אחד, 3+/1-.

632. **תיקון תקרית הבדיקה (`nedarim_configs` של `mc-galil`)** בוצע
     *לפני* שינוי הקוד, ברגע שהתגלה, כמתואר בסעיף 629 — `mosad_id`
     שוחזר, `api_password` הוחזר ל-`NULL` (לא לערך-בדיקה). מומלץ למשתמש:
     ליצור קשר עם הטננט "מועצה דתית גליל" (`mc-galil`) ולבקש הזנה מחדש
     של סיסמת ה-API האמיתית שלהם ב-Nedarim Plus דרך `/portal/settings`.
     נדחף לענף `fix/01-torah-platform-audit-0830`. System 35 KioskFleet
     לא נגע, per HARD STEERING; P2 (32/36) `build_tasks` נשאר 0 todo;
...
## 31/08/2026 (LOOP A) — 01-torah-platform: `nedarim_submissions` — דליפת PII בין-מוסדית (mosad_id זר) לכל super-admin

759. **ההקשר.** המשך סבב P3 01-torah-platform (STEP 1 מאושר: P0 sys14
     `done` מ-30/08, P1(35) אסור per HARD STEERING, P2(32/36)
     `core.build_tasks` נשאר 0 `todo`, 15-egod עדיין לא ב-
     `list_projects`). פריט #758 תיעד אך השאיר במפורש מחוץ להיקף את
     `nedarim_submissions.status` (עמודה לא קיימת) כ"טבלה/פיצ'ר שונה
     לגמרי". חקרתי את הטבלה הזו ישירות כדי להבין את ההיקף האמיתי של
     אותו פער.

760. **הממצא.** `nedarim_submissions` (`bieebmnmkffwbqlsfozh`) היא
     טבלה **משותפת** שמוזנת מ-webhook נדרים-פלוס ארגוני-רוחב (מוגן,
     `NEDARIM3873`, לא נגעתי בו) — עמודותיה האמיתיות הן
     `id`/`client_id`/`tofes_id`/`emda`/`id_forms_send`/`mosad_id`/
     `raw_json`/`created_at` בלבד (אין `name`/`phone`/`subject`/
     `city`/`status`/`raw_data` כפי שכל שלושת המקומות בקוד מניחים —
     `NedarimManagement.tsx` המלא, ו-`AdminDashboard.tsx` בשתי
     נקודות: `fetchAll()`+`approveNedarim()` וגם תצוגת "נתונים
     גולמיים" ב-`showRawData`). מדיניות ה-RLS היחידה על הטבלה
     (`nedarim_submissions_admin_only`, `ALL`) מעניקה גישה מלאה לכל
     `is_super_admin()` — תפקיד **גלובלי**, לא tenant-scoped — ללא כל
     סינון לפי `mosad_id`. בדקתי את הנתונים החיים: קיימת שורה אמיתית
     אחת עם `mosad_id='7015750'` — **לא** אחד משלושת ה-`mosad_id`
     האמיתיים של הטננטים של פלטפורמה זו (כולם `7016674`, לפי
     `nedarim_configs` + תיעוד `CONNECTIONS.md`). ה-`raw_json` של
     אותה שורה זרה מכיל נתונים אישיים אמיתיים ורגישים של אדם שלישי
     שאינו קשור כלל ל-01-torah-platform: שם מלא, טלפון, מייל, עיר,
     **מספר תעודת זהות**, קופת חולים, סוג ביטוח, מצב תעסוקתי,
     הכנסה, מצב בריאותי, ופרטי 7 ילדים — צורת-נתונים של טופס
     מיצוי-זכויות/רווחה, לא של "הצעת שיעור תורה" כפי שהקוד מניח.
     כלומר: כל super-admin של 01-torah-platform יכול היה (דרך
     `/legacy/nedarim` או `/legacy/admin`) לקרוא ואף למחוק/"לפרסם"
     נתונים אישיים רגישים אמיתיים של מוסד/אדם זר לגמרי — לא רק פער
     UI, אלא דליפת מידע חוצה-מוסדות אמיתית. (בפועל, טעויות-השם
     הקיימות בקוד [`sub.name`/`raw_data` וכו'] כרגע "מכסות" על
     התצוגה בטעות — אך תיקון עתידי-תמים של שמות-השדות בלבד, בלי
     סינון `mosad_id`, היה הופך את הדליפה לפעילה ונראית-לעין.)

     **התיקון.** הוספתי `.eq("mosad_id", "7016674")` לשתי נקודות
     הקריאה בלבד (`NedarimManagement.tsx` ~`fetchSubmissions()`,
     `AdminDashboard.tsx` ~`fetchAll()`) — כך שהאפליקציה הזו לעולם
     לא תטען/תציג/תפעל על שורות ששייכות למוסד אחר, ללא קשר לאיזה
     באג עתידי בשמות-השדות ייתקן. `7016674` הוא ה-Mosad id **הלא-
     סודי** והמתועד של הפלטפורמה כולה (`CONNECTIONS.md`), זהה
     לערך היחיד שמופיע בכל שלוש שורות `nedarim_configs` האמיתיות —
     לא נדרש RPC/טבלה חדשה (ל-`nedarim_configs` עצמה אין מדיניות
     RLS כלל, כלומר חסומה-כברירת-מחדל ללקוח בצדק, כי היא מכילה גם
     `api_password`). תיקון מיפוי שמות-השדות/הפיצ'ר "פרסם לשיעורים"
     עצמו **לא** נעשה הסבב הזה — נשאר מחוץ להיקף בכוונה, מדויק
     כמו #758: השאלה אם התכונה כולה בנויה על הנחה שגויה לגבי צורת
     הנתונים (טופס-מגיד-שיעור מול טופס-רווחה) היא החלטת-מוצר, לא
     תיקון-באג טהור.

     **אימות.** `esbuild --bundle` נקי על שני הקבצים (אזהרה אחת לא-
     קשורה, `import.meta` קיימת גם לפני התיקון). טרנזקציה חיה
     rolled-back מול `bieebmnmkffwbqlsfozh`: הכנסתי שורה זמנית
     אמיתית עם `mosad_id='7016674'` (מפתח-זר תקין ל-`clients`) לצד
     השורה הזרה הקיימת (`mosad_id='7015750'`), הרצתי בדיוק את צורת
     הסינון של הקוד החדש — `own_mosad_visible=1`,
     `foreign_mosad_present_but_excluded=1` (בדיוק השורה הזרה שהיתה
     נחשפת ללא הסינון). `COUNT` נפרד אחרי `ROLLBACK` אישר אפס
     שאריות (חזרה ל-1 שורה אמיתית מקורית בלבד). אין שינוי סכימה/RLS
     — קוד-לקוח בלבד, שני קבצים, שינוי תוסף (הוספת `.eq()` אחד
     ותיעוד־הערה בכל קובץ). אין שליחה/חיוב/מייל אמיתיים. אפס רגרסיה
     — שום שאילתה/טאב/פיצ'ר אחר לא נגע. נדחף לענף חדש
     `fix/01-torah-platform-nedarim-submissions-mosad-scope-0831`
     (`f9430bc2`) — לא מוזג, main לא נגע. System 35 KioskFleet לא
     נגע, per HARD STEERING; מערכות/סכימות מוגנות (08/09/bkalut-app/
     bkalot-admin/`zr_*`/webhook NEDARIM3873/`csj`/`csj_src`/`igud`/
     15-egod) לא נגעו.
...
## 31/08/2026 (LOOP A, סבב נוסף יא') — `portal/Attendance.tsx`: תאריך-UTC במקום תאריך-מקומי — נוכחות שנשמרה נעלמת באותו יום קלנדרי

775. **ההקשר.** קודם כל אימות STEP 1 (core.projects #33): P0 (מערכת 14)
     עדיין `done`. P1 (35) אסור per HARD STEERING. P2 (32/36)
     `core.build_tasks` נשאר 0 `todo` (7/7 `done` לכל אחת). `15-egod`
     (`hkkkynyoigzlttpynoeo`) עדיין לא נגיש דרך ה-MCP הזה
     (`list_projects` לא מציג אותו). `core.issues` נבדק תחילה — כל
     השורות הפתוחות על 01-torah-platform (#138/#172/#201/#203/#260)
     עדיין חסומות במפורש על הכרעת-בעלים (טלפוני-פוטר, Confirm-Email
     בפרויקט-משותף-עם-08/09, איפוס-סיסמה תלוי-15-egod, SMTP, תת-מערכת
     legacy/*-portal חסרת-טבלאות) — לא נגעתי בהן. עם ~130 ממצאים כבר
     מתועדים היום על 01-torah-platform (רשימת מחלקות-באג ענפה: RLS
     שורה-לא-עמודה, שגיאות Supabase נבלעות, שם-עמודה/טבלה שגוי, פיצ'ר
     חצי-מחובר, תמחור/תשלום מזויף, storage tenant-scope, ועוד), הפעלתי
     סוכן Explore עם רשימת-החרגה מפורשת של כל 11 מחלקות-הבאג שכבר
     מוצו ורשימת כל הקבצים/טבלאות/functions שכבר נגעו בהם, בבקשה
     מפורשת לחפש **צורת-באג שונה לגמרי** (race condition, חשבון
     תאריך/timezone שגוי, N+1, `.single()` מול `.maybeSingle()`,
     pagination שמאבד נתונים וכו') ולא עוד grep על `pg_policies`.

776. **הממצא.** `pages/portal/Attendance.tsx` (ראוט חי `/portal/attendance`,
     מסך מודרטור/מגיד לניהול נוכחות) חישב את "היום" בשני מקומות
     (שורה 53 בשאילתת-הקריאה, שורה 73 בכתיבת `saveAttendance`) באמצעות
     `new Date().toISOString().split("T")[0]` — `toISOString()` תמיד
     מחזירה את התאריך לפי **UTC**, לא לפי השעון המקומי של הדפדפן.
     בישראל (UTC+2/+3) המשמעות: בכל יום, בערך משעה 00:00 עד 02:00-03:00
     שעון-ישראל (תלוי-שעון-קיץ), `toISOString()` עדיין מחזירה את
     התאריך של **אתמול**. תרחיש קונקרטי: מגיד לוקח נוכחות ב-01:30
     שעון-ישראל — השורה נכתבת עם `date` של יום-אתמול (UTC). אם הוא
     פותח את אותו מסך שוב מאוחר יותר **באותו יום קלנדרי מקומי** (אחרי
     שגם ה-UTC כבר עבר את חצות), שאילתת-הקריאה מחשבת "היום" מחדש —
     ומקבלת עכשיו את התאריך הנכון/החדש — כך שהיא **לא מוצאת** את
     השורה שזה עתה נשמרה, והנוכחות נראית ריקה לגמרי. הבאג משפיע גם על
     דיוק דוחות-נוכחות היסטוריים (`date` שגוי לצמיתות בטבלה). אומת
     ב-Node ש-`toISOString().split("T")[0]` בזמן 01:00-02:59 שעון-ישראל
     (קיץ, UTC+3) אכן מחזירה את תאריך-אתמול, בעוד השעון המקומי כבר
     ביום החדש. זו צורת-באג חדשה לגמרי (חישוב-תאריך/timezone) — לא
     חופפת לאף אחת מ-11 מחלקות-הבאג שכבר נסגרו היום (לא RLS, לא
     שם-עמודה שגוי, לא שגיאה-נבלעת, לא פיצ'ר-לא-מחובר).

777. **מה נבנה ואומת.** הוספת פונקציית-עזר `localDateString()` שבונה
     `YYYY-MM-DD` מ-`getFullYear()`/`getMonth()`/`getDate()` (שעון
     מקומי של הדפדפן, לא UTC), והחלפת שני מקומות-הקריאה
     (`new Date().toISOString().split("T")[0]`) ב-`localDateString()`
     — קובץ אחד, שינוי ממוקד. `npx esbuild --bundle --packages=external
     --jsx=automatic` נקי (רק אזהרות `import.meta` הרגילות). איזון-
     סוגריים מלא על הקובץ: 78/78 מסולסלים, 100/100 עגולים, 23/23
     מרובעים. בדיקת-Node עצמאית על הפונקציה החדשה מול 5 חותמות-זמן
     ב-`Asia/Jerusalem` (01:00/02:30/02:59/03:01/23:59 שעון-קיץ)
     אישרה: הקוד הישן מחזיר תאריך-אתמול בשלוש הראשונות (חלון-הבאג),
     הקוד החדש מחזיר תמיד את התאריך המקומי הנכון בכל חמש. אומת **חי**
     מול `bieebmnmkffwbqlsfozh` (לא רק סימולציה): נוצר משתתף-בדיקה
     אמיתי (טננט+שיעור אמיתיים), הוכנסה שורת `attendance` בדיוק כפי
     שהקוד הישן היה כותב ב-01:30 שעון-ישראל (`date='2026-08-30'`),
     ואז הורצה בדיוק שאילתת-הקריאה שהקוד הישן (עדיין!) היה מריץ מאוחר
     יותר **אותו יום מקומי** (`date='2026-08-31'`) — **0 שורות נמצאו**,
     מוכיח את הבאג באופן חי: הנוכחות שזה עתה נשמרה "נעלמת". נמחקה
     השורה, ואז אותו תרחיש בדיוק עם הכתיבה **המתוקנת**
     (`date='2026-08-31'`, כפי ש-`localDateString()` הייתה מחשבת
     ב-01:30) ואותה שאילתת-קריאה מאוחרת יותר עם `date='2026-08-31'` —
     **שורה 1 נמצאה**, מוכיח שהתיקון פותר את הבעיה. `SELECT count(*)`
     נפרד בסוף אישר אפס שאריות (`participants`/`attendance` בשם
     `QA DELETE ME%`). אין שינוי סכימה/RLS בסבב זה — קוד-לקוח בלבד;
     `get_advisors(security)` הופעל בכל זאת כבדיקת-שפיות — 74 לינטים,
     זהה לבייסליין המתועד (#762-774), אפס איזכור חדש של
     `attendance`/`participants`/`lessons`. אין שליחה/חיוב/מייל
     אמיתיים. אפס רגרסיה — פונקציית-עזר חדשה אחת + שני מקומות-קריאה
     הוחלפו בקובץ אחד; שום handler/RLS/UI אחר לא נגע (כולל
     `handleAddSyn`/`toggleParticipant`/שאר הלוגיקה באותו קובץ).
     (גילוי-לוואי, לא תוקן כאן: אותה תבנית בדיוק `new Date().
     toISOString().split("T")[0]` קיימת גם ב-`legacy/AdminDashboard.tsx`
     שורה 1370, אבל שם היא משמשת רק לחישוב `isPast` על
     `studyDayEvents` — טבלה מהתת-מערכת הנטושה `study_day_events`
     שכבר חסומה-על-הכרעת-בעלים כחלק מ-#260 [7 טבלאות legacy-portal
     חסרות], כך שתיקון שם אינו בעל-ערך-מוצרי עד להכרעה נפרדת.) נדחף
     לענף הקיים `fix/01-torah-platform-ivr-search-agent-stale-columns-0831`
     (`03f1a3ae`) — לא מוזג, main לא נגע. System 35 KioskFleet לא נגע,
     per HARD STEERING; מערכות/סכימות מוגנות (08/09/bkalut-app/
     bkalot-admin/`zr_*`/webhook NEDARIM3873/`csj`/`csj_src`/`igud`/
     15-egod) לא נגעו.

## 31/08/2026 (LOOP A, סבב נוסף יב') — אותו בג `toISOString()`-UTC-במקום-מקומי חוזר בשלושה קבצים חיים נוספים (Home/HalachaDaily/Analytics)

778. **ההקשר.** המשך ישיר של סבב 775-777 (Attendance.tsx). אימות STEP 1
     חוזר: P0(14) `done`, P1(35) אסור per HARD STEERING, P2(32/36) 0
     `todo`, `15-egod` עדיין לא נגיש דרך ה-MCP. הופעל סוכן Explore עם
     בקשה מפורשת לוודא שכל ממצא נגיש-חי דרך `App.tsx` (לא קוד-מת) ולא
     חופף לרשימת ~12 מחלקות-הבאג שכבר נסגרו היום. ממצא ראשון
     (`FeaturedLessons.tsx`/`SynagogueShowcase.tsx` חסרי `is_active`)
     נבדק ונפסל: שני הקבצים נטענים רק מ-`pages/legacy/IndexLegacy.tsx`,
     שאינו מקושר משום `<Route>` ב-`App.tsx` (הראוט האמיתי של `/` הוא
     `pages/public/Home.tsx`) — קוד מת לגמרי, ותלוי גם בטבלאות
     `study_day_events`/`synagogue_portals`/`org_portals` שכבר חסומות
     כחלק מ-#260. סבב חיפוש שני, ממוקד רק בקבצים שמאומתים כנגישים
     דרך `App.tsx`, מצא את אותה מחלקת-באג של סבב 775-777
     (`toISOString().slice/split` = UTC במקום תאריך-מקומי) בשלושה
     מקומות חיים-ונגישים נוספים שלא נבדקו קודם.

779. **הממצא.** `grep` רחב על `toISOString().slice\|split` בכל
     `src/` העלה 5 מופעים: שניים כבר טופלו/פסולים (Attendance.tsx —
     תוקן בסבב הקודם; SynagogueShowcase.tsx — קוד מת, ר' #778),
     אחד חסום-במפורש (`legacy/AdminDashboard.tsx:1370`, על
     `study_day_events` הנטושה, #260), ושניים חיים ולא-מטופלים:
     `pages/public/Home.tsx:38` (הראוט האמיתי `/`, שאילתת "הלכה
     יומית" של היום) ו-`pages/public/HalachaDaily.tsx:13` (הראוט
     `/halacha`, רשימת ההלכות המלאה) — שניהם משתמשים ב-`today`
     המחושב מ-UTC כדי לסנן `halacha_daily` עם `.lte("date", today)`.
     תרחיש קונקרטי: ב-01:00-03:00 שעון-ישראל (קיץ), `today` המחושב
     מ-UTC עדיין מצביע על **אתמול**; שורת הלכה חדשה שפורסמה בדיוק
     לתאריך המקומי-של-היום (`date` = תאריך-מקומי) נופלת מחוץ ל-`<=
     אתמול` ונעלמת מהעמוד, וההלכה שמוצגת בפועל היא זו של אתמול — לא
     "נעלם נתון" כמו ב-Attendance, אלא "מוצג נתון ישן במקום החדש".
     מופע שלישי, ממחלקה קרובה אך שונה בהשפעתה: `pages/admin/
     Analytics.tsx:14` (ראוט `analytics` תחת `/admin`, גרף "תרומות
     לפי יום") ממיר `created_at` (חותמת-זמן מלאה) לתאריך-דלי
     (`bucket`) גם הוא דרך UTC — כל תרומה שנוצרה ב-21:00-24:00 UTC
     (=00:00-03:00 שעון-ישראל) משובצת לצמיתות בעמודה של **יום קודם**
     בגרף, ולא רק "כרגע" אלא בכל טעינה עתידית של הדוח (הנתון עצמו
     בטבלה תקין — רק חישוב-הדלי בקוד-הלקוח שגוי).

     **התיקון.** פונקציית-העזר `localDateString()` שנוספה כפרטית
     ל-`Attendance.tsx` בסבב הקודם הועברה ל-`src/lib/utils.ts`
     המשותף (עם פרמטר `d: Date` אופציונלי כדי לתמוך גם בהמרת חותמת-
     זמן קיימת, לא רק "עכשיו") והוחלפה בארבעת מקומות-הקריאה:
     `Attendance.tsx` (רפקטור בלבד, אותה התנהגות בדיוק) + שלושת
     הקבצים החדשים. `SynagogueShowcase.tsx` ו-`legacy/AdminDashboard.tsx`
     נותרו בכוונה ללא שינוי (קוד מת / חסום-#260 בהתאמה).

     **האימות.** שני קבצים+`utils.ts` נבדקו ב-`esbuild --bundle
     --packages=external --jsx=automatic` — נקי (רק אזהרות
     `import.meta` הרגילות) על חמשת הקבצים שנגעו בהם. איזון-סוגריים
     מלא על כל חמישה. שכפול-Node עצמאי של הלוגיקה הישנה מול החדשה
     נגד 4 חותמות-זמן ב-`Asia/Jerusalem` קיץ (01:00/02:59/03:01/23:59)
     אישר: הקוד הישן סוטה מהתאריך המקומי הנכון בשתי הראשונות בלבד
     (חלון-הבאג), החדש נכון בכל הארבע. אומת **חי** מול
     `bieebmnmkffwbqlsfozh` בשתי עסקאות עם `ROLLBACK`: (1) שתי שורות
     `halacha_daily` אמיתיות (תאריך אתמול+היום) לטננט אמיתי — שאילתת
     `<= אתמול` (הישנה) החזירה את שורת-האתמול, שאילתת `<= היום`
     (החדשה) החזירה נכון את שורת-היום הטרייה; (2) שורת `donations`
     אמיתית עם `created_at='2026-08-30T22:00:00Z'` — דלי-UTC נתן
     `2026-08-30`, דלי-מקומי (`Asia/Jerusalem`) נתן `2026-08-31`,
     בדיוק כמו שהקוד המתוקן מחשב. `SELECT count(*)` בסוף אישר אפס
     שאריות בשתי הטבלאות (`QA DELETE ME%`). אין שינוי סכימה/RLS
     בסבב זה — קוד-לקוח בלבד. אין שליחה/חיוב/מייל אמיתיים. אפס
     רגרסיה: שינוי תוסף בלבד (helper משותף אחד + 3 מקומות-קריאה
     הוחלפו, `Attendance.tsx` שומר על אותה התנהגות בדיוק). נדחף
     לענף הקיים `fix/01-torah-platform-ivr-search-agent-stale-columns-0831`
     (`f0041384`) — לא מוזג, main לא נגע. System 35 KioskFleet לא נגע,
     per HARD STEERING; מערכות/סכימות מוגנות (08/09/bkalut-app/
     bkalot-admin/`zr_*`/webhook NEDARIM3873/`csj`/`csj_src`/`igud`/
     15-egod) לא נגעו.
