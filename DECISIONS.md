...
## 20/08/2026 — סבב 172 (loop A)

838. **שתי עדשות חדשות נבדקו ויצאו נקיות.** לפני פתיחת עדשת
     ניגודיות צבעים, נבדקו שני המועמדים הראשונים מתור סבב 171:
     (א) קישורי `target="_blank"` בלי `rel="noopener"/"noreferrer"`
     — נסרקו כל 56 המופעים ב-01-16 (לא כולל 08/09 המוגנות), כולם
     כבר עם `rel` תקין; (ב) כפתורים בתוך `<form>` בלי `type="button"`
     מפורש (עלולים לשלוח את הטופס בטעות) — נסרקו כל האפליקציות,
     לא נמצא אף כפתור-פעולה-לא-שליחה בלי `type` מפורש. שתי העדשות
     נסגרות ריקות (ללא תיקון נדרש).
839. **עדשת ניגודיות צבעים (WCAG AA) — נמצא ממצא אמיתי.** נבדקו
     שילובי טקסט/רקע נפוצים (`text-gray-300/400`, `text-slate-400`
     וכו') מול הצבעים המוגדרים בפועל ב-`tailwind.config`/CSS של כל
     אפליקציה, עם חישוב יחס ניגודיות (relative luminance) ולא ניחוש.
     רוב האפליקציות נקיות (01/02/04/12/15/16). ריכוז ממצאים אמיתי
     ב-**03-igud-ads**: `text-gray-400` (`#9ca3af`) על רקעים
     לבנים/אפורים-בהירים (`bg-white`/`bg-gray-100`) מחשב ל-~2.3-2.5:1
     — מתחת לסף 4.5:1 לטקסט רגיל — פרוס על פני כ-26 מופעים ב-11
     קבצים (בעיקר עמודי אדמין + עמוד "המודעות שלי" הציבורי).
840. **תוקן העמוד הראשון בתור: `app/my/projects/page.tsx` (ציבורי,
     "המודעות שלי").** שני מופעי `text-gray-400` (תיבת placeholder
     סטטוס "בעיבוד.../שגיאה/ממתין" בשורה 48, וחותמת זמן בשורה 62)
     הוחלפו ל-`text-gray-600` (`#4b5563`) — לא ניחוש: אותו צבע כבר
     בשימוש בפועל בכותרת המשנה של אותו עמוד (שורה 29). אפס רגרסיה:
     `git diff --stat` קובץ אחד, 2+/2-, שינוי מחלקת צבע בלבד, אין
     שינוי ב-JSX/state/fetch logic. איזון סוגריים בפייתון: תואם
     (21/21 מסולסל, 24/24 עגול, 6/6 מרובע). הנתיב חסום ע"י
     `.gitignore` — נדרש `git add -f` (תקדים קבוע). Commit
     `588b7c1f` על `fix/a-icon-only-buttons-round2-0820`, נדחף
     ל-origin (מפעיל פריסת Vercel תחת more30.com/modaot).
841. **הבא בתור בעדשה הזו:** יתר קבצי 03-igud-ads עם אותו ממצא —
     עמודי אדמין `payments/`, `projects/[id]/`, `projects/`,
     `stats/`, `users/`, `audit/`, `templates/[id]/`, `templates/`,
     `transcripts/`, `settings/`, `notifications/` (כולם
     `text-gray-400` על רקע בהיר), וכן `text-gray-300`
     (`projects/[id]/page.tsx:81`) ו-`text-slate-400` (2 מופעים ב-02
     ו-03, טקסט גדול/מודגש, סף 3:1). אחרי סגירת 03 — לבדוק אם
     האפליקציות האחרות (03 חוץ, 05-07/10/11/13/14 טרם נסרקו במלואן)
     חולקות אותו דפוס.

## 20/08/2026 — סבב 173 (loop A)

842. **סגירת עדשת ניגודיות צבעים על כל 03-igud-ads + שני המופעים
     של `text-slate-400` (02+03).** תוקנו 12 קבצי `text-gray-400`
     שנותרו מסבב 172 (עמודי אדמין `payments`, `projects` + `[id]`,
     `transcripts`, `stats`, `users`, `audit`, `templates` + `[id]`,
     `settings`, `notifications` הציבורי, `NotificationBell.tsx`),
     `text-gray-300` היחיד (`projects/[id]/page.tsx:81`, מפריד "|"
     דקורטיבי בין קישור לכותרת), ושני מופעי `text-slate-400`
     בכפתורי סגירת מודל ("×", `text-2xl`) — אחד ב-
     `03-igud-ads/app/(admin)/admin/transcribe/uploads/page.tsx`
     ואחד ב-`02-igud-transcribe/app/(admin)/admin/uploads/page.tsx`
     (אותו קומפוננט מודל בשתי האפליקציות).
843. **חישוב ניגודיות מאומת, לא ניחוש:** `#9ca3af` (gray-400) על
     רקע לבן/`bg-surface` ≈ 2.54:1 — נכשל גם בסף הטקסט הגדול
     (3:1). `#94a3b8` (slate-400) בגודל `text-2xl` ≈ 2.57:1 — נכשל
     גם 3:1. שני הצבעים הוחלפו ל-`text-gray-600`/`text-slate-600`
     (`#4b5563`≈7.56:1, `#475569`≈7.59:1) — אותו יעד כמו סבב 172,
     עובר AA בנוח גם לטקסט רגיל. `text-gray-500` (הצמד הקיים
     בעמודי הפרטים לשדות כמו "מזהה"/"מסלול") נבדק בנפרד ≈4.83:1 —
     עובר בקושי, לא נגע בו (לא היה חלק מהממצא).
844. **תיקון לוואי:** ב-`NotificationBell.tsx` שורה 147 היה
     `text-gray-400 hover:text-gray-600` — ה-sed להחלפת הבסיס
     היה הופך את זה ל-`text-gray-600 hover:text-gray-600` (hover
     חסר אפקט). תוקן ידנית ל-`hover:text-gray-800` כדי לשמר את
     אפקט ה-hover המקורי.
845. **אפס רגרסיה מאומתת:** כל 14 השינויים הם החלפת שם-מחלקת
     Tailwind בלבד (`git diff` — 31+/31-), ללא שינוי JSX/state/
     fetch logic מלבד תיקון ה-hover הנקודתי לעיל. `grep` על כל
     01-16 מאמת: אין יותר `text-gray-400`/`text-slate-400` נותרים
     בטווח הסמכות. הנתיבים חסומים ע"י `.gitignore` — נדרש
     `git add -f` (תקדים קבוע). Commit `6abc8e78` על
     `fix/a-icon-only-buttons-round2-0820`, נדחף ל-origin (מפעיל
     פריסת Vercel תחת more30.com/modaot ו-more30.com/imud-torani).
846. **מצב העדשה:** עדשת ניגודיות צבעים סגורה על 02-igud-transcribe
     ו-03-igud-ads. **הבא בתור:** לסרוק את יתר האפליקציות
     (05-07/10/11/13/14 טרם נסרקו במלואן לניגודיות; 01/04/12/15/16
     כבר נבדקו נקיים בסבב 172) לאותו דפוס `text-gray-400`/
     `text-gray-300`/`text-slate-400`, או לפתוח עדשה חדשה, או
     לחזור ל-`core.project_tasks`/`core.project_bugs` (5 פריטים
     פתוחים, עדיין חסומים).

## 20/08/2026 — סבב 174 (loop A)

847. **05-07/10/11/13/14 אינם Tailwind — נבדקו ישירות ב-CSS גולמי.**
     שש האפליקציות הנותרות (05-financial-marketing-site,
     06-kupot-holim, 07-zol [אין CSS], 10-bkalot-rights,
     11-bkalut-marketing2, 13-property-identity, 14-bsmachot-plus)
     הן אתרים סטטיים (HTML/CSS/JS) בלי Tailwind — `grep` על
     `text-gray-*`/`text-slate-*`/`tailwind` לא החזיר כלום. במקום
     זאת נבדקו ישירות ערכי `color:#...` הגולמיים ב-CSS מול הרקעים
     בפועל (חישוב ניגודיות relative-luminance כמו בסבבים 172-173).
848. **ממצא אמיתי ב-06-kupot-holim/site/styles.css.** לקובץ הזה כבר
     היה מעבר ניגודיות קודם ומתועד (הערות דו-לשוניות עם יחסים
     מדודים לכל טוקן, כולל השוואה בין ערכת אור/חושך) — אך שני ערכים
     פרטניים נשארו מתחת לסף: (א) `--text-faint:#93A2AB` בערכת האור
     על `--surface:#FFFFFF` ≈ **2.63:1** (בשימוש ב-`.topic-src` ו-
     `.form-note`, שניהם 12px טקסט רגיל — נכשל אפילו מול סף הטקסט
     הגדול); (ב) `.footer-note .muted{color:#6E7C83}` על
     `--footer-bg:#16232B` הקבוע (לא מתהפך בין ערכות, ראה הערה
     בקוד) ≈ **3.72:1** — נכשל 4.5:1.
849. **תיקון:** `--text-faint` → `#6A7580` (≈4.70:1 על לבן; ערכת
     החושך `#7B8B95`≈4.75:1 לא נגעתי בה — כבר עוברת). `.muted` בפוטר
     → `#7E8F97` (≈4.78:1 על `#16232B`). שני הערכים נבחרו לא בניחוש
     אלא בסריקת גוון/רוויה זהים לצבע המקורי עם עמעום/הבהרה מדורג עד
     חצייה ברורה של 4.5:1 (כמו בסבב 172-173).
850. **אפס רגרסיה מאומתת:** `git diff --stat` — קובץ אחד, 2+/2-,
     שינוי ערכי צבע בלבד (משתנה CSS + כלל אחד), אין שינוי במבנה/
     סלקטורים/לוגיקה. איזון סוגריים מאומת על הקובץ המלא (198/198
     מסולסל). הנתיב `apps/06-kupot-holim/site` חסום ע"י
     `.gitignore` — נדרש `git add -f` (תקדים קבוע). Commit
     `83c59820` על `fix/a-icon-only-buttons-round2-0820`, נדחף
     ל-origin (מפעיל פריסת Vercel תחת more30.com/kupot-holim).
851. **מצב העדשה:** עדשת ניגודיות צבעים נסגרת ריקה על 05/07/10/11/
     13/14 (אין Tailwind, ובדיקת CSS גולמי לא העלתה ממצא נוסף מעבר
     ל-06 שתוקן כאן). **הבא בתור:** לסרוק שאר קבצי ה-CSS/HTML
     הגולמיים של 01-04/12/15/16 (נבדקו רק מול Tailwind, לא מול
     inline-style/CSS-vars גולמיים כמו ב-06), או לפתוח עדשה חדשה
     (למשל: alt-text על תמונות, focus-visible states, heading-order
     סמנטי), או לחזור ל-`core.project_tasks`/`core.project_bugs` (5
     פריטים פתוחים, עדיין חסומים).

## 20/08/2026 — סבב 175 (loop A)

852. **שתי עדשות חדשות נבדקו — אחת ריקה, אחת עם ממצא אמיתי.**
     נבדקה תחילה עדשת alt-text חסר על תמונות (WCAG 1.1.1) בכל
     01-16 (דילוג 08/09): 52 מופעי `<img>` בסך הכל, כולם כבר עם
     `alt` (ריק מכוון או דינמי או תיאורי) — נסגרת ריקה. לאחר מכן
     נבדקה, כמתוכנן מסבב 174, עדשת ניגודיות צבעים על ערכי CSS
     גולמיים (משתני `--x:#hex`/`hsl(...)` ו-`color:` פשוטים, לא
     מחלקות Tailwind) באפליקציות 01/02/03/04/12/15/16 — נמצא ממצא
     אמיתי אחד שאומת ידנית (החישוב הראשוני של סוכן-הסריקה על 4
     מועמדים נוספים נבדק וכשל: 3 מהם התבססו על גרדיאנטים/שקיפות
     מעורבבים שקשה לאמת במדויק, ואחד (12-smel-ndln Premium.tsx
     dark-mode) התברר כשגוי בבדיקה חוזרת — `hsl(43 58% 62%)` על
     כרטיס `--card` כהה (`hsl(218 40% 12%)`) מחשב ל-**8.84:1**,
     עובר בנוח, לא נגעתי בו).
853. **הממצא שאומת: `12-smel-ndln/client/src/pages/Home.tsx:130`.**
     מחלקת `.gold-text` (מוגדרת ב-`index.css:222-227`) מתחלפת בין
     ערך בהיר-מדי-לרקע-כהה (`hsl(43 62% 32%)`, ברירת מחדל) לערך
     בהיר (`hsl(43 58% 62%)`, תחת `.dark`) — אך רקע ה-hero ב-
     `Home.tsx` (`bg-gradient-to-b from-primary to-[hsl(218_50%_22%)]
     dark:from-[hsl(218_44%_11%)] dark:to-background`) הוא **כחול-
     נייבי כהה בשני מצבי התצוגה גם יחד**, לא רק ב-dark mode. כתוצאה
     מכך, במצב האור (ברירת המחדל של האתר) הטקסט "לפני שקונים"
     (`text-3xl font-extrabold`, סף 3:1 לטקסט גדול) מוצג בגוון הכהה
     המיועד לרקע בהיר, על רקע נייבי כהה — יחס נמדד **2.46-2.88:1**
     (חושב מ-HSL אמיתי דרך נוסחת relative-luminance, לא ניחוש) —
     נכשל אפילו מול סף 3:1. אומת גם ש-4 המופעים האחרים של
     `.gold-text` (Logo.tsx, Report.tsx, ScoreGauge.tsx, Premium.tsx)
     יושבים כולם על רקעי `--card`/`--background` בהירים במצב אור —
     שם `hsl(43 62% 32%)` עובר בנוח (**5.10:1**) — כך שהתיקון חייב
     להיות ממוקד ל-hero בלבד, לא שינוי גורף של `.gold-text`.
854. **התיקון:** נוספה מחלקה חדשה וממוקדת `.gold-text-on-dark`
     ב-`index.css` (אחרי `.dark .gold-text`) עם אותו ערך בהיר הקיים
     כבר ב-`.dark .gold-text` (`hsl(43 58% 62%)`) — לא צבע חדש
     שהומצא, אלא הצבע שכבר מוכח עובר בנוח על רקעים כהים (אומת שוב:
     **6.70-7.85:1** על שני קצוות גרדיאנט ה-hero). ב-`Home.tsx`
     שורה 130 הוחלף `className="gold-text"` ל-`"gold-text-on-dark"`
     — כעת הטקסט תמיד בגוון הבהיר, עקבי עם רקע ה-hero הכהה-קבוע,
     בשני מצבי התצוגה. `.gold-text` המשותפת נשארה בדיוק כפי שהייתה,
     כך שארבעת המופעים האחרים (העוברים כבר) לא הושפעו.
855. **אפס רגרסיה מאומתת:** `git diff --stat` — שני קבצים, 7+/1-
     (הוספת מחלקת CSS חדשה + החלפת שם-מחלקה אחת ב-JSX), אין שינוי
     במחלקה המשותפת `.gold-text` הקיימת ואין שינוי בלוגיקה/state/
     מבנה. איזון סוגריים ב-`index.css` המלא: תואם (24/24 מסולסל,
     161/161 עגול). שני הנתיבים כבר עוקבים ב-git (לא נדרש
     `git add -f` הפעם). Commit ממתין על
     `fix/a-icon-only-buttons-round2-0820`, יידחף ל-origin (מפעיל
     פריסת Vercel תחת more30.com/smel-ndln).
856. **הבא בתור:** 3 המועמדים הנוספים מהסריקה הזו (01-torah-platform
     `SynagoguePortal.tsx:173` — טקסט כפתור "שתף" בגוון
     `hsl(40 80% 35%)` על `#fdf8f0`, ~3.04:1; 04-imud-torani
     `Home.tsx:40,119` — `#da6c80` על רקעי dark-mode מעורבבי-שקיפות,
     ~2.90/3.70:1) דורשים אימות ידני נוסף לפני תיקון (הגרדיאנטים/
     שקיפויות שם מורכבים יותר מהמקרה שנפתר כאן, וסוכן הסריקה כבר
     טעה פעם אחת בסבב הזה). לחלופין: לפתוח עדשת focus-visible states
     או heading-order סמנטי, או לחזור ל-`core.project_tasks` (5
     פריטים פתוחים, עדיין חסומים).

## 20/08/2026 — סבב 176 (loop A)

857. **אימות ידני עצמאי של 3 המועמדים שנותרו מסבב 175 — לא תוקנו,
     נשארים בתור.** חישבתי בעצמי (נוסחת relative-luminance מלאה מ-HSL
     אמיתי, לא ניחוש) ניגודיות לשני המועמדים המורכבים: (א)
     `01-torah-platform/src/pages/legacy/SynagoguePortal.tsx:173` —
     כפתור `variant="outline"` בתוך `<header>` עם
     `bg-[#fdf8f0]/85 backdrop-blur-md` — הרכב הרקע בפועל תלוי במה
     שגולל מתחת ל-header הדביק (sticky), לא רק בלבן; חישבתי מקרה
     סביר (הרכבה על רקע לבן קבוע) ויצא **~4.11:1** — שונה מהותית
     מהערכת סוכן-הסריקה (~3.04:1) וגובל בדיוק סביב סף 4.5:1 לטקסט
     רגיל; (ב) `04-imud-torani/Home.tsx:40` — `dark:text-[#da6c80]`
     על `bg-primary/5` בתוך `bg-gradient-to-b from-accent/40` מעל
     `bg-background` הכהה — חישבתי הרכבה בנקודה הכי "בעייתית" של
     הגרדיאנט (הכי קרוב ל-`from-accent/40`) ויצא **~4.59:1**, עובר
     בקושי — גם כאן שונה מהותית מהערכת הסוכן (~2.90:1). כיוון ששני
     החישובים העצמאיים שלי לא מאשרים כשל ברור (אחד גובל בדיוק על
     הסף, השני עובר), ותלויים ברמת שקיפות/גרדיאנט/backdrop-blur
     שאי-אפשר לאמת במדויק בלי רינדור בפועל בדפדפן — לא נגעתי בהם.
     מסומן כ"נבדק, לא חד-משמעי, לא תוקן" ולא כ"תוקן" — מניעת תיקון
     שגוי על בסיס חישוב לא ודאי חשוב יותר מסגירת הפריט.
858. **שתי עדשות חדשות נפתחו וסגרו לפי התור מסבב 175.** (1)
     focus-visible states (WCAG 2.4.7) — סוכן Explore סרק את כל
     01-16 (דילוג 08/09) אחר `outline-none`/`focus:outline-none` בלי
     תחליף `focus-visible:ring`/`focus:ring` נלווה, וגם כללי CSS
     גולמיים `:focus{outline:none}` בלי `:focus-visible` נלווה —
     כל המקרים שנמצאו (כ-137 קומפוננטות shadcn/radix, div-ים
     `role="button"` מותאמים אישית, ו-CSS גולמי ב-05/06/10/11)
     כבר מטפלים נכון (או משאירים את מתאר ברירת המחדל של הדפדפן, או
     מחליפים ב-`focus-visible` תואם) — נסגרת ריקה, ללא ממצא. (2)
     heading-order סמנטי (WCAG 1.3.1) — סוכן Explore סרק דפים
     ציבוריים/מחוברים בכל 01-16 (דילוג 08/09, 07/13 בלי frontend)
     אחר קפיצת רמת כותרת (h1→h3 בלי h2, וכו'). נמצא ממצא שיטתי גדול
     ב-01-torah-platform (`CardTitle` ב-`components/ui/card.tsx`
     מרונדר כ-`<h3>` קבוע כברירת מחדל, כבר עם מנגנון override
     מתועד `as="h2"` שהופעל בעבר רק ב-`Home.tsx` — סה"כ כ-32 עמודים
     עם קפיצה או עם חוסר `h1` לגמרי), ועוד כמה ממצאים קטנים יותר
     ב-03/05/10/11/15 (לא טופלו הסבב הזה — התור הגדול היה 01).
859. **התיקון:** לכל 32 העמודים המושפעים ב-01-torah-platform, הוספת
     prop קיים כבר (`as="h2"`/`as="h1"`) ל-`<CardTitle>` הרלוונטי —
     ללא שינוי ל-`card.tsx` עצמו וללא שינוי ללוגיקה/מבנה/טקסט. שני
     דפוסים: (א) עמודים עם `<h1>` אמיתי בעץ הרינדור — כל `CardTitle`
     שהוא ראשון-אחרי-ה-h1 בלי `h2` ביניים קיבל `as="h2"` (כולל מקרים
     של כמה כרטיסים אחים במקביל, למשל `Contact.tsx` עם שני
     `CardHeader` צמודים, ו-`Settings.tsx` עם 3 פאנלים); `CardTitle`
     בתוך `.map()` שכבר מקונן תחת `h2` ספרותי משלו (למשל
     `FindLesson.tsx`, `RabbiQuestions.tsx`) נשאר `h3` ללא שינוי —
     זה קינון תקין; (ב) עמודים בלי `<h1>` כלל (למשל `SignIn.tsx`,
     `LessonDetail.tsx` עם `{lesson.title}` כ-`CardTitle` יחיד) —
     ה-`CardTitle` היחיד/הראשי קיבל `as="h1"` במקום זאת, ו-כל
     `CardTitle` שני שהיה תלוי בו (למשל `SynagogueDetail.tsx`,
     "זמני תפילה" אחרי "{data.name}") קיבל `as="h2"`. אומתו ידנית
     שני מקרי-קצה עם סדר-קבצים מטעה (h2 מופיע בקובץ *לפני* ה-h1):
     `DonationPage.tsx` ו-`Checkout.tsx` — שני ה-h2 יושבים ב-branch
     נפרד לגמרי (`if (iframeUrl) return ...`, מסך "השלמת תשלום"
     שמוחלף לגמרי את מסך הטופס), לא ברינדור בו-זמנית עם ה-h1 —
     תקין, לא קונפליקט אמיתי.
860. **אפס רגרסיה מאומתת:** `git diff --stat` — 33 קבצים (32 עמודים
     + לא נגעתי ב-`card.tsx`), 39+/39-, כל שינוי הוא הוספת prop
     `as="..."` בודד ל-`<CardTitle>` קיים, ללא שינוי JSX/state/
     className/טקסט אחר. בדיקת איזון סוגריים (Python, `{`/`(`) על
     כל 32 הקבצים אחרי העריכה — כולם תואמים. הנתיב
     `apps/01-torah-platform/src/pages` חסום ע"י `.gitignore` —
     נדרש `git add -f` (תקדים קבוע). Commit `6c8597f8` על
     `fix/a-icon-only-buttons-round2-0820`, יידחף ל-origin (מפעיל
     פריסת Vercel תחת more30.com/torah-platform).
861. **הבא בתור:** עדשת heading-order נשארת פתוחה על שאר האפליקציות
     שהסוכן דגל בהן בסבב הזה — ממצאים קטנים יותר ב-03-igud-ads (2
     עמודים), 05-financial-marketing-site (h2→h4 בפוטר),
     10-bkalot-rights (h2→h4), 11-bkalut-marketing2 (h2→h4),
     15-egod (7 עמודים — `CardTitle` שם *אין* לו מנגנון `as=` בכלל,
     צריך קודם להוסיף את ה-prop לקומפוננטה כמו ב-01, ואז ליישם, אותו
     דפוס דו-שלבי). לחלופין: 3 מועמדי הניגודיות מסבב 175 (עדיין לא
     אומתו כשל חד-משמעי, לדרוש כלי רינדור אמיתי כדי להכריע), עדשה
     חדשה (alt כפול/redundant, lang= תקין על מסמכים), או
     `core.project_tasks` (5 פריטים פתוחים, עדיין חסומים).
     פריטים פתוחים, עדיין חסומים).

## 20/08/2026 — סבב 177 (loop A)

862. **המשך עדשת heading-order מהתור של סבב 176: 03-igud-ads (2
     עמודים).** בניגוד ל-01-torah-platform, האפליקציה הזו היא
     Next.js App Router עם `<h1>`-`<h6>` גולמיים בכל עמוד — אין כאן
     רכיב `CardTitle`/`card.tsx` משותף ואין מנגנון `as=` (אומת:
     חיפוש `CardTitle`/`components/ui/card` בכל האפליקציה — אפס
     תוצאות). לכן התיקון היה פשוט: החלפת תג ישירה, לא הוספת prop.
863. **שלושה קפיצות h1→h3 אומתו ותוקנו:** (א)
     `app/(admin)/admin/templates/[id]/page.tsx:208` — כותרת "כללי
     עיצוב (style_rules)" ישבה ישירות אחרי ה-`h1` היחיד בעמוד (שם
     הטמפלט, שורה 129) בלי `h2` ביניים; (ב) אותו קובץ שורה 316 —
     "אזור מסוכן" (Danger zone), אותה בעיה, אח עצמאי לא-מקונן תחת
     הכותרת הקודמת; (ג) `app/notifications/page.tsx:76` — כותרת כל
     הודעה (`{n.title}`) בתוך `.map()` על רשימת ההתראות, בלי `h2`
     עוטף את הרשימה (לא כמו `FindLesson.tsx` ב-01 שבו ה-map כן מקונן
     תחת h2 עצמו) — לכן זו לא חריגה תקינה, כל שלושתן הוחלפו מ-`h3`
     ל-`h2`. אומתו ידנית כל מופעי `<h[1-6]` בשני הקבצים לפני העריכה
     (grep) כדי לוודא שאין כותרת נוספת בעץ שהוחמצה.
864. **אפס רגרסיה מאומתת:** `git diff` — 2 קבצים, 3 שורות שונו, כל
     שינוי הוא שם-תג בלבד (`h3`→`h2`, אותו `className`, אותו טקסט/
     ילדים), אין שינוי JSX/state/לוגיקה, ואין סלקטור CSS ישיר על
     `h3` באלמנטים האלה (עיצוב כולו דרך מחלקות Tailwind על התג עצמו,
     לא סלקטור-אלמנט חיצוני שהיה נשבר). הנתיב `apps/03-igud-ads/app`
     חסום ע"י `.gitignore` — נדרש `git add -f` (תקדים קבוע). Commit
     `1b45637d` על `fix/a-icon-only-buttons-round2-0820`, נדחף
     ל-origin (מפעיל פריסת Vercel תחת more30.com/modaot).
865. **הבא בתור:** יתרת עדשת heading-order מהתור: 05-financial-
     marketing-site (h2→h4 בפוטר), 10-bkalot-rights (h2→h4),
     11-bkalut-marketing2 (h2→h4), 15-egod (7 עמודים — צריך קודם
     להוסיף מנגנון `as=` לרכיב הכותרת המשותף, כמו ב-01, לפני
     שאפשר ליישם את התיקון עצמו — דפוס דו-שלבי). לחלופין: 3 מועמדי
     הניגודיות מסבב 175 (עדיין לא אומתו כשל חד-משמעי, דורשים כלי
     רינדור אמיתי כדי להכריע), עדשה חדשה (alt כפול/redundant,
     lang= תקין על מסמכים), או `core.project_tasks` (5 פריטים
     פתוחים, עדיין חסומים).

## 20/08/2026 — סבב 178 (loop A)

866. **המשך עדשת heading-order מהתור של סבב 177: 05-financial-
     marketing-site (פוטר).** האפליקציה היא עמוד סטטי בודד
     (`index.html` + `styles.css`, ללא רכיב React משותף). מבנה
     הכותרות בעמוד עצמו תקין (h1 בעמוד 50, h2 ב-115/156/195/232/278,
     h3 מקוננים תחת כל h2). הבעיה: הפוטר (`<footer class="site-
     footer">`, שורה 322) מגיע מיד אחרי ה-h2 האחרון בעמוד (שורה 278,
     "מעוניינים בפרטים?") וכולל 3 כותרות ניווט (`<h4>השירות</h4>`
     שורה 337, `<h4>בקלות</h4>` שורה 343, `<h4>קשר</h4>` שורה 349)
     בלי h3 ביניים — קפיצת רמה קלאסית.
867. **התיקון:** שלוש הכותרות הוחלפו מ-`h4` ל-`h3` (h2→h3 עכשיו,
     תקין). אומת מראש שהעיצוב לא מבוסס-className בלבד: `styles.css`
     שורה 506 הכילה סלקטור-תג ישיר `.site-footer h4 { color: var(
     --ivory); margin: 0 0 0.75rem; font-size: 0.95rem; font-weight:
     700; }` — עודכן ל-`.site-footer h3` באותו commit כדי לא לאבד
     את העיצוב. תופעת לוואי מכוונת: כלל גלובלי בשורה 64
     (`h1, h2, h3 { font-family: var(--font-display); })`) כבר כלל
     h3 אך לא h4 — הפונט המוצג בכותרות הפוטר יתאחד עכשיו עם שאר
     כותרות העמוד (עקבי יותר, לא רגרסיה).
868. **אפס רגרסיה מאומתת:** `git diff` — 2 קבצים, 4 שורות שונו
     (3 שמות-תג ב-HTML, 1 סלקטור ב-CSS), אין שינוי טקסט/מבנה/
     class/JS. **הבא בתור:** יתרת עדשת heading-order: 10-bkalot-
     rights (h2→h4), 11-bkalut-marketing2 (h2→h4), 15-egod (7
     עמודים — צריך קודם להוסיף מנגנון `as=` לרכיב הכותרת המשותף,
     כמו ב-01, לפני שאפשר ליישם את התיקון עצמו — דפוס דו-שלבי).
     לחלופין: 3 מועמדי הניגודיות מסבב 175, עדשה חדשה (alt כפול/
     redundant, lang= תקין על מסמכים), או `core.project_tasks` (5
     פריטים פתוחים, עדיין חסומים).
## 20/08/2026 — סבב 179 (loop A)

869. **המשך עדשת heading-order מהתור של סבב 178: 10-bkalot-rights
     (h2→h4).** האפליקציה היא עמוד סטטי בודד (`index.html` + `app.js`
     + `style.css`, ללא רכיב React משותף). בדקתי תחילה את ה-HTML
     הסטטי — מבנה תקין (h1 שורה 93, h2-ים ב-117/129/144/317/338/356,
     h3-ים מקוננים כראוי תחת כל h2, כולל בתוך `downloads`). הבעיה
     נמצאה ב-JS: `<section id="funds">` (שורה 336) מכיל `<h2>השוואת
     קופות חולים</h2>` (שורה 338) ומיד אחריו `<div id="fundCompare">`
     ריק שמתמלא דינמית ע"י `renderFundCompare()` ב-`app.js:422` עם
     `<div class="fc-col"><h4>${fundNames[fk]}</h4>` (שם קופת חולים,
     כללית/מכבי/מאוחדת/לאומית) — בלי h3 ביניים, קפיצת רמה קלאסית.
     אימתתי גם את שאר מופעי `h4` באפליקציה (`app.js:328/342` בתוך
     modal, ו-`app.js:131` בכרטיסי תוצאות) — כולם כן מקוננים כראוי
     תחת h3 (כותרת ה-modal או כותרת ה-block) בסדר-מסמך, רק המקרה
     הזה חורג.
870. **התיקון:** ה-`<h4>` הוחלף ל-`<h3>` ב-`app.js:422` (h2→h3
     עכשיו, תקין). אומת מראש שהעיצוב לא מבוסס-className בלבד:
     `style.css:194` הכיל סלקטור-תג ישיר `.fc-col h4{...}` — עודכן
     ל-`.fc-col h3{...}` באותו commit כדי לא לאבד את העיצוב (צבע,
     קו תחתון, גודל פונט). כלל גלובלי `style.css:55`
     (`h1,h2,h3,h4{font-family:...}`) כבר כלל גם h3 וגם h4 — אין
     שינוי בפונט המוצג.
871. **אפס רגרסיה מאומתת:** `git diff` — 2 קבצים, 2 שורות שונו
     (1 שם-תג ב-JS, 1 סלקטור ב-CSS), אין שינוי טקסט/מבנה/class/
     לוגיקה אחרת. Commit `e6220a63` על
     `fix/a-icon-only-buttons-round2-0820`, נדחף ל-origin (מפעיל
     פריסת Vercel תחת more30.com/10). **הבא בתור:** יתרת עדשת
     heading-order: 11-bkalut-marketing2 (h2→h4), 15-egod (7 עמודים
     — צריך קודם להוסיף מנגנון `as=` לרכיב הכותרת המשותף, כמו ב-01,
     לפני שאפשר ליישם את התיקון עצמו — דפוס דו-שלבי). לחלופין: 3
     מועמדי הניגודיות מסבב 175 (עדיין לא אומתו כשל חד-משמעי, דורשים
     כלי רינדור אמיתי כדי להכריע), עדשה חדשה (alt כפול/redundant,
     lang= תקין על מסמכים), או `core.project_tasks` (5 פריטים
     פתוחים, עדיין חסומים).

## 20/08/2026 — סבב 180 (loop A)

872. **המשך עדשת heading-order מהתור של סבב 179: 11-bkalut-
     marketing2 (h2→h4 בפוטר).** אותה תבנית בדיוק כמו סבב 178
     (05-financial-marketing-site) — עמוד סטטי בודד (`index.html`+
     `styles.css`+`script.js`, ללא רכיב משותף). מבנה הכותרות בעמוד
     עצמו תקין (h1 שורה 51, h2-ים ב-119/146/171/190/221/248,
     h3-ים מקוננים כראוי תחתיהם, כולל בתוך כרטיסי הקטגוריות
     וה-`.map`-מקבילים ב-`script.js`). הבעיה: הפוטר (`<footer
     class="site-footer">`, שורה 313) מגיע מיד אחרי ה-h2 האחרון
     בעמוד (שורה 248, "מוכנים להתחיל?") וכולל 3 כותרות ניווט
     (`<h4>מערכת</h4>` שורה 328, `<h4>משפטי</h4>` שורה 334,
     `<h4>קשר</h4>` שורה 340) בלי h3 ביניים — קפיצת רמה קלאסית.
     בדקתי גם את שלושת ה-`<dialog>` (terms/privacy/consent, שורות
     353-395) שמכילים `<h2>` בכותרת — אלה דיאלוגים נייטיביים
     (`<dialog>`) שנפתחים/נסגרים דרך JS מחוץ לזרימת המסמך הרגילה,
     אותו תקדים כמו מודאלים בסבבים קודמים — לא נגעתי בהם.
873. **התיקון:** שלוש הכותרות הוחלפו מ-`h4` ל-`h3` (h2→h3 עכשיו,
     תקין). אומת מראש שהעיצוב לא מבוסס-className בלבד:
     `styles.css:491` הכיל סלקטור-תג ישיר `.site-footer h4 {
     color: var(--paper); margin: 0 0 0.75rem; font-size: 0.95rem;
     font-weight: 700; }` — עודכן ל-`.site-footer h3` באותו commit
     כדי לא לאבד את העיצוב. בניגוד לסבב 178, כאן אין כלל גלובלי
     משותף על `h1,h2,h3` (`grep` על הקובץ אימת: אין סלקטור-תג בודד
     `^h[1-4]` בכל ה-CSS) — כך שאין תופעת-לוואי על גופן. אומתו גם
     שלושת סלקטורי ה-`h3` הקיימים האחרים בקובץ (`.steps h3`,
     `.cats-grid h3`, `.channels-grid h3`) מוגבלים למחלקת-הורה
     ספציפית שאינה `.site-footer` — אין התנגשות קסקדה.
874. **אפס רגרסיה מאומתת:** `git diff` — 2 קבצים, 6 שורות שונו
     (3 שמות-תג ב-HTML, 1 סלקטור ב-CSS), אין שינוי טקסט/מבנה/
     class/JS. הנתיב לא חסום ע"י `.gitignore` הפעם (בניגוד ל-05/10)
     — `git add` רגיל הספיק. Commit `d63811db` על
     `fix/a-icon-only-buttons-round2-0820`, יידחף ל-origin (מפעיל
     פריסת Vercel תחת more30.com/11). **הבא בתור:** יתרת עדשת
     heading-order: 15-egod (7 עמודים — `CardTitle` שם *אין* לו
     מנגנון `as=` בכלל, צריך קודם להוסיף את ה-prop לקומפוננטה כמו
     ב-01, ואז ליישם, אותו דפוס דו-שלבי). זו הבעיה האחרונה בתור
     המקורי מסבב 176 — לאחריה: 3 מועמדי הניגודיות מסבב 175 (עדיין
     לא אומתו כשל חד-משמעי, דורשים כלי רינדור אמיתי כדי להכריע),
     עדשה חדשה (alt כפול/redundant, lang= תקין על מסמכים), או
     `core.project_tasks` (5 פריטים פתוחים, עדיין חסומים).

## 20/08/2026 — סבב 181 (loop A)

875. **סגירת עדשת heading-order: 15-egod (הפריט האחרון בתור מסבב
     176).** לפני היישום בדקתי בפועל את ההנחה מסבבים 179-180 שה-
     `CardTitle` (`src/components/ui/card.tsx:17-22`, עותק מקומי של
     shadcn/ui, מרנדר `<h3>` קבוע ללא prop `as=`) הוא מקור בעיית
     ה-heading-order באפליקציה. `grep -rn "CardTitle"` על כל
     `src/` הראה שהוא **קוד מת** — מיוצא מהקובץ אבל אף עמוד/
     קומפוננטה לא מייבא או מרנדר אותו בפועל. כלומר מנגנון ה-`as=`
     הדו-שלבי (כמו בסבב 176 ב-01) לא רלוונטי כאן כלל — הקפיצות
     האמיתיות מגיעות מתגי `<h3>` גולמיים.
876. **מיפוי מלא של 8 העמודים הציבוריים** (7 העוטפים ב-`Layout`
     + `Invite.tsx` שמשתמש ב-`Footer` הישיר בלי `Layout`) גילה
     שלוש קפיצות אמיתיות מ-h1 ישר ל-h3 (בלי h2 באמצע): (1)
     `Footer.tsx:22,31,39` — שלוש כותרות ניווט משותפות שמופיעות
     בתחתית **כל** העמודים (כולל Invite) אחרי ה-h1/h2 האחרון של
     כל עמוד; (2) `About.tsx:31` — כותרת בלולאת "הערכים שלנו"
     (6 כרטיסים) שמגיעה ישר אחרי ה-h1, לפני ה-h2 בשורה 37; (3)
     `FindLesson.tsx:84` — כותרת נושא-שיעור בלולאת תוצאות החיפוש,
     גם היא ישר אחרי ה-h1 (שורה 40), בלי h2 באמצע. עמוד הבית
     (Index) ו-Questionnaire כבר היו תקינים (h1→h2→h3 מסודר,
     או h2 יחיד ללא h1 — לא בגדר קפיצה).
877. **התיקון:** שלוש הכותרות הוחלפו מ-h3 ל-h2 (5 שורות קוד
     בסה"כ). אומתתי מראש שאין תלות בתג ב-CSS: `src/index.css:129`
     מכיל רק כלל גורף `h1,h2,h3,h4,h5,h6 { font-family: ... }`
     (משותף לכל הרמות, ללא שינוי חזותי), וכל העיצוב בפועל מגיע
     ממחלקות Tailwind ישירות על כל אלמנט (למשל `text-xl
     font-bold`), שנשארו זהות — רק שם התג השתנה. אין קובץ CSS
     גולמי עם סלקטור h3 ספציפי כמו באתרים הסטטיים (05/10/11).
     בדיקת איזון תגיות ב-Python (regex על `<h[1-6]` מול `</h[1-6]>`)
     אימתה 0 חוסר-איזון בשלושת הקבצים שנערכו.
878. **תוצאה מאומתת לכל העמודים:** Index: h1→h2→h3→h2→h3→h2→
     h2×3(footer, היה h3×3) — ללא קפיצה חדשה (ירידת רמה תמיד
     תקינה). About: h1→h2×6(ערכים, היה h3)→h2(שורה 37)→h2×3
     (footer) — תקין. Login/JoinTeacher/RequestLesson: h1→h2×3
     (footer בלבד) — תקין. FindLesson: h1→h2×N(תוצאות, היה h3)→
     h2×3(footer) — תקין. Invite (לא היה בתור המקורי, לא עטוף
     ב-Layout אבל כן משתמש ב-Footer המשותף): h1→h2×3(footer) —
     בונוס: תוקן אגב כתוצר לוואי נקי מאותו שינוי משותף, ללא מאמץ
     נוסף. Questionnaire ללא שינוי (h2 יחיד, אין h1 בעמוד — פער
     נפרד, לא קפיצת-רמה, לא טופל כאן לפי התקדים מסבבים קודמים
     שמטפלים רק בקפיצות בפועל).
879. **אפס רגרסיה מאומתת:** `git diff --stat` — 3 קבצים, 5+/5-,
     שינויי שם-תג בלבד (h3→h2), אין שינוי טקסט/class/JSX/לוגיקה
     אחרת. הנתיב `apps/15-egod/src` מסומן ב-`.gitignore` הראשי
     (שורה 29, `/apps/**` עם whitelist חלקי) אבל הקבצים כבר
     tracked מסבבים קודמים (git מתעלם מ-gitignore על קבצים
     tracked) — `git add` הרגיל סירב ("ignored paths"), נפתר עם
     `git add -u` (מעדכן רק קבצים שכבר tracked, לא בודק gitignore
     על נתיב חדש). Commit `23d8f992` על
     `fix/a-icon-only-buttons-round2-0820`, נדחף ל-origin (מפעיל
     פריסת Vercel תחת more30.com/egod). **עדשת heading-order סגורה
     לחלוטין** — כל התור המקורי מסבב 176 (01/03/05/10/11/15) טופל.
     **הבא בתור:** 3 מועמדי הניגודיות מסבב 175 (עדיין לא אומתו
     ככשל חד-משמעי, דורשים כלי רינדור אמיתי כדי להכריע), עדשה
     חדשה (alt כפול/redundant, lang= תקין על מסמכים, target=_blank
     ללא rel על 01-16 באופן שיטתי), או `core.project_tasks` (5
     פריטים פתוחים, עדיין חסומים על secrets/החלטות מיזוג מחוץ
     לסמכות הסוכן).

## 20/08/2026 — סבב 182 (loop A)

880. **שלוש עדשות חדשות נבדקו על יתרת התור מסבב 181 — כולן נסגרו
     ריקות, אימות מלא.** (1) `target="_blank"` בלי `rel="noopener"`/
     `"noreferrer"` על כל 01-16 (דילוג 08/09) — נסרקו כל מופעי
     `target="_blank"` (כ-50), כולל אלה שה-`rel` יושב בשורת JSX
     נפרדת (attributes מרובות שורות) — כולם כבר עם `rel` תקין. (2)
     `lang=` על `<html>` בכל האפליקציות עם `index.html`/`layout.tsx`
     — כולם `lang="he" dir="rtl"` תקין. (3) `tabIndex` חיובי (שובר
     סדר טאב טבעי) — נסרקו כל מופעי `tabIndex={...}`/`tabindex=` —
     רק `0`/`-1` נמצאו, אין ערך חיובי בשום מקום. נבדק גם meta
     viewport (`user-scalable=no`/`maximum-scale` שחוסם zoom) על כל
     11 קבצי `index.html` — כולם מאפשרים zoom, אין חסימה.
881. **עדשה רביעית — Explore agent — מצאה ממצא אמיתי:** `apps/
     01-torah-platform/src/pages/legacy/LessonDirectory.tsx` (עמוד
     מנותב חי, `/lessons-dir` ב-`App.tsx:216`, לא קוד מת) — ארבעה
     שדות סינון `<select>` (עיר/קהל יעד/סגנון/שפה, שורות 145-172)
     עם `<label>` טקסטואלי גלוי אבל בלי `htmlFor`/`id` תואם —
     screen reader לא יכול לקשר תכנותית בין התווית לשדה (WCAG 1.3.1
     Info and Relationships). אומתתי ידנית בקריאת הקובץ (לא רק
     דיווח הסוכן) לפני התיקון.
882. **התיקון:** הוספת `id="filter-city"`/`"filter-audience"`/
     `"filter-style"`/`"filter-language"` על ה-`<select>` המתאים,
     ו-`htmlFor` תואם על ה-`<label>` שמעליו — 4 זוגות, 8 שורות
     שונו. אומתתי מראש שאין התנגשות `id` קיימת בקובץ (`grep id="`
     על הקובץ המלא — היו רק 4 המזהים החדשים).
883. **אפס רגרסיה מאומתת:** `git diff --stat` — קובץ אחד, 8+/8-,
     כל שינוי הוא הוספת attribute (`htmlFor`/`id`) בלבד, אין שינוי
     ב-`value`/`onChange`/`className`/טקסט/state. איזון סוגריים
     מלא בפייתון על הקובץ: `{}` 118/118, `()` 120/120 — תואם. הנתיב
     `apps/01-torah-platform/src` חסום ע"י `.gitignore` אבל הקובץ
     כבר tracked — `git add` רגיל הספיק (בניגוד לחלק מהסבבים
     הקודמים שדרשו `-f`). Commit `01e5ac0e` על
     `fix/a-icon-only-buttons-round2-0820`, נדחף ל-origin (מפעיל
     פריסת Vercel תחת more30.com/torah).
884. **הבא בתור:** להמשיך את עדשת `label`/`select` בלי `htmlFor`/
     `id` על שאר 01-16 (רק 01 נבדק בסבב הזה, יש עוד `<select>`
     ב-`PortalSettings.tsx` שכבר משתמש ב-`aria-label` במקום —
     נדרש אימות אם זה מספיק או דורש גם `id`), שני מועמדי הניגודיות
     שנותרו לא-חד-משמעיים מסבב 175 (עדיין דורשים כלי רינדור אמיתי),
     או `core.project_tasks` (5 פריטים פתוחים, עדיין חסומים על
     secrets/החלטות מיזוג מחוץ לסמכות הסוכן).

## 20/08/2026 — סבב 183 (loop A)

885. **המשך עדשת `label`/`select` בלי `htmlFor`/`id` (מסבב 182) —
     נסגרת ריקה על 02-16.** נסרקו כל 01-16 (דילוג 08/09): ל-02/03/
     04/05/06/07/10/11/12/13/14 אין בכלל `<select>` בקוד. ל-15-egod
     (`PortalSettings.tsx`) כל 4 שדות ה-`<select>` כבר עם `id`/
     `htmlFor` תואמים (כולל שדה השפה שצוין כלא-ודאי בסבב 182). ל-16
     (`GabaiLayout.tsx` + 4 שימושים ב-`Field`/`Select` הגנרי ב-
     `SynagogueForm`/`AdminServices`/`AdminLessons`/`GabaiPrayers`)
     כל ה-`id` על ה-`<Select>` תואמים ל-`htmlFor` על ה-`<Field>`
     העוטף. עדשת ה-`select`/`label` **סגורה לגמרי** על 01-16.
886. **שתי עדשות מועמדות נוספות נבדקו ויצאו ריקות/חלשות מדי לתיקון
     בטוח:** (א) `<input>`/`<textarea>` עם `<label>` גלוי לא-מקושר
     (המשך אותה משפחת באגים ל-inputs טקסטואליים) — הממצא היחיד
     שנמצא (`01-torah-platform/UpdateLesson.tsx:227-236`) הוא
     כותרת-קבוצה ("מיקום") מעל 4 שדות, לא תווית פר-שדה — לא אותה
     משפחת באג, לא תוקן כדי לא להמציא סמנטיקה שלא הייתה שם. (ב) alt
     כפול/redundant (מועמד שחזר ותועד כ"לא מומש" בסבבים 157-182) —
     נסרקו כל 44 מופעי `alt=` שנותרו ב-01/15/16 (שאר האפליקציות ללא
     `<img>` עם alt לא-ריק) בפועל, אחד-אחד — כולם תיאוריים תקינים
     או `alt=""` דקורטיבי מכוון, אין כפילות מול טקסט גלוי סמוך.
     עדשה זו נסגרת **ריקה בפועל** (לא רק "נדחתה") — לא אמורה לחזור
     כמועמד עתידי.
887. **עדשה שלישית — כן הניבה ממצא אמיתי:** `autoComplete` חסר על
     שדות login (WCAG 1.3.5 Identify Input Purpose). נבדקו כל
     קבצי `PasswordInput`/`type="password"` ב-01-16 — כל טפסי
     הכניסה האמיתיים כבר עם `autoComplete` תקין (`SignIn`/`SignUp`/
     `ResetPassword`/`ActivateInvite` ב-01, `Login` ב-15, `AdminLogin`
     ב-16) **חוץ מ**-`apps/01-torah-platform/src/pages/legacy/
     AdminLogin.tsx` — נתיב חי (`/legacy/admin-login`, מנותב
     ב-`App.tsx:281`) עם שני שדות (שם משתמש + סיסמה) בלי
     `autoComplete` בכלל, בניגוד לכל שאר טפסי הכניסה באותה אפליקציה.
     (שדה `api-password` ב-`portal/Settings.tsx` נבדק ונפסל — זו
     סיסמת API של נדרים פלוס, לא אישור כניסה של המשתמש, לא אותה
     משפחת באג.)
888. **התיקון:** הוספת `autoComplete="username"` על שדה שם המשתמש,
     ו-`autoComplete="current-password"` על ה-`PasswordInput` — 2
     שורות, תואם בדיוק לתבנית הקיימת כבר ב-`SignIn.tsx:107` באותה
     אפליקציה. אפס רגרסיה מאומתת: `git diff` — קובץ אחד, 2+/0-,
     תוספת attribute טהורה, אין שינוי ב-JSX/state/handler. איזון
     סוגריים בפייתון על הקובץ המלא: `{}` 54/54, `()` 58/58 — תואם.
     `git add -u` נדרש (תקדים קבוע, נתיב חסום ב-`.gitignore` אבל
     כבר tracked). Commit `adfa693e` על
     `fix/a-icon-only-buttons-round2-0820`, נדחף ל-origin (מפעיל
     פריסת Vercel תחת more30.com/torah).
889. **הבא בתור:** לבדוק אם אותו דפוס `autoComplete` חסר חוזר על
     שדות login אחרים שטרם נסרקו (`api-password` בשאר
     האפליקציות, שדות OTP/מייל-בלבד), שני מועמדי הניגודיות שנותרו
     לא-חד-משמעיים מסבב 175 (עדיין דורשים כלי רינדור אמיתי), או
     `core.project_tasks`/`core.project_bugs` (5 פריטים פתוחים,
     עדיין חסומים על secrets/החלטות מיזוג מחוץ לסמכות הסוכן).

## 20/08/2026 — סבב 184 (loop A)

890. **המשך שאלת #889 — נבדק ונסגר ריק על 01-16.** האצלתי לסוכן
     Explore בדיקה שיטתית: האם דפוס ה-`autoComplete` החסר חוזר על
     שדות login אחרים שטרם נסרקו (שדות OTP, מייל-בלבד/magic-link,
     טפסי כניסה נוספים ב-02/03/04/05/06/07/10-16). תוצאה: **אין
     שדות OTP חיים בכלל** (רק primitive `input-otp.tsx` לא-בשימוש
     ב-12-smel-ndln), **אין זרימת magic-link** באף אפליקציה, וכל
     טפסי הכניסה האמיתיים שנבדקו (02/03/06/15/16) כבר עם
     `autoComplete` תקין. שלושה מועמדים גבוליים שנמצאו (`Teachers.tsx`/
     `Users.tsx` ב-01, `AdminTeachers.tsx` ב-15) הם שדות "סיסמה
     ראשונית" שאדמין קובע *עבור משתמש אחר* בהקמת חשבון — לא
     אישורי-הכניסה של המשתמש עצמו, אותה משפחה כמו `api-password`
     שכבר נפסל בסבב 183. **עדשת ה-autoComplete סגורה סופית.**
891. **שתי עדשות חדשות נבדקו וחזרו ריקות: `<iframe>` בלי `title`
     (WCAG 2.4.1/4.1.2) וטקסט-קישור גנרי ("לחץ כאן"/"קרא עוד",
     WCAG 2.4.4).** לעדשת ה-iframe: נסרקו כל 01-16 (דילוג 08/09) —
     רק 3 `<iframe>` חיים בכל ה-scope (תשלום נדרים פלוס ב-
     `DonationPage.tsx`/`Checkout.tsx` ב-01, תצוגה מקדימה ב-
     `Editor.tsx` ב-04), כולם כבר עם `title` תקין; אין `<embed>`/
     `<object>` בכלל. לעדשת טקסט-קישור: `grep` ישיר על "לחץ כאן"/
     "קרא עוד"/"click here"/"read more"/"learn more" בכל ה-scope —
     אפס תוצאות.
892. **עדשה שלישית — Use of Color (WCAG 1.4.1) — הניבה ממצא אמיתי
     אחד.** `apps/03-igud-ads/app/notifications/page.tsx:74`
     (`/notifications`, נתיב חי — מקושר מ-`NotificationBell.tsx`
     שמורכב ב-`SiteHeader.tsx` הגלובלי): אינדיקטור "לא-נקרא" היה
     נקודה כחולה בלי טקסט/`aria-label`, כשהאות הנוספת היחידה
     (גוון רקע/מסגרת כרטיס `bg-blue-50 border-blue-200`) גם היא
     צבע-בלבד — משתמשים עיוורי-צבע/כבדי-ראייה לא יכלו להבחין
     נקרא/לא-נקרא. שתי עדשות מקבילות (טפסים עם `border-red-*`
     בלי טקסט שגיאה נלווה, מקראות-גרפים בצבע-בלבד) נבדקו ויצאו
     ריקות — כל שגיאת טופס שנמצאה כבר עם טקסט שגיאה גלוי,
     קומפוננטת ה-chart היחידה שנמצאה היא קוד מת לא-מיובא.
893. **התיקון:** הוחלפה הנקודה הבודדת בתג `<span>` עם טקסט גלוי
     "חדש" (`text-xs font-semibold text-blue-700`) לצד הנקודה
     (שנשארה כעיטור, קיבלה `aria-hidden="true"` כי הטקסט הסמוך
     כבר מתאר אותה). שינוי תוסף בלבד — לא נגע בלוגיקת
     `markRead`/`is_read`/`className` של הכרטיס העוטף. אפס
     רגרסיה מאומתת: `git diff` קובץ אחד, 6+/1-. איזון סוגריים
     בפייתון על הקובץ המלא: `{}` 32/32, `()` 37/37 — תואם. הנתיב
     `apps/03-igud-ads/app` חסום ב-`.gitignore` אך כבר tracked —
     `git add -u` (תקדים קבוע). Commit `6139e690` על
     `fix/a-icon-only-buttons-round2-0820`, יידחף ל-origin (מפעיל
     פריסת Vercel תחת more30.com/modaot).
894. **הבא בתור:** שני מועמדי הניגודיות שנותרו לא-חד-משמעיים
     מסבב 175 (עדיין דורשים כלי רינדור אמיתי — לא ניתנים לפתרון
     בלי דפדפן חי), עדשת נגישות חדשה נוספת על 01-16 (למשל: זמן-קצוב
     (timeout) לפעולות בלי אזהרה, `aria-sort` על עמודות טבלה
     ניתנות-למיון, `<a>` ריק/decorative בלי טקסט), או
     `core.project_tasks`/`core.project_bugs` (עדיין אותם 5 פריטים
     חסומים על secrets חסרים או החלטות merge/origin מחוץ לסמכות
     הסוכן — נבדק מחדש הסבב הזה, שום דבר חדש).

## 20/08/2026 — סבב 185 (loop A)

895. **ארבע עדשות נגישות חדשות נבדקו ברצף וחזרו ריקות** לפני
     שנמצא ממצא אמיתי: (1) `aria-sort` על עמודות טבלה ניתנות-למיון
     — נסרק כל 01-16 (דילוג 08/09), אין אפילו טבלה אחת עם מיון
     אינטראקטיבי בכל ה-scope (כל הכותרות סטטיות); (2) `<a>`
     ריק/icon-only בלי שם נגיש — נמצא מועמד גבולי יחיד
     (`16-chatzor-connect/AdminSynagogues.tsx:113`) אך יש לו כבר
     `title="צפייה באתר"`, שזה מנגנון שם-נגיש תקין לפי הספק (לא
     `aria-label` המועדף, אך לא הפרה) — לא תוקן; (3) אלמנטים
     פוקוסביליים בתוך מכולה עם `aria-hidden="true"` (WCAG 4.1.2) —
     52 מופעי `aria-hidden="true"` בכל ה-scope, כולם על עלים
     דקורטיביים (svg/span/div ריקים), אף לא אחד עוטף
     button/a/input/select/textarea; (4) `id` סטטי כפול בתוך
     `.map()`/רכיב-שחוזר-בעמוד — כל שימושי `.map()` כבר עם `key`
     דינמי, כל ה-`id`-ים הקשיחים שנמצאו הם ברכיבים חד-פעמיים
     בעמוד (לא כפולים בפועל).
896. **עדשה חמישית — הניבה ממצא אמיתי ומהותי.** הרחבתי את בדיקת
     `autoComplete` (WCAG 1.3.5) שנסגרה בסבב 184 על שדות
     login/סיסמה **לשדות מידע-אישי כלליים** (שם/טלפון/מייל/כתובת)
     בטפסי יצירת-קשר/הרשמה/שאלונים — תחום שלא נבדק קודם. סוכן
     Explore סרק את כל 01-16 ומצא: `01-torah-platform` — 7 שדות
     ב-4 קבצים (`ContactSection.tsx`, שני טפסי יצירת-קשר ב-
     `Footer.tsx`, `TeacherForm.tsx`, `SeekerForm.tsx`) בלי
     `autoComplete`; ממצאים דומים סומנו גם ב-`15-egod`,
     `16-chatzor-connect`, `12-smel-ndln` (לא טופלו הסבב הזה, בתור
     להמשך). בדקתי ידנית כל שדה שהסוכן דגל לפני תיקון — **סיננתי
     החוצה** שדות המתארים **צד שלישי** ולא את ממלא הטופס עצמו
     (שם נפטר/עיסוקו/מיקום בית אבלים ב-`SeekerForm.tsx`, שם בית
     כנסת/שם גבאי): SC 1.3.5 חל על מידע *של המשתמש עצמו*, לא על
     שדות שמתארים אדם אחר — הצמדת `autoComplete="name"` לשדה "שם
     הנפטר" הייתה גורמת לדפדפן להציע את השם של ממלא הטופס עצמו,
     טעות ולא שיפור. גם שכונה/רחוב לא תוקנו (אין טוקן סטנדרטי
     חד-משמעי, ביטחון נמוך).
897. **התיקון (ב-01-torah-platform בלבד הסבב הזה):**
     `ContactSection.tsx` (name/tel/email על 3 שדות בטופס יצירת-
     הקשר הראשי), `Footer.tsx` (name על 2 טפסי יצירת-קשר —
     tel כבר היה תקין בשניהם), `TeacherForm.tsx` (name/tel על
     `fullName`/`phone`, address-level2 על `city` — email כבר היה
     תקין), `SeekerForm.tsx` (name/tel/email על `contactName`/
     `phone`/`email` בשלב "פרטי הפונה", address-level2 על `city`
     — כל השדות בשלב הזה מתארים את איש-הקשר הממלא את הטופס
     עצמו, אומת ידנית מהקשר הכותרת "פרטי הפונה"/"איש קשר"). תוך
     כדי גם נוסף `type="tel"`/`inputMode="tel"` ו-
     `type="email"`/`inputMode="email"` היכן שחסרו (תואם לדפוס
     הקיים כבר באותם קבצים בשדות מייל/טלפון אחרים).
898. **אפס רגרסיה מאומתת:** `git diff --stat` — 4 קבצים,
     12+/10-, כל שינוי הוא הוספת attribute בודד (`autoComplete`/
     `type`/`inputMode`) לתוך `<input>`/`<Input>` קיים, אין שינוי
     ב-JSX structure/state/handler/className. איזון סוגריים
     בפייתון על כל 4 הקבצים אחרי העריכה: כולם תואמים. אימות נוסף:
     `components/ui/input.tsx` מעביר `type`/`...props` ישירות
     לתגית `<input>` המקורית — אין קונפליקט. הנתיב
     `apps/01-torah-platform/src` חסום ב-`.gitignore` אך כבר
     tracked — `git add -u` (תקדים קבוע). Commit `b1c9e431` על
     `fix/a-icon-only-buttons-round2-0820`, יידחף ל-origin (מפעיל
     פריסת Vercel תחת more30.com/torah).
899. **הבא בתור:** להמשיך את עדשת ה-`autoComplete` על מידע-אישי
     שנפתחה הסבב הזה על שאר האפליקציות שדוגלו (`15-egod` —
     `TeacherForm`/`SeekerForm` באותו מבנה בדיוק כמו 01,
     `16-chatzor-connect` — `InquiryForm`/`AskRav`, `12-smel-ndln`
     — טופס Premium/checkout), שני מועמדי הניגודיות שנותרו לא-
...
## 20/08/2026 — סבב 188 (loop A)

911. **עדשה חדשה: `<label>` בלי קישור תקין לשדה (WCAG 1.3.1 /
     4.1.2)** — בדיוק הפריט שסומן כ"בא בתור" בסבב 187. סוכן
     Explore סרק את 01-16 (לבד מ-08/09 המוגנים) וחזר עם שני סוגי
     ממצא: (א) קומפוננטות "קבוצת-צ'יפים" משותפות (`ChipPicker`/
     `ChipSelect`/`MultiSelect`/`RadioSelect`) שבהן `<label>` הוא
     כותרת ויזואלית לקבוצת כפתורי-בחירה, בלי `htmlFor` (וגם בלי
     `role="group"`/`aria-labelledby` על מיכל הכפתורים) — כותרת
     שקורא-מסך רואה כ"תווית יתומה" ולא מקושרת לקבוצה; (ב) שדות
     `<Input>`/`<Textarea>` אמיתיים עם `<label>` בלי `htmlFor`
     בכלל — כאן התווית לא מקושרת לשדה *בשום צורה*, כך שקורא-מסך
     שמנווט לפי שדות-טופס לא שומע שם לשדה בכלל.
912. **תיקון סוג (א) — 4 קומפוננטות משותפות, `useId()` +
     `role="group"` + `aria-labelledby`:** `PortalLessonForm.tsx`
     (01, קומפוננטת `ChipPicker` פנימית — מתקן בבת אחת את 7
     השימושים בקובץ: נושא/סגנון שיעור/סגנון דיבור/סגנון/קהל/שפה/
     קהל יעד), `forms/ChipSelect.tsx` (01, קומפוננטה משותפת),
     `questionnaire/MultiSelect.tsx` + `questionnaire/
     RadioSelect.tsx` (**15-egod בלבד** — הגרסאות המקבילות ב-01
     כבר היו עם התיקון הזה מסבב קודם; אומת בקריאה ישירה לפני
     שנגעתי). כל קומפוננטה קיבלה `labelId = useId()`, ה-`<label>`
     קיבל `id={labelId}`, ומיכל הכפתורים קיבל `role="group"
     aria-labelledby={labelId}` — קישור פורמלי מהקבוצה לכותרת שלה
     בלי לשנות שום מבנה/state/handler קיים.
913. **תיקון סוג (ב) — שדות טופס אמיתיים, קונבנציית `id`/
     `htmlFor` תואמת:** `questionnaire/SeekerForm.tsx` (01, שדה
     `seekerType` שלב "סוג הפנייה" — אותו תיקון role=group כמו
     סוג א') וגם `01-torah-platform`'s `TeacherForm.tsx`/
     `SeekerForm.tsx` — שם גיליתי ש**15-egod כבר תוקן** (יש שם
     `id="teacherform-fullname"`/`htmlFor="teacherform-fullname"`
     וכו' על כל שדה, כנראה מתיקון קודם), אבל **01-torah-platform
     לא** — פער בין שני אפליקציות תאומות (אותו מבנה קוד בדיוק).
     החלתי את **אותה קונבנציה בדיוק** (`teacherform-*`/
     `seekerform-*`) על 01 כדי להשוות בין שני העותקים: 6 שדות
     ב-`TeacherForm.tsx` (שם מלא/טלפון/מייל/עיר/שכונה/הערות) ו-14
     שדות ב-`SeekerForm.tsx` (איש קשר/טלפון/מייל/עיר/שכונה/רחוב/
     שם הנפטר/עיסוק הנפטר/מיקום בית האבלים/זמני התפילות/שם בית
     הכנסת/שם הגבאי/כמות מתפללים/הערות). בניגוד לעדשת ה-autoComplete
     (1.3.5) שסיננה החוצה שדות-צד-שלישי, **1.3.5 אינו רלוונטי
     כאן** — קישור `label`/`for` (1.3.1) חל על *כל* שדה טופס בלי
     יוצא מן הכלל, כולל שדות שמתארים נפטר/בית כנסת, אז לא היה
     צורך בסינון.
914. **אפס רגרסיה מאומתת:** `git diff --stat` — 7 קבצים,
     135+/118- (ה-118 מחיקות הן כמעט כולן שורות שנוסח-מחדש
     מ-arrow-expression `=> (...)` ל-block-body `=> { return (...)
     }` כדי לפנות מקום ל-`const labelId = useId()` — אותו JSX
     בדיוק בפנים, רק עטיפה). איזון סוגריים בפייתון על כל 7
     הקבצים המלאים אחרי העריכה — כולם תואמים. אומתתי גם
     שקומפוננטות `Input`/`Textarea` המשותפות (`components/ui/`)
     מעבירות `id` דרך `{...props}` ישירות לתגית ה-HTML המקורית —
     אין קונפליקט עם `id` שכבר קיים בהן. בדקתי גם כפילויות `id`
     בכל קובץ (`grep | sort | uniq -c`) — כולן ייחודיות. הנתיבים
     `apps/01-torah-platform/src`, `apps/15-egod/src` חסומים
     ב-`.gitignore` אך כבר tracked — `git add` רגיל צפוי להצליח
     כרגיל.
915. **הבא בתור:** שני מועמדי הניגודיות שנותרו לא-חד-משמעיים
     מסבב 175 (עדיין דורשים כלי רינדור אמיתי), לבדוק אם ל-
     `16-chatzor-connect`/`12-smel-ndln`/שאר 02-07/10-14 יש אותו
     פער `label`/`for` שנמצא כאן בין 01 ל-15-egod (הסבב הזה כיסה
     רק 01 ו-15-egod — שתי המערכות שדוגלו ב-explore, לא נבדקו
     כל 16 המערכות שדה-שדה), עדשת `required` שדה בלי
     `aria-required`/סימון חזותי, או `core.project_tasks`/
     `core.project_bugs` (עדיין אותם 5 פריטים חסומים).

## 20/08/2026 — סבב 189 (loop A)

916. **המשך עדשת label/for (WCAG 1.3.1/4.1.2) לשאר 02-07/10-14/16:**
     בדיוק המשימה שסומנה "הבא בתור" בסוף סבב 188. סוכן Explore
     סרק את `02-igud-transcribe`, `03-igud-ads`, `04` (imud-torani),
     `05-07`, `10-11`, `12-smel-ndln`, `13-14`, `16-chatzor-connect`
     (לבד מ-08/09 המוגנים) וחזר עם 3 ממצאים אמיתיים מסוג (א) —
     `<label>` כותרת מעל קבוצת כפתורי-בחירה, בלי `role="group"`/
     `aria-labelledby` מקשר. לא נמצאו ממצאי סוג (ב) (שדה טופס אמיתי
     עם `<label>` בלי `htmlFor` בכלל) — כל האפליקציות שנסרקו כבר
     משתמשות בקונבנציית `id`/`htmlFor` תקינה על שדות אמיתיים; ה-04
     (`Wizard.tsx`) כבר תוקן בסבב קודם (משמש כדוגמה שהעדשה מוכרת).
     אימתתי כל אחד משלושת הממצאים בקריאה ישירה לפני עריכה.
917. **תיקון — 3 קבצים, אותה קונבנציה כמו סבב 188:**
     `02-igud-transcribe/app/(public)/upload/page.tsx` ו-
     `03-igud-ads/app/(public)/transcribe/upload/page.tsx` (שני
     קבצים כמעט זהים — טופס העלאת הקלטה, בורר "סגנון עריכה" עם 3
     כפתורי בחירה) קיבלו `id="upload-style-label"` קבוע על ה-`label`
     (מופע יחיד בעמוד, אין צורך ב-`useId()`) ו-`role="group"
     aria-labelledby="upload-style-label"` על מיכל הכפתורים.
     `12-smel-ndln/client/src/pages/Premium.tsx` — קומפוננטת
     `QuestionField` המשותפת, ענף `item.type === "scale"` (שאלון
     דירוג 1-5 עם כפתורי מספרים): הוספתי `const fieldId =
     \`question-${item.key}\`` (אותה קונבנציה בדיוק כמו הענפים
     "single"/"number"/"text" הסמוכים באותה קומפוננטה, שכבר השתמשו
     ב-`fieldId` דרך `item.key`), `id={fieldId}` על ה-`label`,
     `role="group" aria-labelledby={fieldId}` על מיכל הכפתורים —
     מתקן כל שאלת-דירוג בכל שאלון שמשתמש בקומפוננטה, לא רק מופע
     יחיד.
918. **אפס רגרסיה מאומתת:** `git diff --stat` — 3 קבצים, 7+/6-
     (כל שינוי הוא הוספת attribute בודד ל-`<label>`/מיכל קיים, פרט
     לשורת `const fieldId` החדשה ב-Premium.tsx שמקבילה לשורות זהות
     בענפים הסמוכים באותה קומפוננטה). קראתי את שלושת הקבצים במלואם
     לפני ואחרי — אין שינוי מבנה/state/handler/סדר. `fieldId` בענף
     "scale" הוא משתנה מקומי חדש בתוך ה-block scope של ה-`if`,
     ללא התנגשות עם `fieldId` שכבר מוצהר בענפים אחרים (block scope
     נפרד לכל `if`).
919. **הבא בתור:** שני מועמדי הניגודיות שנותרו לא-חד-משמעיים
     מסבב 175 (עדיין דורשים כלי רינדור אמיתי), עדשת `required` שדה
     בלי `aria-required`/סימון חזותי (עדשה חדשה, לא נבדקה עדיין), או
     `core.project_tasks`/`core.project_bugs` (עדיין אותם 5 פריטים
     חסומים על סודות חסרים/החלטות מיזוג מחוץ לסמכות הסוכן). עדשת
     ה-label/for (1.3.1/4.1.2) נחשבת כעת סגורה על כל 16 המערכות
     שנסרקו (01/15 בסבב 188, שאר 02-16 בסבב זה) — לא נמצאו ממצאי
     סוג (ב) נוספים.

## 20/08/2026 — סבב 190 (loop A)

920. **עדשה חדשה: שדה `required` בלי `aria-required`/סימון חזותי
     (WCAG 3.3.2)** — בדיוק הפריט שסומן כ"בא בתור" בסופי סבבים
     188-189. סוכן Explore סרק את 01-07/10-16 (לבד מ-08/09
     המוגנים) בחיפוש אחר שדות טופס שדורשים ערך (בין אם `required`
     HTML גולמי ובין אם ולידציית zod/react-hook-form כמו
     `.min()`/`.email()` בלי `.optional()`) שבהם **גם** אין
     `aria-required="true"` **וגם** אין סימון חזותי (`*`,
     "(חובה)" וכו') בתווית — כלומר משתמש קורא-מסך לא מקבל שום
     איתות שהשדה חובה עד שהוא מגיש את הטופס ומקבל שגיאת ולידציה.
     נמצא ממצא אמיתי אחד: שלושת שדות "פרטי התקשרות"
     (`fullName`/`phone`/`email`) ב-`12-smel-ndln/client/src/
     pages/Premium.tsx` (טופס ה-checkout של הדוח המפורט) — מחייבים
     דרך סכמת zod (`z.string().min(2, ...)` / `.min(9, ...)` /
     `.email(...)`, ללא `.optional()`) אבל ה-`<FormLabel>`/
     `<Input>` לא כוללים שום דבר מהשניים. אימתתי גם את קומפוננטת
     ה-`form.tsx` המשותפת (`components/ui/form.tsx`) — `FormLabel`
     ו-`FormControl` שם לא מזריקים `aria-required`/`*` אוטומטית
     (בניגוד ל-`Field.tsx` המשותף ב-16-chatzor-connect, שכן מזריק
     `*` כש-`required` מועבר — שם זו לא הייתה בעיה). שאר האפליקציות
     שנסרקו כבר משתמשות בקונבנציית `required aria-required="true"`
     עם `*` בתווית (כמו ב-`01-torah-platform`'s `SignUp.tsx`/
     `JoinTeacher.tsx`, `16-chatzor-connect`'s `AdminLogin.tsx`) —
     אומתה הקונבנציה הקיימת בקריאה ישירה לפני שנקבעה כסטנדרט
     ההשוואה.
921. **תיקון — קובץ יחיד, אותה קונבנציה קיימת:** שלושת שדות
     ה-`<Input>` ב-`Premium.tsx` קיבלו `required aria-required=
     "true"` (תואם ל-`16-chatzor-connect/AdminLogin.tsx` ול-
     `01-torah-platform`'s auth/public forms), ושלושת ה-`<FormLabel>`
     המקבילים קיבלו `" *"` בסוף טקסט התווית (תואם ל-`01-torah-
     platform`'s `JoinTeacher.tsx`). לא נגעתי בשום `field`/handler/
     zod schema קיים — רק תוספת attributes על ה-`<Input>` ותוספת
     טקסט על ה-`<FormLabel>`.
922. **אפס רגרסיה מאומתת:** `git diff --stat` — קובץ יחיד, 6+/6-
     (כל שורה שהשתנתה היא הוספת `required aria-required="true"`
     ל-`<Input>` קיים באותה שורה, או הוספת `" *"` לטקסט `<FormLabel>`
     קיים באותה שורה — ללא שינוי מבנה/state/handler/סדר spread).
     קראתי את הקובץ המלא לפני ואחרי העריכה. הנתיב
     `apps/12-smel-ndln/client` חסום ב-`.gitignore` אך כבר tracked —
     הפעם `git add` רגיל סירב (התנהגות git ידועה: תיקיות מוזכרות
     מפורשות ב-`.gitignore` דורשות `-f` גם על קבצים כבר-tracked),
     אז השתמשתי ב-`git add -f` על הקובץ הבודד (לא על התיקייה) —
     תואם למדיניות כי הקובץ כבר tracked ואין תוכן חדש שדולף. Commit
     `42bee089` על `fix/a-icon-only-buttons-round2-0820`, יידחף
     ל-origin (מפעיל פריסת Vercel תחת more30.com/smel-ndln).
923. **הבא בתור:** עדשת ה-`required`/`aria-required` (3.3.2)
     נחשבת סגורה על 01-07/10-16 שנסרקו הסבב הזה (ממצא יחיד תוקן).
     שני מועמדי הניגודיות שנותרו לא-חד-משמעיים מסבב 175 עדיין
     ממתינים לכלי רינדור אמיתי לאימות. `core.project_tasks`
     נשאר עם אותם 5 פריטים חסומים על סודות חסרים/החלטות מיזוג מחוץ
     לסמכות הסוכן (כולל `OPENAI_API_KEY` ל-02-igud-transcribe —
...
## 20/08/2026 — סבב 192 (loop A)

928. **שלוש עדשות מועמדות נבדקו ונסגרו ריקות לפני שנמצא ממצא
     אמיתי.** (1) מודאלים/dropdowns מגולמים-ביד (לא דרך
     Radix/shadcn `Dialog`/`Popover`) שחסרים `role="dialog"`/
     `aria-modal`/Escape-to-close/focus trap — סוכן Explore סרק
     01-16 (לבד מ-08/09) ומצא שכל המודאלים המגולמים-ביד הקיימים
     (`01-torah-platform`'s `LessonDetailModal.tsx` ו-שני מודאלים
     ב-`AdminDashboard.tsx`, `02-igud-transcribe`'s
     `admin/uploads/page.tsx`, `03-igud-ads`'s
     `admin/transcripts/page.tsx`, `16-chatzor-connect`'s
     `components/ui/Modal.tsx`) כבר עם כל השלושה — נסגרת ריקה. (2)
     כפתורי-שליחה שמחליפים טקסט לספינר-בלבד בזמן טעינה בלי
     `aria-label`/טקסט נגיש (WCAG 4.1.3) — נסרקו כל טפסי הכניסה/
     יצירת-קשר/תשלום/CRUD-אדמין ב-01-16 — כל כפתור-טעינה שנמצא כבר
     שומר טקסט גלוי (למשל "שומר…") לצד/במקום האייקון — נסגרת ריקה.
     (3) יומן `core.project_tasks`/`core.project_bugs` נבדק מחדש —
     אותם 5 פריטים חסומים על secrets/החלטות מיזוג מחוץ לסמכות
     הסוכן, שום דבר חדש.
929. **עדשה רביעית — אייקוני-סטטוס/פעולה בודדים בלי שם נגיש (WCAG
     1.1.1/4.1.2, המשפחה שקראה לענף `icon-only-buttons` מלכתחילה)
     — הניבה 3 ממצאים אמיתיים.** סוכן Explore סרק כפתורי אייקון-
     בלבד שמבצעים toggle/פעולת-מצב (לא כפתורי טעינה/מודאל שכבר
     נבדקו) בטבלאות/רשימות ב-01-16: (א)
     `01-torah-platform/src/components/portal/PortalMessagesTab.tsx:136-144`
     — כפתור מחזור-סטטוס פנייה (חדש/טופל/לא טופל) עם `title="שנה
     סטטוס"` קבוע (לא משקף את הסטטוס הנוכחי) ובלי `aria-label`;
     (ב) `15-egod/src/pages/admin/AdminForums.tsx:150-154` — שלושה
     כפתורים (נעץ/בטל-נעיצה, חסום/בטל-חסימה, מחק) בלי `aria-label`
     או `title` בכלל; (ג)
     `15-egod/src/pages/portal/Attendance.tsx:127-138` — שני כפתורי
     סימון נוכחות (נוכח/נעדר) לכל משתתף, בלי `aria-label` בכלל —
     החמור מביניהם, כי לכל כפתור באותה שורה יש את אותו אייקון
     בדיוק (✓/✗) ללא הבדלה טקסטואלית כלשהי בין המשתתפים. אימתתי כל
     שלושת הקבצים בקריאה ישירה (לא רק דיווח הסוכן) — כל שלושה
     נתיבים חיים ומנותבים (לא קוד מת). שתי עדשות דומות (Pin/Ban
     icon usage אחר, ובדיקת badge-status סמוך שכבר כולל טקסט) נבדקו
     ונפסלו — הסוכן אימת שהבדג'ים הקיימים (למשל שורה 117-119 ב-
     `PortalMessagesTab.tsx`) כבר מציגים טקסט+אייקון תקין; רק
     הכפתור עצמו (הפעולה, לא התצוגה) היה חסר.
930. **התיקון — 3 קבצים, אותה קונבנציה כמו `aria-label="החלפת
     לוגו"` הקיימת כבר ב-`RabbiPortal.tsx`:** `PortalMessagesTab.tsx`
     קיבל `aria-label` דינמי שמשקף את הסטטוס הנוכחי ("סטטוס נוכחי:
     ${label}. לחץ לשינוי סטטוס") לצד ה-`title` הקיים, בלי להסיר
     אותו. `AdminForums.tsx` קיבל `aria-label` דינמי (משקף
     is_pinned/is_blocked) על כפתורי הנעיצה/חסימה, וקבוע ("מחק
     פוסט") על כפתור המחיקה. `Attendance.tsx` קיבל `aria-label`
     דינמי הכולל את שם המשתתף ("סמן את ${full_name} כנוכח/כנעדר")
     כדי להבדיל בין שורות זהות, ו-`aria-pressed` (תקדים קיים כבר
     ב-`16-chatzor-connect/PasswordInput.tsx` לכפתורי toggle) כדי
     לחשוף את מצב הבחירה הנוכחי לקורא-מסך. לא נגעתי בשום
     `onClick`/state/handler/className קיים — רק תוספת attributes.
931. **אפס רגרסיה מאומתת:** `git diff --stat` — 3 קבצים, 10+/5-
     (כל שינוי הוא הוספת `aria-label`/`aria-pressed` על `<Button>`
     קיים; שורת ה-`onClick` בשני הכפתורים ב-`Attendance.tsx` פוצלה
     למספר שורות כדי להוסיף את ה-attributes, אך התוכן הפונקציונלי
     זהה). בדיקת איזון סוגריים בפייתון על כל 3 הקבצים המלאים אחרי
     העריכה: `{}`/`()` תואמים בכולם. הנתיבים `apps/01-torah-platform/
     src`, `apps/15-egod/src` חסומים ב-`.gitignore` אך שלושת
     הקבצים כבר tracked — `git add` רגיל סירב (תקדים ידוע), נפתר
     עם `git add -u`. Commit `b84e1ba8` על
     `fix/a-icon-only-buttons-round2-0820`, נדחף ל-origin (מפעיל
     פריסת Vercel תחת more30.com/torah, /egod).
932. **הבא בתור:** להמשיך את עדשת אייקון-סטטוס-בלי-שם-נגיש על שאר
     01-16 שטרם נסרקו שיטתית (הסבב הזה בדק דוגמאות ספציפיות
     שהועלו, לא סריקה ממצה של כל 01-16 — ייתכנו מופעים נוספים
     בטבלאות אדמין אחרות). לחלופין: שני מועמדי הניגודיות שנותרו
     לא-חד-משמעיים מסבב 175 (עדיין דורשים כלי רינדור אמיתי), שני
     מופעי `FormMessage` לא-פעילים מסבב 191 (04-imud-torani,
     15-egod — ממתינים לאימוץ בפועל), או `core.project_tasks`/
     `core.project_bugs` (5 פריטים פתוחים, עדיין חסומים).

## 20/08/2026 — סבב 193 (loop A)

933. **המשך שיטתי של עדשת אייקון-בלי-שם-נגיש (WCAG 1.1.1/4.1.2)
     שנפתחה בסבב 192 — סריקה ממצה של 01-16 (לבד מ-08/09 המוגנים),
     לא רק דוגמאות בודדות.** סוכן Explore חיפש כפתורי-אייקון-בלבד
     (`<Button size="icon">`/`variant="ghost"` עם אייקון lucide
     יחיד כתוכן, בלי טקסט, בלי `aria-label`, בלי `title`) שמבצעים
     פעולה אמיתית (מחיקה/עריכה/שינוי-כמות) בתוך רשימות/טבלאות
     חוזרות — הגרוע ביותר עבור משתמש קורא-מסך כי מספר כפתורים
     עם אותו אייקון בדיוק נשמעים כ"button, button, button" בלי
     דרך להבדיל ביניהם. נמצאו 10 ממצאים אמיתיים ומאומתים (קריאה
     ישירה של כל קובץ, לא רק דיווח הסוכן) ב-6 קבצים: `01-torah-
     platform`'s `shop/Cart.tsx` (כפתורי הפחת/הוסף-כמות ומחיקה
     לכל פריט בעגלה, 3 כפתורים), `shop/ProductDetail.tsx` (כפתורי
     הפחת/הוסף-כמות, 2), `portal/Schedule.tsx` (מחיקת שיעור, 1),
     `portal/StudySchedule.tsx` (עריכה+מחיקה של לוח לימוד, 2),
     `legacy/OrgPortal.tsx` (הסרת רב מרשימה, 1), ו-`15-egod`'s
     `admin/AdminTips.tsx` (מחיקת טיפ, 1).
934. **התיקון — 10 כפתורים ב-6 קבצים, `aria-label` דינמי הכולל את
     שם הפריט הרלוונטי בכל מקום שהמידע זמין** (למשל `` `הסר מהסל:
     ${it.product_name}` ``, `` `מחק לוח לימוד: ${sched.title}` ``,
     `` `הסר רב: ${r.rabbi_name}` ``) כדי להבדיל בין שורות זהות
     לקורא-מסך — תואם לתקדים שנקבע בסבב 192
     (`PortalMessagesTab.tsx`/`Attendance.tsx`). שני כפתורי הכמות
     ב-`ProductDetail.tsx` קיבלו `aria-label` קבוע ("הפחת כמות"/
     "הוסף כמות") כי אין הבדל בין שורות (מוצר יחיד בעמוד). לא נגעתי
     בשום `onClick`/state/handler/className קיים — רק תוספת
     attribute יחיד לכל כפתור.
935. **אפס רגרסיה מאומתת:** `git diff --stat` — 6 קבצים, 10+/10-
     (כל שינוי הוא הוספת `aria-label={...}` על `<Button>` קיים
     באותה שורה, ללא שינוי מבנה/תוכן/סדר attributes אחר). קראתי את
     כל 6 הקבצים במלואם (או את האזור הרלוונטי + imports) לפני
     העריכה ואת ה-diff המלא אחרי. `apps/01-torah-platform/src` ו-
     `apps/15-egod/src` חסומים ב-`.gitignore` אך כל 6 הקבצים כבר
     tracked — `git add -u` הצליח כמו בסבבים קודמים. Commit יוצג
     למטה על `fix/a-icon-only-buttons-round2-0820`, יידחף ל-origin
     (מפעיל פריסת Vercel תחת more30.com/torah, /egod).
936. **הבא בתור:** עדשת האייקון-בלי-שם-נגיש (1.1.1/4.1.2) נחשבת
     סגורה כעת על כל המופעים שנמצאו בסריקה ממצה של 01-16 (סבבים
     192+193 יחד) — לא נמצאו ממצאים נוספים ב-02/03/04/05/06/07/
     10/11/12/13/14/16 מעבר למה שכבר תועד. מועמדים לסבב הבא: שני
     מועמדי הניגודיות שנותרו לא-חד-משמעיים מסבב 175 (דורשים כלי
     רינדור אמיתי), שני מופעי `FormMessage` לא-פעילים מסבב 191
     (04-imud-torani, 15-egod — ממתינים לאימוץ בפועל), או
     `core.project_tasks`/`core.project_bugs` (5 פריטים פתוחים,
     עדיין חסומים על secrets/החלטות מיזוג מחוץ לסמכות הסוכן).

## 20/08/2026 — סבב 194 (loop A)

937. **עדשת אייקון-בלי-שם-נגיש נחשבת סגורה (סבב 193#936) — נבדק
     `portal/api` (תשתית משותפת, admin dashboard incl. API balances,
     בתחום המפורש) ונמצא באג פונקציונלי אמיתי ולא נגיש: `credits.ts`
     (הבדיקה שמזינה את "יתרות ה-API" בלוח האדמין) ו-`spec-analyze.ts`
     (ניתוח AI לאפיון מערכת 33) שניהם קראו למודל Anthropic
     `claude-opus-5` — מזהה שאינו קיים (אומת מול הרפרנס העדכני של
     ה-Claude API: המשפחה הנוכחית היא `claude-fable-5`/
     `claude-opus-4-8`/`claude-sonnet-5`/`claude-haiku-4-5`, אין
     "Opus 5" גולמי). כל קריאה כזו מחזירה `404 not_found_error`
     מ-Anthropic במקום לבדוק את המפתח.
938. **השפעה אמיתית משתמש, לא קוסמטית:** ב-`credits.ts`, ה-`check()`
     של ספק Anthropic בודק `res.status===401/403 → invalid`,
     `!res.ok → unknown`, אחרת `ok` — עם `404` הענף הנבחר הוא
     `unknown` לתמיד, כך שלוח "יתרות ה-API" באדמין (`portal/public/
     admin-credits.html`) אף פעם לא יכול להראות Anthropic כ-`ok`
     גם עם מפתח תקין לגמרי — בדיוק סוג הבאג ש-`credits.ts` עצמו
     מתעד בהערותיו כדבר שהמסך נועד למנוע ("לא נמדד" עדיף על מספר
     יפה בלי כלום מאחוריו, אבל כאן זה גרוע יותר: מצב תקין מדווח
     כלא-ידוע). ב-`spec-analyze.ts`, `AI_MODEL` הוא ברירת המחדל
     כש-`process.env.AI_MODEL` לא מוגדר — כלומר תכונת "שלח לניתוח
     AI" בניהול (מערכת 33) שבורה כברירת מחדל בכל פריסה שלא הגדירה
     במפורש משתנה סביבה חלופי.
939. **התיקון — 2 קבצים, שינוי מזהה מודל בלבד:** `credits.ts` עבר
     ל-`claude-haiku-4-5` (המודל הזול/מהיר ביותר הקיים בפועל —
     מספיק לבדיקת-תקפות-מפתח בלבד, לא לאיכות תשובה). `spec-analyze.ts`
     עבר ל-`claude-opus-4-8` (מודל איכות, תואם את כוונת התכונה —
     ניתוח אפיון אמיתי, לא בדיקת מפתח). `grep` על כל הריפו אימת
     ששני אלה הם המופעים היחידים של `claude-opus-5`/מזהי מודל לא
     תקינים אחרים.
940. **אפס רגרסיה מאומתת:** `git diff` — 2 קבצים, 2+/2- (שינוי מחרוזת
     יחיד בכל קובץ, אין שינוי לוגיקה/מבנה/handler אחר). קראתי את
     שני הקבצים במלואם לפני העריכה. Commit `728a76bb` על
     `fix/a-icon-only-buttons-round2-0820`, יידחף ל-origin (מפעיל
     פריסת Vercel תחת more30.com/portal — משפיע על admin-credits.html
     ועל תכונת ניתוח-ה-AI של מערכת 33).
941. **הבא בתור:** לסרוק שיטתית אם יש עוד מזהי מודל AI לא תקינים
     (OpenAI/Gemini) בקוד המשותף (`portal/api`, `admin/`) או ב-01-16;
     סבב זה בדק רק Anthropic. לחלופין: שני מועמדי הניגודיות שנותרו
     לא-חד-משמעיים מסבב 175, שני מופעי `FormMessage` לא-פעילים
     מסבב 191, או `core.project_tasks`/`core.project_bugs` (5
     פריטים פתוחים, עדיין חסומים).

## 20/08/2026 — סבב 195 (loop A)

942. **בדקתי את המועמד שנפתח בסוף סבב 194 (מזהי מודל AI לא תקינים
     של OpenAI/Gemini) — נסגר ריק.** `grep` על כל `portal/api`
     ו-01-16 (דילוג 08/09) אחר `gpt-*`/`gemini-*` העלה רק מזהים
     אמיתיים וקיימים: `gpt-4o`, `gpt-4o-mini`, `gpt-image-1`
     (יצירת תמונות), `gemini-2.0-flash`, ו-`google/gemini-2.5-flash`
     (דרך Lovable AI Gateway, שמקדים ספק לפני שם המודל בכוונה —
     זו לא טעות הקלדה אלא מוסכמת הניתוב של השער עצמו). קראתי כל
     קובץ רלוונטי (`apps/03-igud-ads/lib/image-generator.ts` כולל
     קריאת vision עם `gpt-4o` על תמונה — תקין, לא ניחוש) לפני
     שסגרתי את העדשה. גם `core.project_bugs` (ריק) ו-
     `core.project_tasks` (אותם 6 פריטים חסומים: 02/12/13/25/32
     סודות/החלטות מיזוג — 25 מחוץ להיקף 01-16 בכלל) נבדקו מחדש,
     שום דבר חדש.
943. **פתחתי עדשה חדשה שלא נבדקה עדיין בכל 15+ הסבבים הקודמים:
     WCAG 2.1.1 (מקלדת) — `<div>`/`<span>` עם `onClick` בלי שום
     דרך הפעלה במקלדת** (לא `role="button"`, לא `tabIndex`, לא
     `onKeyDown`) — בניגוד לעדשת "אייקון-בלי-שם-נגיש" (1.1.1/4.1.2)
     שנסגרה בסבבים 192-193 ובדקה כפתורי `<Button>` אמיתיים (שכבר
     נגישים למקלדת מטבעם), העדשה הזו תפסה כרטיסים/שורות שלמות
     שמגיבות ללחיצת עכבר בלבד. סוכן Explore סרק 01-07/10-16 ומצא
     6 קבצים אמיתיים; אימתתי כל אחד בקריאה ישירה של האזור המלא
     (לא רק דיווח הסוכן) לפני עריכה: `01-torah-platform`'s
     `components/FeaturedLessons.tsx:60-64` (`LessonCard` המשותף —
     נבחר בעמוד הבית של ההאב, מופיע גם ברשימה נגללת וגם ברשת
     מסוננת), `pages/legacy/PublicOrgPage.tsx:124-126` ו-
     `pages/legacy/PublicRabbiPage.tsx:135-137` (כרטיסי שיעור
     ברשימה ציבורית, אותו דפוס בשני קבצים), `pages/legacy/
     LessonDirectory.tsx:227-232` (רשת כרטיסי שיעור ציבורית),
     ו-`pages/admin/MatchingGuru.tsx:133-134,175-176` (שתי רשימות
     בחירה — מבקשי-שיעור ומגידי-שיעור — במסך "מנוע ההתאמה"
     האדמיני). `15-egod`'s `pages/admin/AdminMatching.tsx` הוא אותו
     קובץ בדיוק (שני העותקים תואמים 1:1) עם אותם שני הממצאים.
     8 מופעים בסך הכל, 6 קבצים.
944. **התיקון — אותה מוסכמת שכבר קיימת בקוד הזה עצמו:** `OrgPortal.tsx`
     (שנבדק כדוגמה לפני העריכה) כבר משתמש ב-
     `role="button"` + `tabIndex={0}` + `onKeyDown={e => { if
     (e.key === "Enter" || e.key === " ") { e.preventDefault();
     ACTION(); } }}` על `Badge` לחיץ — אימצתי בדיוק את אותה
     מוסכמת (לא המצאה חדשה) על כל 8 המופעים, כאשר ACTION הוא אותה
     קריאת `onClick` הקיימת בכל אתר. ב-`MatchingGuru.tsx`/
     `AdminMatching.tsx` יש `<Button>` מקונן בתוך חלק מהשורות (עם
     `e.stopPropagation()` קיים) — לא נגעתי בו, הוא כבר נגיש
     למקלדת בעצמו כ-`<button>` אמיתי, וה-`stopPropagation` הקיים
     ממשיך למנוע כפילות הפעלה.
945. **אפס רגרסיה מאומתת:** `git diff --stat` — 6 קבצים, 28+/4-
     (הוספת 3 attributes על אלמנט קיים בכל מופע, בלי לגעת ב-
     `onClick`/state/handler/className קיים). בדיקת איזון סוגריים
     בפייתון על כל 6 הקבצים המלאים אחרי העריכה: `()`/`{}`/`[]`
     תואמים בכולם. שני הנתיבים (`apps/01-torah-platform/src`,
     `apps/15-egod/src`) חסומים ב-`.gitignore` אך כל 6 הקבצים כבר
     tracked — `git add -u` הצליח כמו בסבבים קודמים. Commit יוצג
     למטה על `fix/a-icon-only-buttons-round2-0820`, יידחף ל-origin
     (מפעיל פריסת Vercel תחת more30.com/torah, /egod).
946. **הבא בתור:** להמשיך את עדשת המקלדת (2.1.1) על שאר 02-07/
     10-14/16 שטרם נסרקו שיטתית (הסבב הזה מצא ממצאים רק ב-01/15 —
     אין אישור שהיא סגורה על כל 16, רק שסבב זה לא בדק את השאר
     לעומק). לחלופין: שני מועמדי הניגודיות שנותרו לא-חד-משמעיים
...
## 20/08/2026 — סבב 197 (loop A)

952. **בדקתי את שלושת המועמדים שנותרו בתור מסבב 196 — כולם עדיין
     חסומים, שום דבר חדש לתקן בהם:** שני מועמדי הניגודיות מסבב 175
     עדיין דורשים כלי רינדור אמיתי שאין לו גישה מהסוכן הזה; שני
     מופעי `FormMessage` לא-פעילים ב-`04-imud-torani`/`15-egod`
     עדיין ללא אף קריאה בפועל (`grep '<FormMessage'` העלה רק את
     ההגדרה עצמה בשני הריפואים) — אין השפעת משתמש לתקן; ו-
     `core.project_tasks` (נבדק חי דרך MCP) עדיין אותם 6 פריטים
     (02/12/13/25/32) שדורשים סוד חסר או החלטת מיזוג/מוצר מחוץ
     לסמכות סוכן. פתחתי עדשה חדשה: skip-to-content (WCAG 2.4.1),
     שנסגרה ב-loop B על 17-28 אך מעולם לא נבדקה שיטתית על 01-16
     (loop A). `grep` על `skip-link`/`Skip to content`/`דלג לתוכן`
     בכל 01-07/10-14/16 (דילוג 08/09) הראה שרוב האפליקציות כבר
     מכילות קישור כזה (01/02/03/04/05/06/11/12/14/16) — ורק
     `10-bkalot-rights` (אתר סטטי חי, `stage=wip`, `live=true`)
     חסר לגמרי. `07-zol` ו-`13-property-identity` נבדקו ואומתו
     שוב כ"אין UI לקוח בכלל" (מניפסט בלבד / edge functions בלבד
     בהתאמה, לפי הממצא המאומת בסבב 196) — אין להם מה לתקן.
953. **התיקון — אותה מוסכמת מ-loop B (`06-kupot-holim`'s
     `site/index.html`/`styles.css`, שנקרא כדוגמה לפני העריכה):**
     קישור `<a href="#top" class="skip-link">דלג לתוכן</a>` נוסף
     כילד ראשון של `<body>` ב-`index.html`, לפני `<header>`.
     `#top` הוא כבר ה-`id` הקיים של ה-`<main>` (משמש כבר כיעד
     הקישור של הלוגו ב-header, `href="#top"`) — לא נוצר `id` חדש.
     CSS: `.skip-link{position:absolute;right:-999px;...}` +
     `.skip-link:focus{right:8px}` (זהה למוסכמת 06, `right` ולא
     `left` כי זה אתר RTL) נוסף ל-`style.css`, עם `background:
     var(--teal-solid)` — הטוקן הקבוע-בכוונה (לא מתהפך במצב כהה)
     שכבר מתועד בהערת הקובץ כמיועד בדיוק לרקע+טקסט-לבן-קבוע כמו
     כפתור/תג.
954. **אפס רגרסיה מאומתת:** `git diff --stat` — 2 קבצים, 4+/0-
     (הוספה בלבד: שורת `<a>` יחידה ב-HTML, שני כללי CSS חדשים).
     בדיקת איזון סוגריים בפייתון על שני הקבצים המלאים: `()`/`{}`/
     `[]` תואמים בשניהם. הנתיב `apps/10-bkalot-rights` **אינו**
     חסום ב-`.gitignore` (בניגוד לרוב האפליקציות האחרות בהיקף) —
     `git status`/`git check-ignore` אימתו זאת, `git add` רגיל
     הספיק בלי `-f`. Commit יוצג למטה על
     `fix/a-icon-only-buttons-round2-0820`, יידחף ל-origin (מפעיל
     פריסת Vercel תחת more30.com/bkalot).
955. **הבא בתור:** עדשת skip-to-content (2.4.1) נחשבת סגורה כעת על
     כל 01-16 (08/09 מוגנים; 07/13 אין להם UI לקוח). מועמדים
     לסבב הבא: שני מועמדי הניגודיות מסבב 175 (דורשים כלי רינדור),
     שני מופעי `FormMessage` לא-פעילים מסבב 191, `core.project_tasks`/
     `core.project_bugs` (6 פריטים חסומים), או עדשת נגישות חדשה
     שטרם נבדקה: heading hierarchy (h1→h2→h3 ללא דילוג) או
     `alt`/`aria-hidden` על אייקוני SVG דקורטיביים.

## 20/08/2026 — סבב 198 (loop A)

956. **פתחתי את עדשת ה-SVG הדקורטיבי (`aria-hidden`, WCAG 1.1.1)
     שנרשמה כמועמדת מסבב 197.** סוכן Explore סרק 01-07/10-16
     (08/09 מוגנים, דילוג) וחיפש `<svg>` inline שיושב ליד טקסט
     גלוי שכבר מעביר את אותה משמעות (ולא ה-`<svg>` היחיד בתוך
     כפתור-בלי-שם — זו עדשה נפרדת שכבר נסגרה בסבבים 192-193) —
     ספריות אייקונים כמו `lucide-react` דולגו כי הן כבר מוסיפות
     `aria-hidden="true"` כברירת מחדל. אימתתי כל ממצא בקריאה ישירה
     של האזור המלא (לא רק דיווח הסוכן) לפני עריכה: `03-igud-ads`'s
     `components/NotificationBell.tsx:78-88` (פעמון בתוך כפתור עם
     `aria-label` קיים), `06-kupot-holim`'s `site/app.js:15-24`
     (8 אייקוני SVG-כמחרוזת ב-`ICONS`, מוצגים ליד `<h3>` עם כותרת
     הפרופיל) ו-`app.js:167` (חץ הרחבה/כיווץ ליד כפתור עם
     `aria-expanded` קיים שכבר מעביר את המצב), ו-`15-egod`'s
     `src/pages/Login.tsx:74` (לוגו Google בתוך כפתור עם הטקסט
     "התחבר עם Google" הגלוי). עדשת heading hierarchy (h1→h3 בלי
     h2) שנבדקה באותו סוכן לא מצאה שום דילוג ברור ב-01-16.
957. **התיקון — הוספת `aria-hidden="true"` על כל 10 מופעי ה-SVG
     הדקורטיביים (3 קבצים):** ב-TSX (`NotificationBell.tsx`,
     `Login.tsx`) הוספתי `aria-hidden="true"` כ-attribute נוסף על
     תג ה-`<svg>` הקיים. ב-`app.js` (vanilla JS, מחרוזות HTML)
     הוספתי `aria-hidden="true"` בתוך תג הפתיחה `<svg ...>` בכל
     8 מחרוזות ה-ICONS (כולן חולקות את אותו prefix מדויק —
     `sed` יחיד על שורות 16-24 טיפל בכולן יחד) ובתג ה-`t-arrow`
     בנפרד. לא נגעתי ב-`viewBox`/`path`/`fill`/`stroke`/class/
     event-handler קיים בשום מופע — רק תוספת attribute יחיד.
958. **אפס רגרסיה מאומתת:** `node --check` על `app.js` אחרי העריכה
     — תחביר תקין. בדיקת איזון סוגריים בפייתון על שני קבצי ה-TSX
     המלאים — תואם. `git diff --stat`: 3 קבצים, 12+/11- (ה-`-11`
     הוא מ-`sed` שמחליף את שורת ה-ICONS השלמה per-line, לא מחיקת
     תוכן — ה-diff בפועל הוא הוספת attribute יחיד לכל שורה).
     שלושת הנתיבים (`apps/03-igud-ads/components`,
     `apps/06-kupot-holim/site`, `apps/15-egod/src`) חסומים
     ב-`.gitignore` אך כל 3 הקבצים כבר tracked — `git add -u` עם
     נתיב מפורש הצליח כמו בסבבים קודמים. Commit `a18a555b` על
     `fix/a-icon-only-buttons-round2-0820`, יידחף ל-origin (מפעיל
     פריסת Vercel תחת more30.com/igud-ads, /kupot-holim, /egod).
959. **הבא בתור:** עדשת ה-SVG הדקורטיבי נחשבת סגורה על 03/06/15;
     שאר 01-07/10-16 נסרקו באותו מעבר ולא נמצא בהן כלום (ספריות
     אייקונים כבר תקינות כברירת מחדל). מועמדים לסבב הבא: שני
...
## 20/08/2026 — סבב 200 (loop A)

965. **אימתתי את מועמד ה-pagination-dots שנרשם בסוף סבב 199
     כ"לא מאומת" ומצאתי שהוא באג אמיתי.** קריאה מלאה של
     `15-egod`'s `src/components/home/TipsSection.tsx` אישרה: ה-
     query מביא עד 20 טיפים פעילים (`limit(20)`), אבל שכבת ה-UI
     מציגה תמיד רק 8 נקודות ניווט (`tips.slice(0, 8)`), עם הדגשה
     לפי `index % 8`. כשיש יותר מ-8 טיפים פעילים (מצב שהקוד עצמו
     תומך בו ישירות) זה שובר שני דברים בו-זמנית: (א) לחיצה על כל
     נקודה קופצת רק לאחד מ-8 הטיפים הראשונים — הטיפים 9-20 לא
     נגישים לעולם דרך הנקודות; (ב) ההדגשה הוויזואלית וגם
     `aria-selected` שגויים לכל `index >= 8` (למשל `index=9`
     מדגיש נקודה 1, כשבפועל מוצג הטיפ ה-10, לא הטיפ ה-2) — גם
     משתמש עכבר וגם משתמש קורא-מסך רואים איזו-שהיא נקודה "נבחרת"
     שלא תואמת את הטיפ שבאמת מוצג.
966. **התיקון: מיפוי יחסי (proportional) בין אינדקס הטיפ לאינדקס
     הנקודה, במקום `% 8` קבוע.** `dotCount = Math.min(tips.length,
     8)` (היה `dots`, רק שינוי שם למשמעות ברורה יותר) +
     `activeDot = Math.floor(index * dotCount / tips.length)`.
     כשיש `tips.length <= 8` (המצב היחיד שנבדק בפועל עד היום),
     היחס `dotCount/tips.length` הוא תמיד 1 — כך ש-`activeDot`
     שווה בדיוק ל-`index` וההתנהגות זהה ל-100% למה שהיה קודם
     (`i === index` ולא `i === index % 8`, אותה תוצאה כש-
     `index < 8`). כשיש יותר מ-8 טיפים, הנקודות הופכות לעמודי
     "טווח" יחסיים: לחיצה על נקודה `i` קופצת ל-
     `Math.floor(i * tips.length / dotCount)` — כעת כל 20 הטיפים
     נגישים דרך 8 הנקודות, וההדגשה תמיד תואמת את הטיפ המוצג
     בפועל. גם רכיב ה-`role="tab"` array עבר מ-`tips.slice(0,
     8).map` ל-`Array.from({length: dotCount}, ...)` — נחוץ כי
     המיפוי כבר לא תלוי בתוכן הפריט הספציפי (`_`), רק במספרו.
967. **אפס רגרסיה מאומתת:** בדיקת איזון סוגריים בפייתון על הקובץ
     המלא לאחר העריכה: `()` 41/41, `{}` 46/46, `[]` 9/9 — תואם.
     `grep` על `dots\b` בקובץ אחרי העריכה — אפס מופעים (כל
     ההפניות עודכנו ל-`dotCount`/`activeDot`, אין שארית מהשם
     הישן). `git diff --stat`: קובץ אחד, שינוי לוגי ממוקד בשתי
     נקודות (חישוב + JSX map), לא נגעתי ב-state/handler/CSS/
     interval/query מעבר לזה. הנתיב `apps/15-egod/src` חסום
     ב-`.gitignore` אך כבר tracked — `git add -u` עם נתיב מפורש
     הצליח כמו בכל סבב קודם. Commit יוצג למטה על
     `fix/a-icon-only-buttons-round2-0820`, יידחף ל-origin (מפעיל
     פריסת Vercel תחת more30.com/egod).
968. **הבא בתור:** מועמד ה-pagination-dots נסגר. נותרו: מועמד
     ה-billing `amountAgorot` (0 blast radius, לא נבדק — החבילה
     ללא ייבוא בריפו), שני מועמדי הניגודיות מסבב 175 (דורשים כלי
...
## 20/08/2026 — סבב 203 (loop A) — מסך `/admin` "יתרות וחיבורים": פס-האחוזים של Recraft הראה ירוק/0% קבוע גם כשהקרדיטים נגמרים

981. **מועמד ראשון מ-Explore agent התברר כ-false positive מול ה-DB החי —
     בדיוק המלכודת שכבר תועדה בסבבים קודמים.** הסוכן טען ש-
     `public.more30_is_super_admin()` לא מוגדר (על בסיס קריאת קבצי
     migration תחת `supabase/migrations/`), וש-`more30_admin_users`/
     `more30_admin_leads` (מסך `/admin` המשתמשים) יקרסו. בדקתי ישירות מול
     `uhnrgujbdxhhmoxcjria` דרך Supabase MCP: הפונקציה **כן** קיימת חי,
     עם פרמטר ברירת-מחדל (`p_user uuid DEFAULT auth.uid()`), כך שקריאה
     בלי ארגומנטים תקינה; גם `more30_admin_users` וגם `more30_admin_leads`
     קיימות. לא בוצע תיקון על בסיס הממצא השגוי הזה.
982. **מועמד שני, בהיקף מפורש (`more30.com/admin dashboard incl API
     balances`) — נבדק ישירות בקוד ונמצא אמיתי.** קראתי את
     `portal/api/credits.ts` (שמזין את `/admin` "יתרות וחיבורים") ואת
     `admin/src/App.tsx` (שמצייר אותו) יחד. ל-Recraft (סטודיו המודעות —
     יצירת תמונות) אין ממשק שמחזיר מכסה/`limit` אמיתיים — רק "כמה קרדיטים
     נשארו" (`GET /v1/users/me` → `credits`). הקוד הקיים מיפה את זה לצורת
     הנתונים המשותפת לכל הספקים: `usage: { used: 0, limit: left, percent:
     0, unit: 'קרדיטים' }`. אבל `percent` הזה בדיוק מה שמניע את פס-הבר
     במסך האדמין (`App.tsx:964`, לפני התיקון): רוחב `${percent}%` וצבע
     `percent >= 80 ? אדום : ירוק`. כיוון ש-`percent` קבוע ל-0 עבור
     Recraft בכל מצב — **גם כשנשארו 0 קרדיטים בפועל** — הפס תמיד היה
     מצויר ירוק וריק (0% רוחב), בדיוק המראה של "יש המון, לא נוגע", בעוד
     המצב האמיתי יכול להיות "אין כלום, יצירת תמונות עומדת להיעצר". זו
     בדיוק המשמעות ההפוכה מהכיתוב שכבר קיים בקוד ליד השדה (`p.usage &&
     p.usage.limit > 0` עם ההערה "עמודה ריקה... נקראת כמו אפס נשאר —
     בדיוק ההפך מהאמת") — אבל אותה תקלה קיימת גם בכיוון ההפוך דרך
     `percent`, ולא כוסתה.
983. **התיקון: דגל `noLimit` על `usage`, לא שינוי ערכי הנתונים עצמם.**
     הוספתי שדה אופציונלי `noLimit?: boolean` לטיפוס `usage` בשני
     הקבצים (השדות **חייבים** להיות זהים בין `portal/api/credits.ts`
     ל-`admin/src/App.tsx` — הערת אזהרה מפורשת כבר קיימת בקוד ב-
     `App.tsx:78-84` על תקלת-עבר בדיוק מהסוג הזה, `usedUsd`/`limitUsd`
     שלא הוחזרו מעולם). סימנתי `noLimit: true` רק ב-`usage` של Recraft
     (`portal/api/credits.ts`). ב-`App.tsx`, כש-`noLimit` דלוק: מדלגים על
     ציור פס-האחוזים (הצבע/הרוחב, שאין להם משמעות אמיתית פה) אבל עדיין
     מציגים את הטקסט האמיתי-ומדויק "נותרו X קרדיטים" (הערך `left` שהספק
     כן מחזיר, ללא שינוי). Apify ו-ElevenLabs (יש להם יחס used/limit
     אמיתי מהספק) לא הושפעו כלל — אין להם `noLimit`, הפס שלהם ממשיך
     להיות מחושב בדיוק כמו קודם.
984. **אפס רגרסיה מאומתת:** `git diff --stat`: 2 קבצים, +25/-7, כל
     השינוי תוספתי-מותנה (שדה טיפוס חדש אופציונלי + ענף רינדור נוסף,
     אין מחיקת ענף קיים). בדיקת איזון סוגריים בפייתון על שני הקבצים אחרי
     העריכה: `()`/`{}`/`[]` תואמים. ה"התראה על מוקש" בהערת התיעוד
     ב-`App.tsx:78-84` (זהות שדות בין שני הקבצים) קוימה במפורש — השדה
     החדש נוסף בשני המקומות עם אותו שם וטיפוס. הקבצים האלה אינם חסומים
     ב-`.gitignore` (מחוץ ל-`apps/**`), אין צורך ב-`git add -f`. Commit
     `d0acd660` על `fix/a-icon-only-buttons-round2-0820`, יידחף ל-origin
     (מפעיל פריסת Vercel תחת more30.com/admin ו-more30.com, שני האתרים
     שקוראים ל-`/api/credits`). לא נגעתי במערכות/סכימות מוגנות
     (08/09/bkalut-app/bkalot-admin/zr_*/NEDARIM3873/csj/csj_src/igud),
     ב-`main`, או במערכת מחוץ להיקף (רק 01-16/gannenet/auth/super-
     admin/admin dashboard/homepage ordering/pricing).
985. **הבא בתור:** שני מועמדי הניגודיות מסבב 175 (דורשים כלי רינדור),
     שני מופעי `FormMessage` לא-פעילים מסבב 191, `core.project_tasks`/
...
## 20/08/2026 — סבב 209 (loop A) — `portal/SpecWizard`: כשל רשת בשליחת האפיון תוקע את הכפתור לצמיתות על "שולח…"

1014. **בדקתי מחדש לפני שהתחלתי:** `core.run_progress` (סבב 208 האחרון,
      commit `82809c44`, תואם ל-HEAD), `16-chatzor-connect` נבדק סופית
      (סוכן Explore סרק את כל `src/` — 5,130 שורות — אחר קישורים/URL-ים
      שנבנים מחוץ ל-router; שלושת התיקונים מסבב 208 היחידים שנדרשו, אין
      ממצא נוסף). התור הישן (טבלאות `client_timeline`/`knowledge_chunks`
      — מחוץ להיקף 01-16 לפי #987, שני מועמדי ניגודיות מסבב 175 — כבר
      נבדקו פעמיים ידנית ונשארו לא-חד-משמעיים בכוונה, `FormMessage`
      לא-פעיל — קוד מת בלי קורא בפועל) לא הניב עבודה חדשה אמיתית, אז
      הרצתי סוכן Explore טרי על שטח שלא נסרק עדיין: `admin/`, `portal/`
      (סידור דף הבית, "pricing plumbing"), `packages/billing`,
      `packages/auth`, ו-`apps/40-gannenet`, בחיפוש אחר שגיאות טיפול-
      אסינכרוני/race/ולידציה — לא עדשות a11y/ניגודיות שכבר מוצו.
1015. **הממצא שאומת ישירות בקוד: `portal/src/SpecWizard.tsx`, פונקציית
      `submit()` (שורה 224 ואילך).** זהו אשף האפיון הציבורי בדף הבית של
      ה-portal (`#systems`-אזור, "pricing plumbing"/onboarding) שכותב
      ל-`core.spec_submissions` דרך RPC בשם `submit_spec`. הקריאה
      `await supa.rpc("submit_spec", {...})` (שורה 251 לפני התיקון) לא
      הייתה עטופה ב-`try/catch` — הקוד טיפל רק במקרה שבו ה-RPC **מחזיר**
      `{error}` (למשל שגיאת ולידציה בצד השרת), אבל לא במקרה שבו ה-fetch
      עצמו **נכשל/נדחה** (timeout, ניתוק רשת, DNS) — תרחיש נפוץ מספיק
      בטופס ציבורי בנייד. אימתתי את ה-UI: `state` מתחיל ב-`"idle"`,
      `submit()` קובע `setState("sending")` בשורה 234 **לפני** קריאת ה-
      RPC, וכפתור השליחה (שורה 414) הוא `disabled={state === "sending"}`
      עם טקסט `"שולח…"`. אם ה-`await` נדחה, אין קוד שמגיע להריץ אחריו —
      `state` נשאר `"sending"` לצמיתות, הכפתור נשאר נעול, ואין שום הודעת
      שגיאה (`{err && <div className="err">...}` בשורה 405 לעולם לא
      מתמלא) — המשתמש תקוע על מסך "שולח…" בלי דרך לנסות שוב, ותשובות
      השאלון האמיתיות שהוא מילא הולכות לאיבוד בשקט.
1016. **התיקון:** עטפתי את קריאת ה-RPC (כולל ה-`if (error)` הקיים
      וה-`setState("done")`/`setStars(...)` שאחריה) ב-`try/catch` יחיד.
      נתיב ההצלחה נשאר **מילה-במילה** זהה (אותו payload, אותה בדיקת
      `error`, אותה מעבר ל-`"done"`). ב-`catch`, `setState("error")` +
      `setErr(...)` עם הודעת השגיאה האמיתית (`e.message`) או הודעת
      נפילה כללית בעברית — בדיוק אותו מנגנון `state === "error"` שכבר
      קיים ומטופל ב-UI (מציג `{err}` ומשחרר את הכפתור כי `state` כבר
      לא `"sending"`), כך שהמשתמש יכול לנסות לשלוח שוב בלי לאבד תשובות
      (`a`/`step` לא התאפסו).
1017. **אפס רגרסיה מאומתת:** `git diff --stat`: קובץ אחד, +28/-20 —
      כל השינוי הוא עטיפת קוד קיים ב-`try {}`/הוספת `catch {}`, בלי
      מחיקת/שינוי שום שורת לוגיקה בנתיב ההצלחה או בנתיב `if (error)`
      הקיים. בדיקת איזון סוגריים בפייתון על הקובץ המלא אחרי העריכה:
      תואם (`{`/`}` 159/159, `(`/`)` 187/187). `git check-ignore -v`:
      exit 1 (לא מסונן). לא נגעתי במערכות/סכימות מוגנות (08/09/
      bkalut-app/bkalot-admin/zr_*/NEDARIM3873/csj/csj_src/igud),
      ב-`main`, או במערכת מחוץ להיקף (רק 01-16/gannenet/auth/super-
      admin/admin dashboard/homepage ordering/pricing). Commit
      `6e24c182` על `fix/a-icon-only-buttons-round2-0820`, נדחף
      ל-origin (מפעיל פריסת Vercel תחת `more30.com/portal`).
1018. **הבא בתור:** שני ממצאים נוספים, נמוכי-ודאות יותר, מאותו סוכן
      Explore — טרם אומתו ידנית: (א) `admin/src/App.tsx:433`,
      `sb.auth.getSession().then(...)` בלי `.catch()` (unhandled
      rejection פוטנציאלי אם קריאת ה-session נכשלת); (ב)
      `apps/03-igud-ads/app/api/payments/webhook/route.ts:83-86` —
      עדכון `ad_projects.parameters` מתבצע *לפני* בדיקת הכפילות
      שנוספה בסבב 23/#174 (לא אחריה), כך שבקריאות webhook חוזרות
      במקביל הפרויקט עלול להתעדכן פעמיים לפני שהבדיקה תופסת — סיכון
      נמוך (אותם נתונים בפועל ברוב המקרים) אבל דורש קריאה זהירה של
      סדר הפעולות המלא ב-route לפני נגיעה. לחלופין: שתי הטבלאות מ-987,
      `core.project_tasks`/`core.project_bugs` (עדיין חסומים).

## 20/08/2026 — סבב 210 (loop A) — `admin/src/App.tsx`: כשל ב-`getSession()` תוקע את מסך הניהול לצמיתות על "בודקים התחברות…" (המשך 1018)

1019. **בדקתי מחדש לפני שהתחלתי:** `core.run_progress` (סבב 209 האחרון,
      commit `6e24c182`/`5f264004`, תואם ל-HEAD). סבב 209 השאיר שני
      ממצאים לא-מאומתים בתור — לקחתי את הראשון, ודאי יותר: `admin/
      src/App.tsx:433`.
1020. **הממצא שאומת ישירות בקוד:** `useEffect` שבודק סשן בעת עליית
      מסך הניהול (`admin/src/App.tsx:431-436`) קורא
      `sb.auth.getSession().then(({ data }) => { setSession(data
      .session); setAuthReady(true); });` **בלי** `.catch()`. `authReady`
      מתחיל ב-`false` (שורה 253) ומגדר את כל הרינדור: כל עוד הוא `false`,
      `App.tsx:566-572` מציג אך ורק `"בודקים התחברות…"` ולא ממשיך
      לבדיקת `isAuthed`/מסך ההתחברות. כלומר אם ה-`Promise` של
      `getSession()` נדחה (שגיאת רשת, `localStorage` חסום/מלא באתחול,
      תקלת SDK) — `.then()` אף פעם לא רץ, `setAuthReady(true)` אף פעם
      לא נקרא, והמסך **תקוע לצמיתות** על "בודקים התחברות…" בלי הודעת
      שגיאה ובלי דרך להגיע למסך ההתחברות בכלל. זה בדיוק בהיקף המפורש
      (`admin dashboard`, `customer login/auth`).
1021. **התיקון:** פיצול ה-`.then()` היחיד ל-`.then()/.catch()`. נתיב
      ההצלחה נשאר **מילה-במילה** זהה. ב-`.catch()`: `setAuthReady(true)`
      (כדי שהרינדור ימשיך למסך ההתחברות/דשבורד במקום להיתקע) +
      `setMsg("בדיקת ההתחברות נכשלה: " + ...)` — אימתתי ש-`msg` כבר
      מוצג בפועל בשני מקומות (`App.tsx:605` במסך ההתחברות, `:660`
      בדשבורד עצמו), כך שהמשתמש רואה הודעה אמיתית במקום מסך תקוע חלק.
1022. **אפס רגרסיה מאומתת:** `git diff --stat`: קובץ אחד, +3/-1 —
      פיצול שורה יחידה ל-`.then()/.catch()`, בלי מחיקת/שינוי לוגיקה
      בנתיב ההצלחה. בדיקת איזון סוגריים בפייתון על הקובץ המלא אחרי
      העריכה: תואם (`{`/`}` 1031/1031, `(`/`)` 832/832). לא נגעתי
      במערכות/סכימות מוגנות (08/09/bkalut-app/bkalot-admin/zr_*/
      NEDARIM3873/csj/csj_src/igud), ב-`main`, או במערכת מחוץ להיקף
      (רק 01-16/gannenet/auth/super-admin/admin dashboard/homepage
      ordering/pricing). Commit `cf2f068a` על
      `fix/a-icon-only-buttons-round2-0820`, נדחף ל-origin (מפעיל
      פריסת Vercel תחת `more30.com/nihul`).
1023. **הבא בתור:** הממצא השני מסבב 209 שעדיין לא אומת —
      `apps/03-igud-ads/app/api/payments/webhook/route.ts:83-86` (סדר
...
## 20/08/2026 — סבב 218 (loop A) — `04-imud-torani`: הערות שוליים "יתומות" בייצוא Word עבור כותרות/מילת-פתיחה/פיוט

1061. **בדקתי מחדש לפני שהתחלתי:** `core.run_progress` (סבב 217
      האחרון, commit `1258ceb4`/`c2f15d99`, תואם ל-HEAD). עקבתי אחרי
      ההמלצה מ-1060 והמשכתי לסרוק את `04-imud-torani` — הפעם
      `Templates.tsx`, `DesignDept.tsx`, `shared/proofread.ts`,
      `shared/wizard.ts` ו-`server/docx.ts` (חמישה קבצים שטרם נסרקו
      בהיקף הזה). ארבעת הראשונים נמצאו תקינים; הממצא נמצא ב-
      `server/docx.ts`.
1062. **הממצא שאומת ישירות בקוד:** `apps/04-imud-torani/server/
      docx.ts`, `buildParagraphs()` (מנוע ייצוא ה-Word). הפונקציה
      בונה מראש מפה `anchorFor[blockIndex] → footnote ids` — כל
      בלוק `footnote` בתוכן מקושר לבלוק-התוכן הלא-הערה שקדם לו
      (שורות 111-127), ללא הבדל בין `layer` שלו. אך בלולאת הרינדור
      עצמה, ארבעה מתוך חמישה סוגי בלוקים — `heading`, `subheading`,
      `openingWord` ו-`poem` — בנו את ה-`Paragraph` שלהם ו-`continue`
      **לפני** שהשתמשו ב-`myAnchors` שחושב עבורם; רק הענף המשותף
      של `main`/שכבות משניות (שורה 260, לפני התיקון) הוסיף בפועל
      `FootnoteReferenceRun` לכל `fid` ב-`myAnchors`. התוצאה: הערת
      שוליים שממוקמת בתוכן מיד אחרי כותרת/כותרת-משנה/מילת-פתיחה/
      שורת-פיוט עדיין נכנסת לאזור ההערות בתחתית העמוד ב-Word (הטקסט
      עצמו לא אבד) — אבל **בלי שום סימן/מספר עוגן בגוף הטקסט**
      שמצביע עליה: הערה ממוספרת "צפה" בתחתית העמוד בלי שום קישור
      נראה לעין אליה. זה תרחיש סביר בספרות תורנית — הערת מקור על
      כותרת פרק היא דפוס נפוץ.
1063. **התיקון:** ארבעת הענפים (`heading`/`subheading`/
      `openingWord`/`poem`) מוסיפים כעת `...myAnchors.map((fid) =>
      new FootnoteReferenceRun(fid))` ל-`children` של ה-`Paragraph`
      שלהם — אותו מנגנון שכבר קיים בענף `main`/משני. עבור `poem`
      (בלוק רב-שורות) העוגנים מצורפים רק לשורה האחרונה (`isLast`),
      כדי לא לשכפל את אותה הערה על כל שורה. שום לוגיקה אחרת
      (פונטים/יישור/עימוד/מספור עמודים/TOC) לא נגעה. `git diff
      --stat`: קובץ אחד, +17/-4. אימות: `npx tsc --noEmit -p
      tsconfig.json` לפני ואחרי השינוי מחזיר בדיוק אותן שתי שגיאות
      טרום-קיימות ולא קשורות (`pagedRender.ts:394` ו-`docx.ts`
      `rightTabStop` ב-`TableOfContents`, קיימות ב-`main` וב-HEAD
      לפני התיקון גם יחד) — השינוי שלי לא הוסיף שגיאת טיפוסים
      חדשה. לא נגעתי במערכות/סכימות מוגנות (08/09/bkalut-app/
      bkalot-admin/zr_*/NEDARIM3873/csj/csj_src/igud), ב-`main`, או
      במערכת מחוץ להיקף. `git add` הרגיל עבד הפעם ללא אזהרת
      `.gitignore` כוזבת. Commit `a59cd017` על
      `fix/a-icon-only-buttons-round2-0820`, נדחף ל-`origin` (מפעיל
      פריסת Vercel תחת `more30.com/imud`).
1064. **הבא בתור:** `04-imud-torani` כמעט נסרק במלואו — נותר רק
      `server/seed.ts` (לא נבדק בהיקף הזה, קובץ seed שכנראה לא
      חוסם). מומלץ לסבב הבא: לסיים שם ואז לעבור ל-`16-chatzor-
...
## 20/08/2026 — סבב 223 (loop A) — `admin-credits.html` + `App.tsx`: מד הקרדיטים של ספק `noLimit` נעלם לגמרי כשהקרדיטים נגמרים בפועל (0)

1084. **בדקתי מחדש לפני שהתחלתי:** `core.run_progress` (סבב 222 האחרון,
      commit `60a02b59`/`f58bccea`, תואם ל-HEAD). דיספצ'תי סוכן Explore
      לסריקה מעמיקה של `16-chatzor-connect` (ההמלצה הראשונה מסבב 222)
      — הוא החזיר מועמד יחיד (`createAnnouncement` חסר `organization_id`
      ב-INSERT), שאותו אימתתי ידנית וקבעתי שהוא לא-באג אמיתי: השדה
      `nullable` בסכימה (`0001_chatzor_schema.sql` שורה 125, בניגוד
      ל-synagogues/lessons/community_services שם הוא `not null`),
      אף מקום בקוד לא קורא אותו בחזרה, ומדיניות ה-RLS היחידה לכתיבה על
      `announcements` (`"ann gabai write"`) מבוססת על `synagogue_id`/
      `manages_synagogue()` ולא על `organization_id` בכלל — כך שה-INSERT
      מצליח תמיד ושום הרשאה לא נשברת. גם לא נמצא באג ממשי נוסף אחרי
      סריקה ידנית של auth.tsx/AdminLogin.tsx/ResetPassword.tsx/
      SynagogueGate.tsx/GabaiLayout.tsx/AdminOverview.tsx/useNearby.ts/
      zmanim.ts/ScrollToHash.tsx/InquiryForm.tsx/AskRav.tsx — כל טופס
      בעל disabled={isPending}, כל מוטציה בעלת onError, כל אריח סטטיסטי
      עם מצב "לא זמין" נפרד מ-0.
1085. **פניתי במקום זאת לשכבת super-admin/API-balances** — בהיקף המפורש
      של הסבב הזה, ומסומנת בסבב 222 כטרם נסרקה סריקה ייעודית משלה (רק
      התיקון הממוקד ב-`noLimit` של סבב 221 נגע בה). קראתי את
      `admin/src/App.tsx` (טאב "מפתחות וקרדיטים") ואת `portal/api/
      credits.ts` (ה-API שמזין את שתי התצוגות — הטאב המצומצם והעמוד
      המלא `admin-credits.html`).
1086. **הממצא שאומת ישירות בקוד:** תיקון סבב 221 טיפל נכון בענף
      הפנימי (מד-אחוזים מול טקסט "נותרו X") של ספק `noLimit` (כרגע
      Recraft — היחיד שמחזיר רק "כמה קרדיטים נשארו", לא יחס used/limit
      אמיתי; `credits.ts` שורות 186-194 שם ב-`usage.limit` את הכמות
      שנותרה עצמה, לא תקרה). אבל **שער חיצוני** שלא נגעו בו בסבב 221
      מגן על כל הבלוק (מד+טקסט+cycleEnds) בשתי התצוגות בו-זמנית:
      `App.tsx` שורה 963 (לפני התיקון) ו-`admin-credits.html` שורה 193
      (לפני התיקון) בדקו שניהם `p.usage.limit > 0` באופן גורף, עם הערת
      קוד שמסבירה נכון תרחיש אחד (ספק בלי תקרה מפורסמת בכלל, כמו Apify
      לפני שהוגדרה לו מכסה — שם `limit=0` אכן אומר "אין נתון להראות").
      אבל עבור ספק `noLimit`, `limit` הוא לא "תקרה שאולי לא פורסמה" —
      הוא **הכמות שנותרה בפועל**, ו-0 שם הוא בדיוק הערך הכי חשוב להראות:
      "נגמרו הקרדיטים". `credits.ts` שורה 186 (`const left =
      Number(j?.credits ?? NaN)`) יכול להחזיר 0 באמת כש-Recraft מדווח
      איפוס מלא — תרחיש ריאלי לחלוטין לשירות שממיר תמונות בכל בקשה
      (`detail` באותו קובץ, שורה 193, כבר מניח ש"קרדיטים ייגמרו" כתרחיש
      צפוי). ברגע הזה בדיוק, `p.usage.limit > 0` הופך ל-`false` ובלוק
      השלם — כולל שורת "נותרו 0 קרדיטים" ותאריך האיפוס — נעלם משתי
      התצוגות. הנזק חלקי ולא שקט לגמרי: `p.detail` ("נותרו מעט קרדיטים
      — יצירת תמונות תיעצר כשייגמרו") ממשיך להיות מוצג תמיד (הוא מחוץ
      לתנאי), כי `left <= 50` נכון גם ב-0 — אבל הניסוח "מעט" מטעה ברגע
      שבו כבר אין כלום, והמידע המדויק (המספר עצמו, 0, ותאריך האיפוס
      הבא) נעלם בדיוק כשהוא הכי קריטי לאדמין שמחליט אם לעצור שימוש
      בסטודיו המודעות.
1087. **התיקון:** בשני הקבצים, השער השתנה מ-`p.usage.limit > 0` ל-
      `p.usage.noLimit || p.usage.limit > 0` (עם `Number()` ב-
      admin-credits.html, תואם לסגנון הקיים בקובץ) — ספק `noLimit`
      תמיד מציג את הבלוק (0 הוא ערך לגיטימי ומשמעותי שם), וספקים עם
      תקרה אמיתית ממשיכים להיבדק כרגיל מול `> 0`. הענף הפנימי
      (מד-אחוזים מול טקסט) שתוקן בסבב 221 לא נגע בו כלל.
1088. **אפס רגרסיה מאומתת:** `git diff --stat`: 2 קבצים, +10/-3 (App.tsx
      +7/-2, admin-credits.html +6/-1 כולל הערות). חילצתי את בלוק ה-
      `<script type="module">` מ-`admin-credits.html` והרצתי עליו
      `node --check` — תחביר תקין. אין `node_modules` מותקן ל-`admin/`
      (workspace עם תלויות `@more30/*`, לא הותקן per instructions),
      ולכן `App.tsx` אומת בקריאה מלאה של הדיף ובדיקת איזון סוגריים
      ידנית בלבד — לא `tsc`. לא נגעתי במערכות/סכימות מוגנות (08/09/
      bkalut-app/bkalot-admin/zr_*/NEDARIM3873/csj/csj_src/igud),
      ב-`main`, או במערכת מחוץ להיקף. Commit `2f6252f2` על
      `fix/a-icon-only-buttons-round2-0820`, נדחף ל-`origin` (מפעיל
      פריסת Vercel תחת `more30.com/admin` ו-`more30.com/admin/credits`).
1089. **הבא בתור:** `16-chatzor-connect` נסרק כעת במלואו לתחום הזה
      בלי ממצא נוסף מעבר לתיקון סבב 220 — לא מומלץ עוד סריקה ייעודית
      שם בלי שינוי קוד חדש. מומלץ: `04-imud-torani/server/seed.ts`
      (עדיין לא נקרא, כנראה זרעי הדגמה לא-חוסמים), או המשך סריקת שכבת
      ה-super-admin/API-balances — `credits.ts` עצמו (ספקים נוספים
      מעבר ל-Apify/Recraft/ElevenLabs) עדיין לא נסרק שורה-שורה לאותו
      דפוס "שער גורף שמתעלם מ-noLimit".

## 20/08/2026 — סבב 224 (loop A) — `13-property-identity/engine.ts`: פריט "קרבה לבית כנסת"/"קרבה למרכז מסחר" נעלם משבירת הציון כשמרחק גדול מהסף העליון

1090. **בדקתי מחדש לפני שהתחלתי:** `core.run_progress` (סבב 223 האחרון,
      commit `2f6252f2`/`0dddfe9b`, תואם ל-HEAD). קראתי `credits.ts` שורה-
      שורה כהמלצת 1089 — 12 הספקים הנוספים (openai/anthropic/gemini/maps/
      google-oauth/gov-il/resend/nedarim/nedarim-platform/greeninvoice/
      yemot/runpod/vercel/supabase) אינם מגדירים אובייקט `usage` כלל, כך
      שדפוס "שער גורף שמתעלם מ-noLimit" שתוקן בסבב 223 שייך רק לשלושת
      הספקים שכבר תוקנו (Apify/Recraft/ElevenLabs) — אין ממצא נוסף שם.
      `04-imud-torani/server/seed.ts` נקרא במלואו (7 שורות, `no-op` מתועד
      במפורש — הסכימה מנוהלת ב-migration, לא בזמן ריצה — לא חוסם, שום
      תיקון נדרש). דיספצ'תי במקביל שני סוכני Explore לסריקת אפליקציות עם
      פחות ביקורים בהיקף הזה: `05-financial-marketing-site`/`07-zol`
      (אתרי שיווק סטטיים/ריקים — ללא ממצא, כמצופה מסבב 52) ו-
      `11-bkalut-marketing2`/`13-property-identity` (11 אתר שיווק סטטי
      ללא ממצא; 13 העלה ממצא אמיתי ב-`smart-research/functions/nadlan-
      smart-research/engine.ts` — קוד מקור וקטור אמיתי שכבר טופל ב-DB-
      drift-fix בסבב 52, ראה #303-305).
1091. **הממצא שאומת ישירות בקוד:** `buildSmartProfile()` ב-`engine.ts`
      בונה `breakdown: ScoreItem[]` מ-12 גורמי ניקוד. עשרה מהם (למשל
      אשכול חברתי-כלכלי שורה 217, קרבה לתחבורה ציבורית שורה 222, קרבה
      לציר תנועה ראשי שורה 266-268/distRoad) תמיד מבצעים `score += pts;
      breakdown.push(...)` — כולל כש-`pts === 0` (למשל distRoad כש-
      `distRoad > 1200`, שורה 268: התנאי המשולש מחזיר `0` בענף האחרון,
      ובכל זאת נדחף ל-breakdown ללא שום `if`). אבל שני גורמים סמוכים,
      `distSynagogue` (שורות 261-263) ו-`distCentral` (שורות 270-272),
      עוטפים את אותו `score += pts; breakdown.push(...)` בתנאי נוסף
      `if (pts) { ... }` שלא קיים בשום גורם אחר בפונקציה. תרחיש כשל
      אמיתי: נכס שנמצא יותר מ-1000 מ' מבית הכנסת הקרוב, או יותר מ-2500
      מ' ממרכז מסחר קרוב — שני מרחקים סבירים לגמרי בפרברים/יישובים
      פרוסים — מקבל `pts=0` (הענף האחרון בכל תנאי משולש), וה-`if(pts)`
      מדלג לגמרי על ה-`push`. הציון עצמו (`score`) לא נפגע (הוספת 0 לא
      משנה כלום), אבל פריט השבירה (`score_breakdown`, המוחזר ב-JSON של
      ה-Edge Function ונצרך על ידי `test_engine.ts` שמדפיס את כל
      הפריטים) חסר לגמרי את שני הגורמים האלה בדיוק במקרה שבו המשתמש
      הכי צריך לראות "בדקנו קרבה לבית כנסת/מרכז מסחר, והיא רחוקה" —
      במקום זה הגורם נעלם כאילו לא נבדק כלל, בניגוד לכל שאר 10 הגורמים
      שתמיד מדווחים גם ניקוד שלילי/אפס.
1092. **התיקון:** הוסרה עטיפת ה-`if (pts)` משני הגורמים, כך ש-`score +=
      pts; breakdown.push(...)` מתבצע תמיד — בדיוק אותו דפוס שכבר קיים
      ומאומת בעשרת הגורמים האחרים באותה פונקציה (כולל `distRoad`, הגורם
      הסמוך במבנה זהה לחלוטין: אותו תנאי משולש עם `0` כענף אחרון, בלי
      עטיפה). שום סף/נקודות/גורם אחר לא נגע. `test_engine.ts` לא מכיל
      assertions קשיחות על גורמים ספציפיים (רק מדפיס את `score_breakdown`
      הקיים), כך שהתיקון לא שובר בדיקה קיימת.
1093. **אפס רגרסיה מאומתת:** `git diff --stat`: קובץ אחד, +2/-2. אין
      `deno`/`node_modules` מותקן מקומית לקובץ Deno Edge Function גולמי
      (per instructions — אין הרצת install); בדיקת איזון סוגריים/מסולסלים/
      מרובעים בפייתון על הקובץ המלא אחרי העריכה: תואם (`(`: 223/223,
      `{`: 108/108, `[`: 27/27). לא נגעתי במערכות/סכימות מוגנות (08/09/
      bkalut-app/bkalot-admin/zr_*/NEDARIM3873/csj/csj_src/igud), ב-`main`,
      או במערכת מחוץ להיקף (13-property-identity נמצא בטווח 01-16). `git
      add -f` נדרש (אותה אזהרת `.gitignore` כוזבת שתועדה בסבבים קודמים על
      `apps/**`). Commit `7be9e7a6` על `fix/a-icon-only-buttons-round2-
      0820`, נדחף ל-`origin` (מפעיל פריסת Vercel לכלל המונורפו; 13 עצמה
      עדיין `stage: wip`/`live: false`/`deployTarget: unknown` לפי
      `app.json` — הפונקציה נפרסת בפועל ל-Supabase Edge Functions, לא
      Vercel, כשתאושר פריסה).
1094. **הבא בתור:** יתר `portal/public/admin-*.html` שטרם נסרקו לעומק
      (activity/automation/customers/imud/issues/kesef/orech/pricing/
      rights/smel/studio/systems — נסרקו בסבב 222 רק לדפוס double-escape
      ולא לדפוסים אחרים), או סבב ייעודי לאפליקציות עם מעט ביקורים יחסית
      בהיקף הזה (06-kupot-holim/10-bkalot-rights/12-smel-ndln יש להן הרבה
      אזכורים אך רובם תיעוד דריפט DB, לא סריקת קוד UI לעומק).

## 20/08/2026 — סבב 225 (loop A) — `portal/public/admin-automation.html`: רענון תור/נושאים אחרי שמירה/ריצה-יבשה נכשל בשקט

1095. **בדקתי מחדש לפני שהתחלתי:** `core.run_progress` (סבב 224 האחרון,
      commit `7be9e7a6`/`22e5493d`, תואם ל-HEAD). המלצת 1094 הייתה לסרוק
      את 12 עמודי `admin-*.html` שנסרקו בסבב 222 רק לדפוס double-escape
      ספציפי, לא לדפוסים אחרים. דיספצ'תי סוכן Explore לקרוא את כל 12
      הקבצים במלואם מול ה-API-backends שלהם (`portal/api/*.ts`), בחיפוש
      אחר מחלקות הבאגים החוזרות בהיקף הזה: double-escape, שער-שמסתיר-0,
      מוטציות בלי טיפול-שגיאה, ערכי `undefined`/`null` גולמיים, מרוצי
      double-submit. הסוכן דיווח 3 ממצאים; ממצא אחד (`admin-systems.html`
      שורה 237, RPC של תקלות שנכשל בשקט) התברר כהחלטת עיצוב מתועדת
      במפורש בהערה בקוד עצמו ("עדיף כרטיס חסר שורה מאשר מסך ריק") — לא
      באג, נדחה. שני הממצאים האחרים היו אותו דפוס בדיוק בקובץ אחד.
1096. **הממצא שאומת ישירות בקוד:** `admin-automation.html` (עמוד "אוטומציית
      תוכן"). שני מטפלי לחיצה — `save` (שורות 218-232, שמירת נושא) ו-
      `dryrun` (שורות 235-247, ריצה יבשה) — מבצעים פעולה, מציגים הודעת
      הצלחה ל-`msg`/`m`, ואז קוראים שוב ל-`more30_auto_overview` כדי
      לרענן את טבלאות הנושאים/התור. שני הקריאות השתמשו ב-`if(!r.error){
      data=r.data; render...(); }` — כלומר כשה-refetch נכשל (רשת, timeout,
      RLS), שום דבר לא קורה: אין הודעת שגיאה, אין `console` log, הטבלאות
      נשארות עם הנתון הישן. המשתמש רואה "נשמר. X זכויות בקטגוריה" (או
      "נבנה ואומת: X · בעיות: Y") ומניח שהתצוגה עדכנית, בעוד שבפועל היא
      לא — בדיוק דפוס ה"מוטציה בלי טיפול שגיאה" שכבר תוקן פעמיים בהיקף
      הזה (סבב 220, `markRead` ב-16-chatzor-connect; וה-`if(r.error) throw
      r.error` הקיים כבר בטעינה הראשונית של אותו קובץ, שורה 156 — כך
      שהקובץ עצמו כבר מכיר את הדפוס הנכון ולא יישם אותו עקבי בשני מקומות
      נוספים).
1097. **התיקון:** בשני המטפלים, ה-`if(!r.error){...}` הוחלף ב-`if(r.error){
      msg.textContent+=' (הרענון נכשל — רענן את הדף כדי לראות את השינוי)';
      return; } data=r.data; render...();` — הודעת ההצלחה המקורית נשארת
      (הפעולה עצמה הצליחה, אין לשקר על כך), אבל נוסף לה משפט מפורש שהרענון
      נכשל והתצוגה לא עודכנה. לא נגעתי בטעינה הראשונית (שורה 156, כבר
      תקינה) ולא בשום מטפל אחר.
1098. **אפס רגרסיה מאומתת:** `git diff --stat`: קובץ אחד, +4/-2. חילצתי
      את בלוק ה-`<script type="module">` והרצתי עליו `node --check` —
      תחביר תקין. לא נגעתי במערכות/סכימות מוגנות (08/09/bkalut-app/
      bkalot-admin/zr_*/NEDARIM3873/csj/csj_src/igud), ב-`main`, או
      במערכת מחוץ להיקף. `git add -f` נדרש (אותה אזהרת `.gitignore`
      כוזבת על `portal/public/**` שתועדה בסבבים קודמים). Commit `a0078cd5`
      על `fix/a-icon-only-buttons-round2-0820`, נדחף ל-`origin` (מפעיל
      פריסת Vercel תחת `more30.com/admin/automation`).
1099. **הבא בתור:** 11 עמודי `admin-*.html` הנותרים (activity/customers/
      imud/issues/kesef/orech/pricing/rights/smel/studio/systems) נסרקו
      עכשיו גם לדפוסים האלה בלי ממצא נוסף מעבר לתיקון סבב 225 — לא
      מומלץ עוד סריקה ייעודית שם בלי שינוי קוד חדש. מומלץ: סבב ייעודי
      לאפליקציות עם מעט ביקורים יחסית בהיקף הזה (06-kupot-holim/
...
## 20/08/2026 — סבב 233 (loop A) — `03-igud-ads/app/(admin)/admin/templates/page.tsx`: תיקון `save()` (עריכה) שחסר basePath

1138. **המשך ישיר להמלצת סבב 232** — סרקתי עמודי admin נוספים תחת
      `03-igud-ads` לדפוס basePath חסר בענף אחד מתוך URL מותנה (`isEdit
      ? ... : ...`), הדפוס שאותר בסבבים 231-232. מצאתי מופע נוסף
      ב-`templates/page.tsx`.
1139. **הבאג:** `save()` (שורה 104-135) בנה את כתובת השמירה כך: שורה
      119 — `` const url = isEdit ? `/api/admin/templates/${form.id}` :
      "/modaot/api/admin/templates" `` — כלומר יצירת תבנית חדשה (ה-POST,
      ענף ה-`else`) כללה `/modaot` נכון, אבל עריכת תבנית קיימת (ה-PATCH,
      ענף `isEdit`) בנתה URL **בלי** `/modaot` — פגיעה ב-404 בשורש
      הדומיין במקום ב-`/modaot/api/admin/templates/<id>`. שלוש קריאות
      ה-`fetch` האחרות באותו קובץ — `load()` (שורה 63), `del()` (שורה
      141) ו-`toggleActive()` (שורה 152) — כבר כללו `/modaot` נכון; רק
      ענף העריכה ב-`save()` היה שגוי. כיוון שה-404 מוחזר כ-HTML, `r.json()`
      (שורה 126) זורק לפני שהודעת השגיאה המיועדת (`j.error`) נקראת, כך
      שאדמין שעורך תבנית קיימת ולוחץ "שמירה" רואה שגיאת JSON לא ברורה
      במקום עדכון מוצלח — והעריכה לא נשמרת בפועל (יצירת תבנית חדשה
      עבדה תקין תמיד).
1140. **התיקון:** שורה אחת — `` `/api/admin/templates/${form.id}` `` ⇒
      `` `/modaot/api/admin/templates/${form.id}` ``. שום שינוי אחר
      בקובץ.
1141. **אפס רגרסיה מאומתת:** `git diff --stat`: קובץ אחד, +1/-1. שום
      פיצ'ר/מסך/כפתור לא הוסר, רק תוקנה כתובת שגויה. בדיקת איזון
      סוגריים/מסולסלים/מרובעים בפייתון על הקובץ המלא לאחר העריכה: תואם
      (`(`: 153/153, `{`: 133/133, `[`: 29/29). `tsc` לא זמין בסביבה —
      אימתתי ידנית ששלוש קריאות ה-`fetch` האחרות בקובץ כבר השתמשו באותה
      תבנית `/modaot/api/...` נכונה. לא נגעתי במערכות/סכימות מוגנות
      (08/09/bkalut-app/bkalot-admin/zr_*/NEDARIM3873/csj/csj_src/igud),
      ב-`main`, או במערכת מחוץ להיקף (03 בטווח 01-16).
1142. **הבא בתור:** עדיין פתוחים מסבבים קודמים: `glossary/page.tsx`
      `remove()` (בלי error handling/in-flight guard) ו-`uploads/page.tsx`
      `remove()` (אותו דפוס). מומלץ להמשיך לסרוק עמודי admin תחת
