            ...
            {(credits ?? []).map((p) => (
              <div key={p.key} style={card}>
                ...
                {/* מד היתרה רק כשיש תקרה אמיתית. עמודה ריקה ליד ספק שאינו
                    ...
                    ספק noLimit שם ב-limit את "כמה נשאר" עצמו (לא תקרה) — 0 שם
                    הוא בדיוק הרגע הכי חשוב להראות (נגמרו הקרדיטים), לא "אין
                    תקרה אמיתית", ולכן לא נסנן אותו כאן. */}
                {p.usage && (p.usage.noLimit || p.usage.limit > 0) && (
