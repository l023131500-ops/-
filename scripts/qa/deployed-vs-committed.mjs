...
// Only sites whose source tree is small enough to build here and whose live URL
// is a plain static mount. Apps under apps/ are force-added into git one file at
// a time (.gitignore /apps/**), so they are the tree this check most wants to
// cover — but each needs its own build command and mount, so they get added as
// they are verified rather than assumed.
const TARGETS = [
  { name: 'admin', dir: 'admin', url: 'https://more30.com/admin/' },
]
...
async function check(target) {
  const result = { name: target.name, url: target.url, ok: false, assets: [] }
...
if (asJson) {
  ...
  for (const r of results) {
    ...
    for (const a of r.assets) {
      ...
      console.log(`  ${a.ok ? 'identical' : 'DIFFERS  '}  ${a.ref}  ${tail}`)
    }
  }
  console.log('')
}
