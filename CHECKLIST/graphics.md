...
- [ ] **13. לולאת סוכנים** — מנהל-אמנותי ← מעצב ← מבקר QA, טיוטה→ביקורת→ליטוש.
      ...
      **[24/08/2026 Loop C] הלולאה האוטומטית המלאה — נבנתה.** אחרי שכל אבני
      הבניין אומתו חיות בסבבים הקודמים (ביקורת, `resolveCritiqueFix` עם
      רשימת-ההרשאה הסגורה + clamp, "החל את כל התיקונים", "בדוק שוב"), הצעד
      שנדחה שלוש פעמים הפך לקומפוזיציה קטנה ובטוחה שלהן: כפתור חדש **"ליטוש
      אוטומטי"** בסרגל העליון (`Editor.tsx`, `handleAutoPolish`, ליד "ביקורת
      AI") מריץ ביקורת → החלת כל התיקונים החד-משמעיים → ביקורת חוזרת, בלי
      אדם בין האיטרציות, עד 3 סבבי-תיקון. בטיחות: (א) כל fix עובר בדיוק את
      אותו `resolveCritiqueFixFor` (שכבה קיימת, שדה מ-9 שדות רשימת-ההרשאה,
      clamp טווח/hex) כמו הכפתורים הידניים — ללולאה אין שום הרשאה חדשה; (ב)
      **guard מבקר-QA**: סבב שהוריד את הציון מוחזר לאחור (הגרסה הטובה נשמרת,
      הלולאה נעצרת) — הלולאה לעולם לא מסיימת עם טיוטה גרועה מזו שקיבלה; (ג)
      עצירה מוקדמת כשאין תיקונים ניתנים-לפתרון או כשהציון ≥ 90; (ד) הלולאה
      עובדת על עותק מקומי של המסמך (`requestCritique(d)`/`resolveCritiqueFixFor(d,…)`
      — הפונקציות הקיימות הופרמו על מסמך-פרמטר, אפס שינוי התנהגות לנתיב
      הידני) ומעדכנת את העורך פעם אחת בסופה — כך שהביקורת-החוזרת מבקרת בדיוק
      את מה שהוחל, בלי מרוץ מול state של React; (ה) שום דבר לא נשמר לשרת —
      "שמור פרויקט" נשאר החלטת אדם. UI: הכפתור מציג "מלטש… סבב N"; דיאלוג
      הביקורת נפתח בסיום עם הביקורת האחרונה + שורת **"מסלול הליטוש האוטומטי"**
      (למשל ‎64 ← 71 (אחרי 3 תיקונים) ← 78‎, כולל סימון סבב שבוטל כי הציון
      ירד); ביקורת ידנית חדשה מאפסת את המסלול. כפתורי "ביקורת AI"/"ליטוש
      אוטומטי" נועלים זה את זה בזמן ריצה. אפס שינוי שרת/סכמה — endpoint
      `/api/ai/critique` הקיים בלבד, אין מה לשקף ל-`vercel-adapter`. **לא
      נבדק חי ולא נפרס** (אותה מגבלת `node_modules`/`vite build` שתועדה מעלה)
      — אומת בקריאה + בדיקת איזון-סוגריים דיפרנציאלית מול HEAD (דלתא מאוזנת
      לחלוטין). **נשאר לסבב שיגיע ל-build:** פריסה + אימות Playwright חי של
      הלולאה (כפתור → סבבים → מסלול ציונים → guard ההחזרה), ואז אפשר לסמן
      את הפריט כולו כבנוי.
...
- [ ] **17. וידאו קידום — הרכבה בפועל (Ken Burns + סנכרון קריינות)** — ממשיך
      ...
      **[20/08/2026 Loop C] כתוביות עבריות בגוף הוידאו** — לא פריט חדש
      בצ'קליסט, המשך ישיר לתכנון פריט זה: רוב הצפייה במדיה חברתית היא בלי
      קול, ווידאו קידום שכל "הטקסט" שבו הוא פס-קול בלבד מפספס את הקהל הזה.
      `lib/videoExport.ts` מפצל את `narrationScript` הקיים (אותו תסריט
      שכבר נשמר לצד הפרויקט מפריט 16 — אין שדה/מיגרציה חדשים) למשפטים
      (ומחדש-מפצל משפט ארוך מדי בגבול מילה, עד ~70 תווים למקטע לקריאות),
      ומחלק את משך הוידאו בין המקטעים לפי יחס אורך-תווים (אין timestamps
      אמיתיים מ-ElevenLabs — `server/narration.ts` מחזיר רק אודיו) עם
      רצפת-זמן מינימלית (1.1 שניות) כדי שמקטע קצר לא יהבהב. כל פריים מצייר
      את הכתובית הפעילה על גבי תנועת ה-Ken Burns — פס רקע כהה מעוגל
      בתחתית + טקסט לבן ממורכז, `ctx.direction="rtl"` + `wrapText` הקיים
      (`lib/autofit.ts`, אותו מנוע גלישת-שורות ששכבות טקסט בעורך כבר
      משתמשות בו) לגלישת-שורות עברית תקינה. פונט "Heebo" — הפונט שכבר טעון
      בעמוד כברירת מחדל ל-UI כולו (`index.html`/`index.css`), אין תלות
      בטעינת פונט נוספת. UI: מתג "כתוביות עבריות בגוף הסרטון" חדש בדיאלוג
      הקריינות, דלוק כברירת מחדל, אפשר לכבות; ללא כתובית כשאין תסריט (זהה
      להתנהגות הקודמת). אפס רגרסיה: `exportPromoVideo` עם הפרמטרים הישנים
      בלבד (בלי `narrationScript`) מתנהג בדיוק כמו קודם — אין כתוביות. **לא
      נבדק חי** (אותה מגבלת build/dev-server שתועדה מעלה) — נבדק בקריאה
      ישירה מול חתימות הפונקציות הקיימות (`wrapText`,
      `CanvasRenderingContext2D` תקני). נשאר לסבב שיגיע לדפדפן אמיתי: אימות
      חזותי של מיקום/גודל הכתוביות מול וידאו אמיתי, יחד עם שתי הבדיקות
      שתועדו מעלה.

      **[20/08/2026 Loop C, סבב 22] כתוביות אנגליות מקבילות (תרגום AI)** —
      המשך ישיר, לא פריט חדש: כתוביות עבריות-בלבד מפספסות קהל לא-דובר-עברית
      שצופה בלי קול. `server/ai.ts`: `translateCaptionsToEnglish(script)`
      חדשה — אותו דפוס `claude()`/`extractJson()` בדיוק כמו `generateCopy`/
      `critiqueDesign` (לא ספק/תשתית חדשים), מתרגמת את `narrationScript`
      לאנגלית טבעית (לא מילולית), לעולם לא ממציאה עובדות. `POST
      /api/ai/translate-captions` חדש ב-`routes.ts` + עותק זהה
      ב-`vercel-adapter/api/_lib/server/{ai,routes}.ts` (אותה מוסכמת-ראי
      שכל route/פונקציה בסטודיו הזה כבר עוקבת אחריה). `lib/videoExport.ts`:
      `PromoVideoOptions.captionScriptEn` אופציונלי — כשקיים, `drawCaptionOverlay`
      מצייר שורה שנייה מתחת לעברית (LTR, פונט קטן יותר, אותו פס-רקע); כש-
      `captionScriptEn` חסר, הגובה/הציור זהים ל-100% למה שהיה (אפס רגרסיה).
      UI: מתג חדש "גם כתוביות באנגלית" בדיאלוג הקריינות (מתחת למתג הכתוביות
      העבריות הקיים, מוצג רק כשהוא דלוק) — הפעלה ראשונה מפעילה תרגום
      אוטומטית (עם מצב טעינה + תצוגה מקדימה מקוצרת), כיבוי-תרגום כושל
      מכבה את המתג בחזרה כדי שלא ייראה כאילו הפיצ'ר פעיל בזמן שהוא לא. כפתור
      הייצוא ננעל גם בזמן תרגום כדי שלא ייווצר וידאו עם כתוביות אנגליות
      חלקיות/חסרות. אפס שינוי לקריינות/לכתוביות העבריות/לרינדור הקיימים.
      **לא נבדק חי** (אותה מגבלת build/dev-server) — נבדק בקריאה ישירה +
      בדיקת איזון-סוגריים (Python) על שישת הקבצים שנערכו. נשאר לסבב שיגיע
      לדפדפן אמיתי: אימות חזותי של בלוק הכתוביות הכפול מול וידאו אמיתי,
      ואימות שהתרגום נשמע/נראה נכון על תסריט אמיתי.

      **[20/08/2026 Loop C, סבב 23] מוזיקת רקע** — המשך ישיר, לא פריט חדש:
      עד כה הפס-קול היחיד היה הקריינות עצמה — אין ספריית מוזיקה חינמית מוכנה
      בכלי, ולכן לא הומצא/הוזרק מקור מוזיקה משום ספק; הפתרון הוא שהמדריך/ה
      מעלה קובץ אודיו משלו/ה (`accept="audio/*"`, נשאר אובייקט-URL מקומי
      לדפדפן בלבד — לא נשמר עם הפרויקט/ב-`layersJson`, לא כמו `narrationScript`/
      `narrationAudioUrl`). `lib/videoExport.ts`: `PromoVideoOptions.musicUrl`/
      `musicVolume` חדשים; כש-`musicUrl` קיים, הקריינות (אם יש) והמוזיקה
      מעורבבות דרך Web Audio API (`AudioContext.createMediaElementSource` לכל
      אחת + `GainNode` נפרד לכל אחת, קריינות בעוצמה מלאה + מוזיקה מוחלשת,
      ברירת מחדל 25%) ל-`MediaStreamAudioDestinationNode` יחיד שמוזן ל-
      `MediaRecorder` — כי `MediaRecorder` לא תופס בבטחה שני tracks אודיו
      מקבילים בכל דפדפן. המוזיקה בלולאה (`<audio loop>`) אם קצרה ממשך
      הקריינות. **אפס רגרסיה**: כש-`musicUrl` חסר, או אם `AudioContext`/
      `createMediaElementSource` זורקים, הקוד נופל בדיוק לנתיב
      `audioEl.captureStream()` הישיר הקודם — פלט זהה-ביט לכל פרויקט קיים.
      UI (`Editor.tsx`): שדה "מוזיקת רקע" חדש בדיאלוג הקריינות (בחירת קובץ /
      הסרה + שם הקובץ), עם `Slider` עוצמה (0–80%, ברירת מחדל 25%) שמופיע רק
      כשנבחר קובץ — אותו קומפוננט `Slider` ש-`FontControl`/בקרות הטקסט כבר
      משתמשות בו. **לא נבדק חי** (אותה מגבלת build/dev-server שתועדה בכל
      הפריטים למעלה) — נבדק בקריאה ישירה + בדיקת איזון-סוגריים/תגי-JSX על שני
      הקבצים שנערכו. נשאר לסבב שיגיע לדפדפן אמיתי: אימות שהערבוב בפועל נשמע
      נכון (איזון עוצמות, לולאת מוזיקה חלקה), ותמיכת `createMediaElementSource`
      בכל הדפדפנים היעד.

      **[20/08/2026 Loop C, סבב 24] וידאו-מוזיקה-בלבד (בלי קריינות בכלל)** —
      תיקון פער אמיתי, לא פריט חדש: מאז סבב 23 `exportPromoVideo` תומכת
      במלואה בוידאו בלי `narrationAudioUrl` (מוזיקה בלבד), אבל כל פאנל
      הכתוביות/המוזיקה/הכפתור ב-`Editor.tsx` היה עטוף ב-`{narrationAudioUrl &&
      (...)}` יחיד — כלומר בלי ליצור קריינות קודם, אי אפשר היה להגיע לכפתור
      הייצוא בכלל, גם אם המשתמש/ת רצו רק וידאו שקט עם מוזיקת רקע (פורמט נפוץ
      ברשתות חברתיות). התנאי הרחב עכשיו ל-`{(narrationAudioUrl || musicFile)
      && (...)}`; מתג הכתוביות (עברית/אנגלית) ממשיך להופיע רק כש-
      `narrationScript` לא ריק (כתוביות בלי טקסט קריינות חסרות משמעות בכל
      מקרה). `lib/videoExport.ts`: כש-`musicUrl` קיים בלי `narrationAudioUrl`,
      משך הוידאו הוא קבוע `MUSIC_ONLY_DURATION_SEC=10` (קליפ-קידום קצר
      סטנדרטי) במקום ליפול ל-`FALLBACK_DURATION_SEC=6` שנועד רק כרשת-ביטחון
      לכשל טעינת מטא-דאטה של קריינות — ולא למוזיקה-בלבד. UI: הודעת-הבהרה
      חדשה ("בלי קריינות — הוידאו יהיה קליפ קצר (10 שניות)...") מופיעה כשנבחר
      קובץ מוזיקה בלי קריינות; תווית עוצמת המוזיקה משתנה מ"ביחס לקריינות" ל-
      "עוצמת המוזיקה" בלבד באותו מצב. אפס רגרסיה: מסלול עם קריינות מתנהג
      זהה-ביט למה שהיה (התנאי `narrationAudioUrl ||` הוא הרחבה, לא שינוי;
      `MUSIC_ONLY_DURATION_SEC` מופעל רק כש-`narrationAudioUrl` חסר).
      **לא נבדק חי** (אותה מגבלת build/dev-server) — נבדק בקריאה ישירה + בדיקת
      איזון-סוגריים/תגי-JSX (Python) על שני הקבצים שנערכו. נשאר לסבב שיגיע
      לדפדפן אמיתי: אימות חזותי של הקליפ בן 10 השניות + לולאת המוזיקה, יחד עם
      שאר הבדיקות שתועדו בסבבים 16-23.

      **[20/08/2026 Loop C, סבב 25] סנכרון כתוביות לפי תזמון אמיתי, לא קירוב** —
      תיקון פער אמיתי בליבת ה-#16/#17: הכתוביות העבריות שנצרבות בפריים
      (`buildCaptionSegments`) חולקו עד כה לפי **יחס אורך-תווים** של כל משפט
      מתוך משך הקריינות הכולל בלבד — כי `server/narration.ts` החזיר רק אודיו,
      לא תזמון. עבור עברית (אורך-מילה משתנה, הפסקות טבעיות) זה גרר כתוביות
      שמופיעות/נעלמות לפני/אחרי המילה שנאמרת בפועל. `server/narration.ts` (+
      `vercel-adapter`) עברו מנתיב ה-TTS הרגיל לנתיב `with-timestamps` של
      ElevenLabs (אותו ספק/מודל/`voice_settings` בדיוק — לא ספק חדש), שמחזיר
      גם `alignment` (תזמון אמיתי לכל תו: `characters[i]` נאמר בין
      `startTimes[i]` ל-`endTimes[i]`) לצד ה-`audio_base64`. `/api/ai/narration`
      מעביר את זה הלאה ב-JSON (`alignment`), אפס שינוי בצורת/בהתנהגות שאר
      התגובה. `shared/tts-hebrew.ts`: טיפוס משותף חדש `NarrationAlignment`
      (שימוש-חוזר בין server/client, בדיוק כמו `toTtsSafeHebrew` שכבר חי שם).
      `lib/videoExport.ts`: פונקציה חדשה `buildCaptionSegmentsFromAlignment` —
      מנרמלת את `alignment.characters` לאותה נורמליזציית-רווחים ש-
      `splitIntoCaptionChunks` מבצעת תוך שמירת מיפוי-אינדקסים חזרה לתווי
      ה-alignment המקוריים, כדי שכל מקטע-כתובית יקבל את זמן-ההתחלה/סיום
      האמיתי שלו (לא קירוב). `PromoVideoOptions.narrationAlignment` חדש;
      `exportPromoVideo` מנסה קודם alignment אמיתי ורק אם ריק/לא תואם (קריינות
      ישנה שנשמרה לפני השדרוג, כתוביות אנגלית מתורגמות שאין להן alignment
      משלהן, או כשל התאמת-טקסט תיאורטי) **נופל בדיוק לקירוב הישן** —
      `buildCaptionSegments` לא נמחקה/שונתה, רק כבר לא נתיב-ברירת-המחדל היחיד.
      `Editor.tsx`/`Projects.tsx`/`templateContext.tsx`: `narrationAlignment`
      נשמר בזיכרון ולצד `narrationScript`/`narrationAudioUrl` ב-`layersJson`
      (אותו דפוס תוספת-שדה בדיוק) כדי שפתיחת פרויקט שמור תשמר את הסנכרון
      המדויק, לא רק ייצוא באותו סשן. **אפס רגרסיה**: קריינות שנוצרה/נשמרה
      לפני הסבב הזה פשוט חסרת `alignment` ⇒ ממשיכה להתנהג זהה-ביט לקודם.
      **לא נבדק חי** (אותה מגבלת build/dev-server שתועדה בכל הפריטים למעלה) —
      נבדק בקריאה ישירה + בדיקת איזון-סוגריים/טיפוסים על ששת הקבצים שנערכו.
      נשאר לסבב שיגיע לדפדפן אמיתי: קריאה חיה ל-ElevenLabs עם `ELEVENLABS_API_KEY`
      אמיתי כדי לאמת שצורת ה-JSON של `with-timestamps` תואמת בדיוק למה שתועד
      (השדות נלקחו מתיעוד הספק, לא מתגובה חיה שנצפתה בסביבה הזו).
