...
# סבב 03/08/2026 (חמישי) — סריקת פלטפורמה מלאה

**`SYSTEMS_STATUS.md` נוצר** — טבלה מאומתת של 27 נתיבים.
**21 עובדות · 3 חלקיות · 2 שבורות · 1 מוגן-כניסה.**

## הממצא המרכזי: 200 אינו "עובד"
30 (`/crm`) ו-31 (`/gesher`) החזירו **200 מושלם והגישו עמוד ריק לגמרי**.
כל סקריפט/CSS החזיר 404; ה-SSR catch-all ענה בעמוד השגיאה של האפליקציה,
כלומר הדפדפן קיבל HTML במקום JS.

**שורש:** `base: '/crm/'` ב-vite משכתב את כתובות הנכסים ב-HTML, אבל Vercel
מפרסמת אותם ל-`/assets/` בלי הקידומת. נמדד: `/assets/x.js` → 200,
`/crm/assets/x.js` → 404, אותו קובץ.
**תיקון:** `vercel.json` בכל אחת עם rewrite. אומת: 6/6 נכסים 200, שגיאות 3→0.
**נשאר:** מסלול ה-index של שתיהן הוא `()=>null` — חוסר תוכן, לא תקלת פריסה.

## כלים חדשים
- `scripts/qa/probe-all.mjs` — 200 + כל נכס + טקסט מרונדר, לכל המערכות.
- `scripts/qa/probe-assets.mjs` — מאמת שכל נכס שנתיב מבקש באמת נפתר.
- `platform-audit.mjs` הורחב ל-kesef/kiosk/tivuch.

## עוד לתקן (פירוט ב-SYSTEMS_STATUS)
21 ו-24 — router לא מכיר את נתיב הבסיס (אותה משפחה כמו 30/31, תיקון קטן).
27 — `/mechiron/api/public/chatbot/config` 404.
15 — 18 יעדי מגע < 24px. 03 — שלושה.
מצב כהה חסר ב-13 מערכות.

---

# סבב 03/08/2026 (שישי) — סוף: חסם מכסת פריסות

## תוקן ואומת חי
- **30 + 31** (`/crm`, `/gesher`) — עמוד ריק מאחורי 200 → עובד. שורש: `base:'/crm/'` מול `/assets/` בלי קידומת.
- **21** (`/mthbram`) — `basename` חסר. 39 → **873** תווים.
- **24** (`/galil`) — `basename` + הוסר וידאו מת. 121 → **1,624** תווים.
- **15** (`/egod`) — יעדי מגע **18 → 0**.

**23/27 עובדות במלואן · 3 חלקיות · 0 שבורות.**

## 🔴 חסם: מכסת Vercel נגמרה
`api-deployments-free-per-day` — 100/יום, מתאפס בעוד 24 שעות.
**אי אפשר לפרוס ולכן אי אפשר לאמת.** שני תיקונים כתובים וממתינים:
`sites/36-nadlan-pro/tivuch/index.html` ו-`sites/34-kesef/kesef/index.html`
(`.footer a` → אזור מגע 24px).

## מלכודת שנפלתי בה — לתעד ולא לחזור
`Get-Content -Raw` ב-PS 5.1 קורא ANSI. קריאה של קובץ עברית UTF-8 וכתיבה עם
`-Encoding utf8` **מקודדת כפול**. ניסיון להפוך זאת דרך CP1252 (שאינו יודע
עברית) הפך כל תו ל-`?`. `TipsSection.tsx` נהרס ושוחזר מה-bundle החי.
**תמיד `[IO.File]::ReadAllText(path, UTF8)` + `WriteAllText` — לא Get/Set-Content.**

## הבא בתור (לפי הסדר)
1. איפוס מכסת Vercel → לפרוס את שני התיקונים הממתינים ולאמת.
2. `.gitignore` (§0א) — **12 קבצי מקור בפרודקשן ובשום ריפו.**
3. 9 יעדי מגע שנותרו — `node scripts/qa/small-targets.mjs`.
4. 27 — צ'אטבוט (דורש אימות מקור נתונים).
5. 30/31 — מסלול index ריק.
6. מצב כהה ב-13 מערכות.

---

# סבב 03/08/2026 (שביעי) — "ריצה מלאה": הצלת קבצים · כפתור הכניסה · מצב כהה

## שלב 1 — הצלת קבצים: הבעיה הייתה גדולה פי 130 ממה שהיה רשום

`NEEDS_USER` §0א מנה **21 קבצים** שחיים רק על הדיסק. הבדיקה בפועל מצאה
ש-`/apps/**` מוחרג ב-`.gitignore` ולכן **2,706 קבצי מקור** — כל קוד
המערכות, לא רק מה שנזכרנו לתקן — לא היו בשום ריפו.

**כולם נשמרו ונדחפו.** מה שהוחרג בכוונה: `.env*`, lockfiles,
`node_modules`, `dist`, `.next`, `.vercel`, `.output`, בינאריים מעל 3MB.

**מה שמנע דליפה:** כל קובץ נסרק לאישורי גישה **לפני** השמירה. נמצאו שישה
JWT של Supabase; כולם פוענחו (ולא הונחו) ואומתו כ-`role=anon` — מפתחות
שממילא נשלחים לדפדפן. אף `service_role` לא נשמר.

גם `_deploy/**` היה מוחרג, וה-API חסר-השרת שנכתב ביד תחת `_deploy/*/api/`
נשמר גם הוא — אותו סיכון בדיוק.

**מלכודת שנפלתי בה ותיקנתי:** `git add -f apps/32-nadlan-berega` על תיקייה
שלמה מבטל את ה-gitignore גם ל-`node_modules` שבתוכה. נתפס לפני ה-commit
(`git reset`), ומאז כל הוספה היא לפי רשימת קבצים מסוננת.

## שלב 3א — כפתור הכניסה: 10 מתוך 26 נתיבים הסתירו פקד אמיתי

לא סגנון — **פקדים שאי אפשר היה ללחוץ עליהם.** `elementFromPoint` אישר
בכל עשרת המקרים שהלחיצה מגיעה לכדור. הגרועים: `/chatzor/` (תפריט + מתג
מצב כהה במובייל) ו-`/torah` (כפתור ההתחברות של המערכת עצמה, בשני הרוחבים).

`auth-button.js` מפרסם `--more30-auth-inset` עם הרוחב שהוא באמת תופס,
נמדד מחדש כשהפונט נוחת, כשהסשן נפתר וב-resize. הפינוי בכל נווט **מחושב**:
הוא מקזז את השוליים שה-container כבר נותן, ולכן בדסקטופ שום דבר לא זז.

**נמדד אחרי פריסה: 6 נקיים · 4 ממתינים לפריסה · 1 חסום על מקור.**

**הפורטל דרש תיקון שני, וזה השיעור.** התיקון הראשון נפרס ועדיין נמדד
כמכוסה: ה-media query ב-720px מגדיר `padding-inline` על `.nav-in`, שכותב
את **שני** הצדדים ומוחק את `padding-inline-end` — בדיוק ברוחב שבו הפינוי
הכי נחוץ. ובנוסף, מתחת ל-`--wrap` איבר השוליים יוצא שלילי וחיסורו
**הוסיף** ריפוד: 124px הפכו ל-519px. שניהם היו נשארים בלי המדידה החוזרת.

## שלב 3ג — מצב כהה: ארבע בעיות שונות, לא אחת

`dark-probe.mjs` שאל את העמודים עצמם, וזה מה שהתברר:

| | |
|---|---|
| **04, 27** | ערכה כהה מלאה כבר בקוד, **אף אחד לא הדליק אותה**. סקריפט לפני-ציור, וזהו. |
| **15, 24** | `darkMode:["class"]` מוגדר, טוקנים סמנטיים בכל קומפוננטה — ואין ערכה שתענה. נכתבו, עם צבעי מותג קבועים והיפוך משטחים בלבד. AA נמדד: 16:1 · 7:1 · 8:1. |
| **30, 31** | ערכה קיימת, לא הודלקה. ב-SSR הסקריפט **חייב** להיות ב-`<head>` — `useEffect` מגיע אחרי שהעמוד כבר צויר לבן. |
| **32** | פלטה קבועה, אפס חוקי `.dark`. שוכתב לשכבת משטחים. |
| **21** | **לא חסר לה מצב כהה** — ה-`:root` שלה הוא ערכה כהה שנבנתה מהלוגו. הוצהר `color-scheme: dark`. |

**המלכודת ב-32, ושווה לזכור אותה:** `navy` שימש לשני תפקידים הפוכים —
147 פעם כדיו של כותרת ו-23 פעם כמשטח כהה. היפוך גורף היה הופך
`bg-navy text-white` ללבן-על-לבן. פוצל: `navy` הוא הדיו ומתהפך,
`navysurface` הוא צבע המותג ונשאר. מאותה סיבה `bg-white` הפך ל-`bg-surface`
בעוד `text-white` נשאר מילולי — הוא יושב על נייבי ועל זהב, שאינם זזים.
`next build` ו-`tsc --noEmit` עוברים.

## שלב 2 — שני באגים אמיתיים שנמצאו במדידה

**zchuyot: 9 שגיאות קונסולה.** `console-probe` מול הפרודקשן: שמונה
`#418` ואחריהן `#423` — ההידרציה נכשלה, React זרק את השורש כולו וצייר
מחדש בכל מקרה. זה בדיוק מה ש-`prerender-spa.mjs` מתעד: `--block-data`
מעביר את react-query ל-*error* ולא ל-*pending*, ולכן הלכידה והלקוח מציגים
שני ענפים שונים. ההידרציה לא נתנה כלום ועלתה תשע שגיאות. חזרה
ל-`createRoot`, נאפה מחדש בלי `--block-data`.

**mechiron: ה-404 של הצ'אטבוט.** המסלול עונה עכשיו — אבל **מדווח,
לא מפעיל**. הוא קורא `automation_configs` במפתח `public_chatbot` דרך אותם
אישורי Supabase שמסלול ההגדרות כבר משתמש בהם (זו האימות של מקור הנתונים
שביקשת), ומחזיר `enabled:false` כשהשורה חסרה או לא קריאה. `/answer`
ו-`/lead` **לא** מומשו: שניהם דורשים את מאגר הזכויות ואת צינור הלידים,
וצ'אטבוט שנפתח ולא יודע לענות גרוע מאחד שלא נפתח.

## שלב 4 — ניקוי

- פרויקט ה-Vercel היתום `dist` **נמחק**, אחרי אימות שאין לו דומיין ושאף
  rewrite אינו מפנה אליו.
- **פרצת ה-anon key ב-`csjekrvu` — נבדק שוב, ועדיין חסום עליך.**
  `list_projects` מחזיר פרויקט אחד בלבד (ההאב). `csjekrvu` בחשבון אחר,
  ולא ניתן להגיע אליו מכאן. נשאר ב-NEEDS_USER §2 עם מה שצריך בדיוק.

## 🔴 החסם: מכסת Vercel נגמרה שוב אחרי 7 פריסות

`probe-all.mjs`: **27/27 נתיבים 200, אפס מסכים ריקים.**
11 פריסות ממתינות — הכול בנוי ומוכן ב-staging. הרשימה המדויקת ופקודות
האימות שאחריה: `NEEDS_USER.md` §0ג.

## הבא בתור
1. איפוס המכסה → 11 הפריסות הממתינות → שלוש בדיקות האימות.
2. מצב כהה ל-5 הנותרות (02, 03, 06, 10, 35) — אותו שכתוב כמו ב-32.
3. רינדור מראש ל-22, 27, 28 (01, 15, 16 כבר נאפו) → Lighthouse ≥ 90.
4. `/smachot` — מקור האתר החי אינו על הדיסק. חוסם יעד מגע וחפיפת כניסה.
## תוספת לסבב השביעי — סריקת הפלטפורמה המלאה

`platform-audit.mjs` רץ על 30 נתיבים × 3 תצוגות (דסקטופ/מובייל/כהה) עם
צילומי מסך. **כל 30 החזירו 200.** מה שהוא מצא מעבר לזה:

**שני מקרים של סחף בין המקור לפרודקשן — וזה הממצא החשוב.**
`apps/06-kupot-holim/site/index.html` **אינו** מכיל את `<base href="/briut/">`
שהעמוד החי כן מכיל. כל נכס שם הוא נתיב יחסי, ולכן העתקה מהמקור לתיקיית
הפריסה הייתה מפילה כל CSS/JS/JSON ל-404 ומגישה **עמוד ריק מאחורי 200** —
בדיוק התקלה של 30 ו-31. אותו דבר, בקטן, ב-`bkalot`: `auth-button.js` חי רק
בעותק הפרוס. שניהם הוחזרו למקור. **נמצא בהשוואת שורות לפני שהעתקתי משהו,
ולא בקריאה** — וזה השיעור: לכל אתר סטטי שמתוחזק ביד, להשוות מול החי קודם.

**נגישות:** galil (טלפון 79×14 ואימייל 157×18 בפוטר) · galil ו-mthbram
(כפתור התפריט במובייל בלי שם נגיש — קורא מסך אומר "לחצן" ועוצר) · bkalot
(ה-`<select>` של ההשוואה בלי תווית) · crm ("שכחתי סיסמה" 75×16). כולם
תוקנו. smachot ו-kiosk נשארו — המקור שלהם אינו כאן.

**באג שנתפס לפני שנפרס.** ב-nadlan הטוקנים החדשים החזיקו hex, ולכן Tailwind
לא יכול היה להחיל עליהם מודיפייר שקיפות והשמיט **36 מחלקות בשקט** —
`border-line/70`, `bg-navy/10`, `text-muted/70` וכו'. הערכים הם עכשיו ערוצי
RGB וה-config עוטף אותם ב-`<alpha-value>`; אומת שהמחלקות באמת מופיעות
ב-CSS הבנוי. ב-tamlul אותו דבר נכשל **בקול** — `@apply border-brand/20`
שבר את הבנייה — וזה מה שגילה את השקט ב-nadlan.

**מצב כהה ל-tamlul (02)** נכתב באותה שיטה: הכחול מתפצל ל-`brand` (דיו,
מתהפך) ול-`brandsurface` (מותג, קבוע). `next build` עובר.

---

# סבב 03/08/2026 (שמיני) — "ריצה כוללת" שלב A: פרסתי, מדדתי, ומצאתי עוד

## המכסה התאפסה — 11 פריסות עברו, וכולן אומתו

| אימות | תוצאה |
|---|---|
| `console-probe /zchuyot` | **9 שגיאות → 0** |
| `console-probe /mechiron` | **404 → 0** |
| `authbutton-overlap` | **20/26 נקיים** (בבוקר: 10 נתיבים הסתירו פקד אמיתי) |
| `dark-probe` | ✅ imud · egod · galil · mechiron · nadlan · crm · gesher · tamlul · zchuyot |

## ארבעה דברים שהמדידה-שאחרי גילתה, ולא הייתי מוצא בלעדיה

**1. bkalot לא הייתה צריכה ערכה כהה — כבר הייתה לה.** הסורק דיווח "אפס
חוקי dark", אבל הרקע כן התהפך ל-`rgb(20,19,15)` בכפייה. הסיבה:
`sheetsUnreadable: 2` — הערכה מגיעה בגיליון הטוקנים הגלובלי של בקלות,
שהוא חוצה-מקורות ולכן `cssRules` שלו אינו קריא מסקריפט. חסר היה רק מי
שידליק אותה.

**2. הסורק עצמו טעה לגבי galil.** הוא מטפס מהגוף ומחפש `backgroundColor`
אטום, ו-galil צובע את העמוד ב**גרדיאנט** — ולכן כל האבות נשארו שקופים
והוא דיווח "no change" על ערכה שמתהפכת מצוין (נמדד ידנית: תמונת הרקע
עברה מ-`rgb(245,247,249)` ל-`rgb(14,24,32)`). **תיקנתי את הסורק** לקרוא
גם `background-image`, כדי שהשלילי-כוזב הזה לא יחזור.

**3. mthbram — ה-`aria-label` שהוספתי הוא מה שהוכיח שהפריסה עבדה.** אחרי
הפריסה הדוח כבר קרא לכפתור בשמו ("פתיחת התפריט") במקום `button""`. אבל
המערכת מעולם לא קיבלה את הפינוי עצמו. נוסף.

**4. mechiron הייתה בלי כפתור כניסה בכלל** ("no pill" בשני הרוחבים). נוסף
— תוספת תצוגה בלבד שנפרסת רק ל-`mechiron-more30` ואינה נוגעת בשרת המוגן.

## רינדור מראש — ו**סירוב** לאפות עמוד אחד

**mechiron נאפתה מצוין:** 2,976 תווים של קופי שיווקי אמיתי, כולו סטטי.

**kupot לא, וזה שווה תיעוד.** לדפדפן הלוכד אין API, ולכן העמוד רינדר
*"0 נושאי קופות חולים · 0 זכויות ממשלה · לא נמצאו תוצאות"* — וזה מה שהיה
נשלח כציור הראשון, וזה מה שסורק היה מאנדקס. **מסך ריק עדיף על עמוד
שמצהיר בביטחון שאין כאן כלום.** בוטל.

הסף הקיים (>200 תווים) אינו תופס את זה — למצב ריק יש שפע תווים. הוספתי
ל-`prerender-spa.mjs` בדיקה שמחפשת את **הצורה** של מצב ריק (אפסים חוזרים
או ניסוח "לא נמצאו") ויוצאת בשגיאה במקום לכתוב את הקובץ. ארבע האפיות
הקיימות (torah, egod, chatzor, zchuyot) נבדקו מולה — כולן נקיות.

*(kupot ממילא לא הייתה מועמדת נכונה: ה-FCP שלה 2.7 שניות, והקפיצה
40→72 באה מתיקון קיפאון רינדור ולא ממשהו שרינדור מראש היה פותר.)*

## מצב כהה — 12 מתוך 13 סגורות

נותרה **35 (kiosk)** בלבד, והמקור שלה vendored ב-Railway.

**briut (06)** הייתה מטוקנת במלואה, אבל עם שתי מלכודות מאותה משפחה כמו
ה-navy של nadlan: `--primary` צבע 15 טקסטים **וגם** ישב מתחת ללבן על
כפתור הפעולה, והפוטר צבע את הרקע שלו ב-`--text`. שניהם פוצלו. בדרך נמצא
שלבן על `--primary` המקורי נמדד **4.03:1** — מתחת ל-AA, על כפתור הפעולה
הראשי של עמוד לכידת לידים. הועבר ל-`#007B87` (5.0:1), גוון שכבר היה בין
השניים שבפלטה.

**modaot (03)** היא הגדולה — 66 מחלקות צבע נבדלות כולל שבבי סטטוס.
אותה שיטה. **בנוי ועובר `next build`, ובכוונה לא נפרס:** זו igud-ads עם
סליקה חיה, ויש שם בקשת אישור פתוחה ממך.

## 🔴 המכסה נגמרה שוב אחרי 11
5 תיקיות בנויות וממתינות + `modaot` שממתין לאישורך. `NEEDS_USER.md` §0ג.
## Lighthouse אחרי הפריסות — הרינדור מראש עשה את שלו, והתקרה זזה

| נתיב | perf | FCP | LCP | TBT | CLS |
|---|---|---|---|---|---|
| torah | 47 | **1.6s** ✅ | 4.4s | 470ms | 🔴 **0.578** |
| chatzor | 59 | **1.5s** ✅ | 5.1s | 920ms | 0.005 |
| egod | 59 | 2.6s | 5.6s | 560ms | 0.025 |
| zchuyot | 49 | 2.6s | 6.9s | 1,090ms | 0.017 |
| nadlan | 79 | 2.5s | 3.0s | — | — |

**ה-FCP נפתר בדיוק כפי שהובטח** — torah ו-chatzor מתחת לסף 1.8 שניות.
**אבל ≥90 עדיין לא מושג, והחסם עכשיו הוא LCP ו-TBT ולא FCP.** זו בעיה
אחרת מזו שאופציה א' אושרה לפתור, והיא רשומה ככזו ולא כאותו פער.

## 🔴 רגרסיה שגרמתי לה — CLS של torah מ-0.001 ל-0.578

`cls-why.mjs` הצביע על אלמנט אחד: "מערכות נוספות מבית איגוד השיעורים".
הסעיף שמעליו הוא רשת יכולות שהכרטיסים בה מותנים בשורת הטננט.

**השורש:** torah נאפית **עם seed** — שורת הטננט נשלפת בבנייה ונכתבת
ל-HTML כ-`window.__TENANT__`. בבנייה מחדש הבוקר הרצתי את
`prerender-spa.mjs` **בלי `--seed-url`**. שום דבר לא נכשל והעמוד נראה
תקין; ה-HTML האפוי צייר את הרשת המלאה, הרינדור הראשון בלקוח היה בלי
טננט וצייר פחות כרטיסים, וכל מה שמתחת קפץ.

**דגל שצריך לזכור הוא דגל ששוכחים.** הדגלים עברו ל-
`scripts/prerender-all.mjs`, לצד המסלול, והוא **מסרב לאפות את torah**
אם לא הצליח לבנות את ה-seed. כתובת ה-seed נבנית בזמן ריצה מה-`.env.local`
של האפליקציה עצמה, ולכן שום אישור גישה אינו נשמר בסקריפט.

## שני כלי אבחון חדשים
`perf 47` לבדו אינו ניתן לפעולה. `lcp-why.mjs` נותן את אלמנט ה-LCP,
פיצול הפאזות והמשימות הארוכות; `cls-why.mjs` נותן את האלמנטים שקופצים.
שניהם קוראים את הדוח ש-`lh-batch` כבר כותב.

**ממצא צדדי:** `auth-button.js` עולה **253ms** של זמן מעבד ראשי ב-zchuyot.
הוא נטען בכל 24 המערכות, ולכן זו הזדמנות רוחבית.

---

# סבב 03/08/2026 (תשיעי) — פריסות, רגרסיה שנסגרה, ופרצת סודות שנפתחה ונסגרה

## 🔴 הממצא החמור ביותר של היום: אישורי סליקה חיים בריפו ציבורי
...
קומיט ההצלה שלי מ-03/08 הוסיף בכוח את כל `apps/`, **והריפו ציבורי** (אומת
מול ה-API של GitHub, לא הונח). כלומר הם פורסמו.

**למה הסריקה שלי לפני אותו קומיט לא תפסה:** היא חיפשה **קידומות** של
טוקנים (JWT, `sk-`, `ghp_`, `sbp_`) ואלה מחרוזות קצרות בלי קידומת. וגם לפי
שם היא לא הייתה תופסת, כי הערך יושב אחרי `process.env.X ||` ולא ישר אחרי
הנקודתיים — ותבנית מעוגנת פשוט עוברת לידו. **ערך ברירת מחדל הוא מקום גרוע
יותר לאישור גישה מאשר השמה רגילה**, כי הוא נקרא כאילו הערך הגיע מהסביבה.

**סדר הפעולות היה קריטי:** ל-`modaot-more30` **לא היו** משתני `NEDARIM_*`
בכלל, כלומר הסליקה החיה רצה כולה על אותם ליטרלים. מחיקה שלהם קודם הייתה
שוברת חיוב. הוגדרו ב-Vercel, אומת שנקלטו, ורק אז הוסרו מהמקור. הקונפיג
זורק עכשיו לפי שם אם משתנה חסר, במקום לחייב לפי ניחוש.

`scripts/qa/secret-scan.mjs` הוא הגרסה הכללית: הוא מחפש **שמות** שנראים
כמו סוד שמקבלים ליטרל, כולל דרך `||`, ומפענח JWT של Supabase כדי
ש-19 מפתחות ה-anon שבריפו ידווחו כ"פומביים בכוונה" במקום להטביע את
הממצא האמיתי. ירד ל-6 ממצאים, חמישה מהם false positive שקראתי ואני יכול
לנקוב בשמם.

**דורש אותך: לסובב את שני האישורים מול נדרים פלוס.** מחיקה מעץ העבודה
אינה מוחקת מהיסטוריית גיט, וריפו ציבורי יש להניח שנקרא.

## הרגרסיה של torah נסגרה ואומתה
**CLS 0.578 → 0 · perf 47 → 62.** הדגל שנשמט הוחזר, והדגלים עברו ל-
`scripts/prerender-all.mjs` שמסרב לאפות בלי seed.

## "נקי" לא היה אותו דבר כמו "תוקן"
`authbutton-overlap` דיווח 20/26 נקיים — וזו התוצאה שחשובה, אבל היא אינה
מבחינה בין נווט שפינה מקום לבין נווט שהפקד האחרון שלו פשוט יושב במקום אחר.
`scripts/qa/authbutton-applied.mjs` בודק את הערך המחושב על האלמנט עצמו.
**9 מתוך 11 באמת פינו 98px. שניים לא, ומשתי סיבות שונות:**

- **mthbram**: ה-CSS של המערכת מכיל `.container{padding-left:1rem!important}`.
  `!important` בגיליון מחבר גובר על סגנון inline רגיל, ולכן ה-`style` של
  React נבלע בשקט — ערך מחושב 16px, ואפילו כפייה של 150px מהקונסולה לא זזה.
  זה מה שהפך את זה לוודאי ולא למשוער. הפינוי עבר לפקד עצמו. **נפרס ואומת:
  נקי בשני הרוחבים, 82px מרווח בפועל.**
- **bkalot**: חפיפה של **פיקסל אחד** (39×1), כי ל-`.header-inner` יש
  `height:64px` קבוע — כשהתוכן לא נכנס הוא גולש מתחת לגובה במקום להגביה את
  הסרגל. `min-height` + `flex-wrap`. ממתין לפריסה.

## לוח "משתמשים פעילים" לסופר-אדמין
`more30_admin_activity()` — סופר-אדמין בלבד (אותו `42501` כמו שאר ה-RPC-ים
הניהוליים). מוגש מהפורטל ב-`/admin/activity`, **ובכוונה לא מ-
`_deploy/nihul-more30`**: NEEDS_USER §4 מתעד שהתיקייה הזו מיושנת ופריסה
ממנה כבר החזירה 404 על מרכז השליטה.

**מה שהוא מסרב לנחש:** ההבחנה "מי בהאב" נקראת מ-
`core.projects.supabase_project` ולא מרשימה בקוד, כדי שמערכת שתעבור להאב
תיספר לבד. וזה שינה את התמונה: הבריף נקב בחמש מערכות שאינן בהאב,
**והתשובה הנמדדת היא 21 מתוך 28**. הן מקבלות טבלה משלהן עם הפרויקט שבו
המשתמשים שלהן באמת יושבים.

מספרים אמיתיים: **19 משתמשים רשומים · 7 פעילים ב-30 יום · 9 שורות חברות ·
7 מערכות בהאב · 21 מחוצה לו.**

## סליקה — שער מחיר, לא סליקה
`more30_subscribe` כבר לא גבה כסף (מחזיר `charged=false`). מה שנוסף הוא
**מניעה**: לכל חמשת המסלולים ב-`core.plans` יש `price_ils` ריק, ולכן
התשובה מחזירה `price_missing: true` בשדה נפרד — כדי שגם מסך וגם קוד יוכלו
לבדוק אותו לפני שמישהו פותח חלון תשלום בסכום שאינו קיים.

## נפרסו ואומתו היום
`torah` · `mthbram` · `bkalot` · `briut` · ועוד 11 מהסבב הקודם.
**המכסה נגמרה שוב.** ממתינים: `portal` (הלוח החדש), `bkalot` (תיקון
הפיקסל), `mechiron`, `modaot`, `chizukim`.
## סיום הסבב — כל התור נפרס ואומת

**24 פריסות היום.** `portal` · `torah` · `mthbram` · `bkalot` · `briut` ·
`mechiron` · `modaot` · `chizukim` — ועוד 16 מהסבבים הקודמים.

### הפריסה הרגישה: modaot (סליקה חיה)
`scripts/qa/modaot-payment-smoke.mjs` נכתב **לפני** הפריסה כדי שתהיה נקודת
השוואה, ורץ שוב אחריה. **חמש תוצאות זהות בשני הצדדים.** הפריסה שינתה
ערכת צבע והוסיפה את כפתור הכניסה, ולא נגעה במסלולי התשלום.

**ממצא נלווה שסומן ולא הופעל:** `/modaot/api/payments/create` מחזיר 404
בפרודקשן **גם** כשהמסלול קיים בבנייה. כלומר זה מבני — לא בנייה מיושנת.

### שלוש חפיפות שנשארו, כולן נגיעות של פיקסלים בודדים
- `/modaot` — **54×4 → 3×4**. הפינוי חל; הנווט רחב ממה שנשאר.
- `/mechiron` — לא זז. `/mechiron` מרנדר משהו אחר מ-`public-landing.tsx`.
- `/bkalot` — **39×1, והמדידה סותרת את הקוד.** ה-`style.css` שנפרס כן מכיל
  את `--more30-auth-inset`, ובכל זאת הריפוד המחושב 0, ה-`min-height` 0,
  גובה הסרגל 94px במקום 64, ו-`.nav` מחושב `display:block` בעוד המקור
  אומר `flex`. **גיליון אחר מנצח, ולא זיהיתי אותו.** רשום עם המספרים.

### לוח המשתמשים חי
`more30.com/admin/activity` — 200, מגודר לסופר-אדמין.

---

# סבב 04/08/2026 — התעלומה נפתרה, ומצב כהה הושלם

## 🔴 `/bkalot` רץ בפרודקשן **בלי גיליון סגנון ובלי JavaScript**

זה הממצא החמור ביותר בסבב, והחפיפה של פיקסל אחד הייתה רק תסמין שלו.

`css-origin.mjs` (כלי חדש) שאל את העמוד מי מנצח על
`padding-inline-end` ב-`.header-inner`, וקיבל תשובה לא צפויה: **אפס
הצהרות תואמות**, ורשימת הגיליונות הראתה `more30.com/style.css` — השורש,
לא `/bkalot/` — עם **אפס חוקים**.

**השורש:** האתר מוגש מ-`more30.com/bkalot`, בלי לוכסן בסוף ובלי `<base>`,
וכל הנכסים יחסיים. `"style.css"` נפתר מול כתובת המסמך → שורש הפורטל →
ה-catch-all עונה **200 עם index.html**.

| הנכס | מה חזר |
|---|---|
| `style.css` | HTML → אפס חוקים |
| `engine.js` · `repo.js` · `app.js` | HTML → **מעולם לא רצו** |
| 30 קישורי הורדה | לשום מקום |

**מנוע הזכויות, השאלון והשוואת הקופות היו מתים.** העמוד נראה חי רק כי
`bkalot-theme.css` הוא כתובת מוחלטת חוצת-מקורות והטקסט הסטטי ב-HTML.

**שום דבר לא החזיר 404**, ולכן `probe-assets` עבר על זה שוב ושוב.

**התיקון:** `<base href="/bkalot/">` — אותה שורה שיש ל-`/briut`.
**אומת:** חמישה נכסים אמיתיים, והחפיפה נפתרה מעצמה — חוק ה-CSS היה נכון
כל הזמן, הוא פשוט לא נטען.

**שני כלים חדשים** כי הקיימים עיוורים לזה: `css-origin.mjs` ו-
`relative-asset-probe.mjs`. **נסרקו כל 22 הנתיבים האחרים — bkalot היה היחיד.**

## `/mechiron` ו-`/modaot` — לא קסקדה, אלא flexbox

`min-width:auto` הוא ברירת המחדל של פריט flex, ולכן הנווט **סירב
להתכווץ** מתחת לרוחב התוכן שלו: הריפוד שהפניתי דחף את כל השורה מעבר
לחלון (נמדד `x14..404` בחלון 390) במקום לכווץ אותה. `min-w-0` +
שורה משלו במובייל. **שניהם נקיים.**

## מצב כהה — 13/13, הושלם
`kiosk` (35) היה רשום כחסום כי המקור אינו על הדיסק. הוא לא חסום:
`app.json` נוקב בריפו, בענף ובתיקייה, ולטוקן יש גישה. נשלף, נכתב, נדחף,
ו-Railway פרס אוטומטית. **נמדד: `rgb(245,247,251) → rgb(11,18,32)`.**

## שני חסמים ב-NEEDS_USER שנסגרו מעצמם
ה-Volume של הקיוסק **מחובר**, ולוג העלייה אומר בדיוק את המשפט שחיפשנו:
`db: /app/data/kioskfleet.db — existing file (data persisted)`.
כלומר הנתונים שרדו דיפלוי בפועל — וזה גם משחרר את החלפת סיסמת האדמין.

## מדידות סיכום
- **כפתור הכניסה: 24/25 נקיים.** נשאר `smachot` בלבד (מקור חסום).
- **`probe-all`: 27/27 מגישים**, אפס נכסים שבורים.
- **`secret-scan`: אפס ממצאים** (20 מפתחות anon מסוננים, 3 מוכרים-שפירים).
- **Lighthouse `/torah`: 47 → 62 → 82.** LCP 4.9s → 3.4s.

> ⚠️ **על שונות המדידה:** `chatzor` נמדד 59 בבוקר ו-49 אחה"צ עם FCP
> 1.5s מול 3.6s — **בלי שהקוד שלו השתנה.** הרשת כאן מוסיפה רעש אמיתי,
> ולכן ציון בודד אינו ראיה. torah השתפר על פני שלוש מדידות נפרדות.

---

# לילה 04/08/2026 — מסמכי הבנייה: אדמין, מחירים, showcase, ותור האוטומציה

## more30adminbuild §2 — שליטת מחירים ✅ חי

`more30.com/admin/pricing`. הבעיה שהייתה צריכה הכרעה לפני שאפשר לבנות:
ל-`core.plans` היה `price_ils` **אחד בלי סוג**, ולכן "שמור 30 ₪" הייתה
הוראה דו-משמעית. נוספה `billing_kind` (מנוי / סליקה / חד-פעמי), ונזרעו
שני הסוגים החסרים ל-29 המערכות + מסלול **"פרו (בדיקה)" ב-1 ₪** כפי
שהמסמך מבקש.

**למחיר יש עכשיו שלושה מצבים, לא שניים:**
...
"ברירת מחדל 0" שבמסמך היא **החלטה**, ולכן 0 ולא null. בעקבות זה
`more30_subscribe` נשען עכשיו על `chargeable` (`price > 0`) ולא על
"לא null" — אחרת מי שיחבר ספק סליקה בעתיד היה פותח חלון תשלום על אפס.

הכל דרך RPC של סופר-אדמין: השער נבדק **בשני הכיוונים** (42501 לזר,
נתונים אמיתיים לאדמין), מחיר שלילי נחסם בפונקציה ולא רק בטופס, וכל
שינוי כותב before/after ל-`core.admin_audit` החדש.

## §4 — "הצג בעולם הסטארטאפים" ✅ חי
מתג לכל מערכת + אתר `more30.com/showcase`. **ברירת המחדל היא מוסתר** —
קטלוג שמצרף מערכות אוטומטית היה מפרסם עבודה בתהליך למשקיעים ברגע
שנוספת שורה. `more30_showcase()` פומבי בכוונה (הדף ציבורי), ומה שמגן
עליו הוא **מה שהוא מחזיר**: שדות שיווקיים בלבד, ורק מערכות שנבחרו.

## §3 — דוח לכל מערכת ✅ חי
`more30.com/admin/systems` — כרטיס לכל מערכת: סטטוס, משתמשים, מנויים,
מחירים, הכנסה משוערת. **28 מערכות · 18 חיות · 7 עם משתמשים בהאב.**
הכלל שנשמר: מערכת שמאגר המשתמשים שלה מחוץ להאב **אינה מקבלת 0** (מספר
שקרי) אלא `users_counted:false` ואת שם הפרויקט האמיתי — זה המצב של 21
מתוך 28.

## §1 — כניסת לקוח: נמדד לפני שנבנה
המסמך מבקש שההתחברות תחזיר את הלקוח **למערכת עצמה**. במקום להניח שצריך
לבנות — נמדד: **15/15 עברו.** הכדור המשותף בכל 12 המערכות שנבדקו נושא
`from` שחוזר לאותו נתיב, ו-`/login` שומר עליו. החלק הזה כבר עבד.

מה שכן נבנה: **`more30.com/admin/customers`** — צירוף לקוח למערכת.
**בלי service_role בשום מקום:** החשבון נוצר דרך ההרשמה הרגילה (anon
מספיק, אישור אימייל כבוי), וה-RPC רק מעניק את החברות. מפתח master בדף
שהדפדפן מגיע אליו הוא משטח תקיפה שלא צריך להיפתח כדי לענות על הדרישה.

> באג שנמנע: `signUp` מחליף את הסשן הפעיל — בלי לקוח Supabase שני עם
> `persistSession:false`, האדמין היה מוצא את עצמו מחובר **כלקוח שהרגע יצר**.

## BKALOT_AUTOMATION_BUILD §8 — התור העצמאי ✅ נבנה
סכימה `bkalot_auto`: `topics` (נושא לפי מספר), `contacts`,
`outbound_queue` (מצב/תזמון/ניסיונות), `delivery_log`, ו-`test_targets`.

**מצב בטוח הוא תכונה של המסד, לא של הקוד:**

| שער | התנהגות |
|---|---|
| `mode='live'` | **זורק 42501.** אין דגל להפוך בקוד; הפעלה אמיתית היא מיגרציה מכוונת |
| נמען בלי הסכמה | לא נכנס לתור כלל |
| יעד שאינו ברשימת הבדיקה | נכנס כ-**`blocked`** עם הסיבה — הניסיון נראה ולא נעלם בשקט |

**אומת מקצה לקצה** על שלושה נמענים: המאושר נכנס לתור, כתובת אמיתית
לכאורה חזרה `blocked`, מי שאין לו הסכמה נדחה, ו-`live` נחסם.
`queue_due` מחזיר **שורה אחת** — המאושרת. `test_targets` ריק פירושו
ששום דבר לא נשלח, וזו ההתנהגות הנכונה.

הסכימה בהאב ולא ב-`csjekrvu`: אין לנו שם דשבורד, ופרצת ה-RLS שם עדיין פתוחה.

## מה שלא נבנה, ובכוונה
**יצירת התוכן והסוכן** מ-BKALOT_AUTOMATION_BUILD §4/§9 — התור מוכן
לקבל אותם, אבל הם לא נבנו הלילה. חצי-בניית מנוע ששולח ללקוחות אמיתיים
היא בדיוק הסיכון שהמסמך מזהיר מפניו.
## תוספת — ניהול האוטומציה בכפתורים (§7)

`more30.com/admin/automation`: נושאים לפי מספר, **תצוגה מקדימה של המייל
בדיוק כפי שהלקוח היה מקבל אותו** (נבנה מ-`rights.catalog` בזמן אמת),
והתור עם המצב והסיבה לכל שורה. תצוגה מקדימה אינה שולחת דבר.

המייל מוצג ב-`iframe` עם `srcdoc` ולא ב-`innerHTML`: הוא מסמך שלם עם
סגנונות משלו, והזרקתו לעמוד הייתה מערבבת אותו עם עיצוב האדמין.

`topic_upsert` **מסרב** לקטגוריה שאין מאחוריה זכויות — בנקודה שבה עוד
אפשר לתקן, ולא בזמן השליחה.

**החלטה שנמנעה בכוונה:** `bkalot_auto` אינה ב-exposed schemas של
PostgREST, והמהלך המתבקש היה להוסיף אותה. `NEEDS_USER` §0א′.3 מתעד
כמעט-תקלה שבה כתיבה נאיבית לרשימה הזו כמעט מחקה את `nadlan` ואת
`chatzor` והפילה שתי מערכות חיות. ארבע עטיפות ב-`public` עלו שלוש שורות
כל אחת ולא נגעו בשום הגדרה גלובלית. כולן מחזירות 401 ל-anon.

**גילוי:** ארבעת מסכי הניהול לא היו נגישים משום מקום — `/admin` הוא
מעטפת SPA בת 815 בתים בלי שום קישור פנימי. הם נוספו לתפריט כפתור הכניסה
לסופר-אדמין, שנטען בכל 24 המערכות; כלומר הם נגישים מכל מקום בלי לגעת
באפליקציית הניהול החיה שאין לי את המקור שלה. הכפתור נבדק אחרי — נקי.
## תוספת אחרונה — ה-worker היבש, ורגרסיה שגרמתי לה ותיקנתי

### ה-worker: השרשרת אומתה מקצה לקצה בלי לשלוח
§10.4 מבקש שולח; §5 אוסר לשלוח בלי אישור. מה שנבנה הוא **הצעד שלפני**:
ה-worker עובר על התור, בונה את התוכן האמיתי לכל שורה, **בודק שוב** שהיעד
עדיין מאושר, ורושם ביומן מה היה נשלח.

**הסטטוס שנכתב הוא `skipped` ולא `sent`** — שום דבר לא נשלח, ולסמן
`sent` היה שקר שהיומן היה נושא לנצח. ה-RPC גם מחזיר `sent_for_real: 0`
כשדה מפורש, כדי שהמסך יצהיר על זה ולא ירמוז.

**הבדיקה החוזרת בזמן העיבוד היא לא כפילות:** הוסרתי יעד מרשימת הבדיקה
*אחרי* שהשורה נכנסה לתור, והשורה חזרה `blocked` עם "היעד הוסר מרשימת
הבדיקה לפני העיבוד" — במקום להישלח לכתובת שאושרה לפני שעה.

**מצב סופי:** 2 blocked · 2 אומתו יבש · **0 נשלחו.**

### 🔴 רגרסיה שגרמתי לה: מסלולי ניהול הופיעו ללקוח
זריעת `charge`/`one_time`/`pro` ל-`core.plans` הכניסה אותם ל-
`more30_system_page`, כלומר **לעמוד המסלולים שהלקוח רואה**. מבקר ב-
`/subscribe` היה רואה **"פרו (בדיקה) — לשנות בניהול לפני שיווק"** כמסלול
לרכישה.

**השורש:** טבלה אחת החזיקה שני סוגי שורות לשני קהלים — מדרגות שיווקיות
שהלקוח בוחר מהן, וסוגי תשלום שהאדמין מגדיר — ושום דבר לא הבחין ביניהם.
נוספה `customer_visible`. הלקוח רואה 3 מדרגות, האדמין עדיין רואה 6.

בדרך תוקן גם `₪0` בעמוד הלקוח: אחרי שהוחלט ש-0 פירושו "ללא חיוב", שלושת
המצבים מקבלים שלושה ניסוחים שונים.

`scripts/qa/pricing-reflects.mjs` בודק את **שני חצאי** ההבטחה שבמסמך:
שהמחיר מגיע ללקוח, ושמסלולי הניהול לעולם לא. 10/10 על חמש מערכות.

*(גם הבדיקה עצמה נולדה עם באג — היא הדפיסה "‎-1 priced" כי `Number(null)`
הוא 0 ושורות "טרם נקבע" נספרו גם כחינמיות. בדיוק אותו בלבול שהשינוי הזה
בא להסיר.)*