...
## מצב כהה — פקד ידני חסר (5 מערכות)

**מה חסר.** ל-02 (תמלול), 03 (מודעות), 06 (בריאות), 10 (בקלות-תצוגה), 35
(קיוסק) אין פקד גלוי בעמוד שמאפשר למי שה-OS שלו **בהיר** לבחור כהה בכל זאת
(ל-smel, chatzor, galil יש כזה פקד). **זו לא פונקציונליות חסרה** — כל חמש
עוקבות אחרי `prefers-color-scheme: dark` בפועל, כלומר מי שה-OS שלו כהה מקבל
עמוד כהה בלי שום פעולה. נמדד ונאושר מחדש 17/08:
