...
interface DashboardPayload {
  ...
  debts: Array<{ id: number; creditor: string; kind: string; currentBalance: number; monthlyPayment?: number | null; status: string }>;
  goals: Array<{ id: number; title: string; targetAmount: number; savedAmount: number; targetDate?: string; status: string }>;
...
function DebtsTab({ data }: { data: DashboardPayload }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <Card className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-muted/40 text-right"><tr><th className="py-2 px-3">נושה</th><th className="py-2 px-3">סוג</th><th className="py-2 px-3">יתרה</th><th className="py-2 px-3">תשלום חודשי</th><th className="py-2 px-3">סטטוס</th></tr></thead>
          <tbody>
            {data.debts.length === 0 ? <tr><td colSpan={5} className="py-6 text-center text-muted-foreground">אין חובות רשומים.</td></tr> :
              data.debts.map((d) => (
