...
מאז המיגרציה הראשונה (0009), ו-`np_contact_save` (יצירה+עדכון) ו-
`np_contact_get` (קריאה, `to_jsonb(c)` מלא) כבר תומכים בשתיהן במלואן — אבל
`contactForm()` (טופס יצירה/עריכה, `sites/36-nadlan-pro/tivuch/app.html`)
מעולם לא הציג שדה קלט לאף אחת מהן, וגם לא נכללו בלולאת בניית ה-payload
שנשלחת ל-RPC. `openContact()` (מגירת הפרטים) גם לא הציגה אותן ב-`.facts`,
למרות ש-`np_contact_get` כבר מחזיר את שתיהן. `grep` על `id_number` בקובץ
המלא החזיר אפס תוצאות לפני הסבב הזה — התוצאה: כל איש-קשר שנוצר-אי-פעם
נשאר עם שני השדות `NULL` קבוע, בלי שום דרך להזין ת.ז./כתובת אישית (שונה
מ`properties.address`, שכבר מוצג בכל מקום אחר בקובץ).

**התיקון:** תוספת טהורה בת שלוש נקודות ב-`app.html`, מחקה מילה-במילה את
דפוס השדה השכן (`city`): שני `<label class="f">` חדשים ("כתובת"/"ת.ז.")
ב-`contactForm()` ליד שדה העיר הקיים; `address`/`id_number` נוספו למערך
שמות-השדות בלולאת ה-payload; שתי שורות `.facts` חדשות ב-`openContact()`
ליד שורת "עיר" הקיימת. שום RPC/מיגרציה נדרשו — שניהם כבר תמכו בקריאה/כתיבה
במלואם.

אומת: `node --check` על ה-`<script type="module">` המחולץ עבר נקי; בדיקת
איזון-סוגריים על הקובץ המלא עברה (פרנתזות 3561→3565, מסולסלים 971→971,
מרובעים 293→293 — תואם בדיוק שלוש תוספות שורה-בודדת ללא מבנה חדש). אומת
חי ב-MCP בתוך `BEGIN/ROLLBACK` מול המשרד האמיתי "משרד בדיקה QA 18/08 - אל
תמחק" (owner אמיתי, `set local role authenticated`+`request.jwt.claims`):
`np_contact_save` עם `address`+`id_number` יצר איש-קשר ששני השדות נשמרו בו
נכון; `np_contact_get` החזיר את שניהם בחזרה נכון; עדכון-חלקי ששלח רק
`id_number` שינה אותו בעוד `address` נשמר ללא שינוי (coalesce, זהה
להתנהגות `city` הקיימת) — אומת דרך טבלת-לוג זמנית באותה טרנזקציה. `ROLLBACK`
+ בדיקה נפרדת נוספת אימתו אפס שיוריות (`count(*)=0` על שם איש-הקשר לבדיקה).

אפס רגרסיה: שלוש תוספות שורה-בודדת בלבד, שום handler/RPC/שדה קיימים אחרים
לא נגעו — `np_contacts` (רשימת סיכום) נותר ללא שינוי בכוונה. `core.build_tasks`
id=126 (system 36, priority 82) הוכנס כ-`done`. נדחף לענף
`fix/36-nadlan-pro-contact-id-address-0903` — לא מוזג. System 35 KioskFleet
לא נגע, לפי ה-HARD STEERING.

## עדכון — 03/09/2026, session 15 (Loop A — 32: פירוט-לפי-קומה בניתוח נסח טאבו מעולם לא נבנה כאן, בניגוד ל-36)
ביקורת עצמאית של סעיף (c)+(e) ב-TOP BUILD DIRECTIVE (TABU workflow,
core.projects #33) — "an extra AI research pass... APPENDS to the research"
ו-"the rights on EACH floor" — מול הקוד בפועל מצאה ש-`lib/tabudoc.ts` (מודול
ניתוח-AI של נסח טאבו על המערכת הזו) מעולם לא ביקש/חילץ פירוט-לפי-קומה, בזמן
ש-`np-tabu-document-analyze` (Edge Function של system 36, build_tasks id=11,
25/08/2026) **כבר בנה בדיוק את זה** — שדה `perFloorRights` (קומה/תת-חלקה +
תמצית-זכויות לכל יחידה), עם הערה מפורשת בקובץ עצמו שהתוספת נועדה בדיוק
למקרה שה-32 flow "אף פעם לא הצטרך פירוק-לקומות כי הוא תמיד מתפענח לדירה
בודדת" — אבל בפועל 32 **כן** תומך בנסח מרוכז-בניין (`scope: 'building'`,
ראה `TabuScope`), שם פירוק-לפי-קומה בדיוק רלוונטי ולא היה קיים.

**התיקון:** פורטו מילה-במילה אותם שלושה שינויים שכבר הוכחו על 36 — סעיף-כלל
ב-`PROMPT` (`lib/tabudoc.ts`), השדה בסכמת ה-JSON המבוקשת, ופונקציית
ה-`normalize()` (סינון-`summary`-ריק זהה) — לצד שדה `perFloorRights` חדש
ב-`TabuAnalysis` (`lib/requests.ts`). שני משטחי-התצוגה שכבר מרנדרים כל שדה
אחר בניתוח קיבלו את אותה שורה בדיוק: `tabuBlock()` (`lib/reporthtml.ts`,
המייל שנשלח בפועל ללקוח) ו-`TabuPanel` (`components/admin/RequestsBoard.tsx`,
לוח הניהול) — לא נגעתי בשום שדה/רינדור קיים אחר.

אומת: `npx esbuild` (רשת זמינה הסבב הזה) תרגם את שלושת הקבצים המלאים נקי;
בדיקת איזון-סוגריים על כל ארבעת הקבצים ששונו עברה (`requests.ts`
149/149,474/474,48/48; `tabudoc.ts` 57/57,118/118,26/26; `reporthtml.ts`
606/606,626/626,59/59; `RequestsBoard.tsx` 190/190,194/194,72/72). לוגיקת
`normalize()`+רינדור-שורה שוכפלה עצמאית ב-Node טהור מול 5 תרחישים — נסח
מרוכז עם 3 יחידות (כולל יחידה בלי `floor`), נסח-דירה עם מערך ריק, שדה חסר
לגמרי מה-AI (נפילה-לאחור בטוחה), יחידה עם `summary` ריק (מסוננת, לא מוצגת
כשורה ריקה), ובריחת-XSS על `floor`/`summary` עוינים — כל 5 תואמים במדויק,
כולל בריחת ה-HTML הנכונה (`esc()`) הזהה לכל שדה אחר שכבר עובר דרכו. אין
דפדפן חי בסנדבוקס הזה לקליק-דרך על לוח הניהול, אותה מגבלה מוצהרת כמו כל
סבב `apps/32` קודם.

אפס רגרסיה: שדה אופציונלי-בפועל (מערך, ברירת-מחדל ריקה) בטיפוס קיים + שורת
רינדור מותנית בשני מקומות — שום שדה/פונקציה/API/רינדור קיימים אחרים לא
נגעו. נדחף לענף `fix/32-nadlan-berega-tabu-per-floor-rights-0903` (1bec52ad)
— לא מוזג. System 35 KioskFleet לא נגע, לפי ה-HARD STEERING.

## עדכון — 03/09/2026, session 14 (Loop A — 36: סיבת "לא יצא לפועל" קיימת בסכמה מאז ומעולם לא נכתבה)
`nadlan_pro.deals.lost_reason` קיימת מאז המיגרציה הראשונה של הסכמה (0009),
ו-`np_deal_save` (0010) כבר יודעת לכתוב אליה (`coalesce(p->>'lost_reason', ...)`)
— אבל שינוי שלב מעולם לא עובר דרך `np_deal_save`: גם ה-select "שלב" בכרטיסיית
עסקה וגם גרירת קלף בלוח הפייפליין קוראים אך ורק ל-`np_deal_stage(p_id, p_stage)`
הייעודי (0010, בכוונה — "dragging a card... must not need the rest of the
record round-tripped"), ופונקציה זו מעולם לא נגעה ב-`lost_reason`. התוצאה:
`lost_reason` נשארה `NULL` קבוע אצל **כל** עסקה שאי-פעם סומנה כ"לא יצא לפועל",
ושום מקום ב-UI לא ביקש או הציג סיבה — `grep` על `lost_reason` ב-`app.html`
החזיר אפס תוצאות לפני הסבב הזה.

**התיקון:** מיגרציה `0163_nadlan_pro_deal_lost_reason.sql` מוסיפה ל-
`np_deal_stage` פרמטר שלישי אופציונלי `p_lost_reason text default null`
(כל קריאה קיימת בת-שני-ארגומנטים ממשיכה לעבוד בלי שינוי), שנכתב באותה שיטת
coalesce כמו ב-`np_deal_save` — מעבר לשלב אחר, או השמטת הפרמטר, אף פעם לא
נוגע בערך הקיים (אין ניקוי-בכוח בפתיחה מחדש של עסקה). ב-`app.html`: גם ה-
`dStage` select בכרטיסיית העסקה וגם ה-drop handler בלוח מבקשים סיבה עם
`window.prompt` (אותו pattern שכבר קיים בקובץ לקישור-הזמנה) ברגע שהיעד הוא
`lost`, ומעבירים `p_lost_reason` ל-`np_deal_stage`; הסיבה מוצגת גם בכרטיסיית
העסקה כשהיא בשלב `lost`.

אומת חי ב-MCP בתוך `BEGIN/ROLLBACK` נגד משרד ה-QA האמיתי: עסקה זמנית עברה
`lead→offer→lost(עם סיבה)→offer(סיבה נשמרת)→lost(עם רווחים-בלבד, לא דרס את
הסיבה הקיימת)` — כל שלב תואם בדיוק את ההתנהגות המיועדת. הוחל בפועל דרך
`apply_migration`; `get_advisors` (security) לא סימן אזהרה חדשה על הפונקציה.
`node --check` על ה-`<script type="module">` שחולץ מ-`app.html` (205,562
תווים) עבר נקי, ובדיקת איזון-סוגריים על הקובץ המלא עברה (3518/3518 עגולים,
962/962 מסולסלים, 283/283 מרובעים — זהה למספרים לפני השינוי, כי כל תוספת
הייתה ביטוי שלם ומאוזן בפני עצמו). אפס רגרסיה — שינוי אחד בחתימת פונקציה
(תוספת פרמטר בברירת-מחדל בלבד, לא הסרה/שינוי של קיים) ותוספות טהורות ב-3
מקומות ב-`app.html`, אף שדה/RPC/זרימה קיימים לא נגעו. System 35 KioskFleet
לא נגע, לפי ה-HARD STEERING.

## עדכון — 03/09/2026, session 13 (Loop A — 32: תחבורה ציבורית (קווים בכל תחנה) מעולם לא הגיעה למצגת/PDF)
המשך לאותה מחלקת-פער שכל סבבי 03/09 היום תיעדו שוב ושוב: נתון שקיים במקור-
אמת ומגיע למסך+למייל, אבל `Presentation.tsx`'s `buildSlides()` בונה שקופיות
ידנית שדה-שדה ומעולם לא כלל תחבורה ציבורית. `TransitLines.tsx` (המסך, קטגוריית
`transport`) מציג לכל תחנת-הליכה את הקווים שעוצרים בה בפועל (GTFS דרך Stride —
ראה `lib/gtfs.ts`), ו-`buildreport.ts` (שורה ~3400) מזין את אותו מידע גם למייל
דרך `notes`/`categories` הגנרי — אבל `grep` על `transit`/`Transit` ב-
`Presentation.tsx` החזיר אפס תוצאות לפני הסבב הזה. מי שרואה רק את המצגת/PDF
מול לקוח לא ידע בכלל אילו קווי אוטובוס/רכבת עוצרים ליד הנכס, גם כשהמידע כבר
קיים בדוח וללא תלות ברמה (`d.transitStops` נמשך תמיד — `tier==='basic'` רק
מקטין ל-3 תחנות במקום 6, לא חוסם לגמרי).

**התיקון:** שקופית חדשה ב-`buildSlides()` (בין שקופית "מה יש מסביב" לשקופית
"מיקום"/המפה, אותו מיקום יחסי כמו במסך — `TransitLines` יושב מיד אחרי
`PlacesPanel`/`MikvePanel` ב-`ReportView.tsx`), מוצגת רק כש-`d.transitStops`
לא ריק: עד 5 תחנות, לכל תחנה שם(+מספר-תחנה כשידוע) + מרחק + שורת-קווים
(עד 6 קווים + "ועוד N"), או הודעת-נפילה מדויקת (קווים-לא-נטענו /
אין-קווים-בחלון) — בדיוק שלושת המצבים ש-`TransitLines.tsx` עצמו כבר מטפל
בהם. אין קריאת-מקור חדשה בכלל: `d.transitStops` כבר נמשך בכל הפקת דוח כולל
PDF (אין guard-עלות מקביל ל-`skipListings` כאן — המקור הזה חינמי, לא Apify).

אומת: `npx esbuild --bundle` (רשת זמינה הסבב הזה) תרגם את הקובץ המלא נקי
(עם `--external` לכל ה-imports החיצוניים); בדיקת איזון-סוגריים על הקובץ המלא
עברה (186/186 מסולסלים, 271/271 עגולים, 43/43 מרובעים). לוגיקת בניית ה-
`rows` שוכפלה עצמאית ב-Node טהור מול 7 תרחישים — רשימה ריקה (אין שקופית),
תחנה רגילה עם קווים+מספר-תחנה, תחנה בלי מספר-תחנה, `linesUnavailable=true`,
תחנה בלי אף קו (לא-unavailable), יותר מ-6 קווים (נחתך + "ועוד N"), ויותר
מ-5 תחנות (נחתך ל-5) — כל 7 תואמים במדויק את ההתנהגות של `TransitLines.tsx`.
אין דפדפן חי בסנדבוקס הזה לגלול בפועל בדק/PDF, אותה מגבלה מוצהרת כמו כל
סבב `apps/32` קודם היום.

אפס רגרסיה: שקופית חדשה אחת בלבד, שום שקופית/טיפוס/פונקציה אחרים לא נגעו —
המסך (`TransitLines.tsx`) והמייל (קטגוריית `transport` הגנרית) לא נגעו כי
הם כבר מציגים את הנתון נכון. נדחף לענף
`fix/32-nadlan-berega-transit-lines-deck-slide-0903` — לא מוזג. System 35
KioskFleet לא נגע, לפי ה-HARD STEERING.

## עדכון — 03/09/2026, session 12 (Loop A — 32: שקופית "מה יש מסביב" בדק/PDF הציגה 3 מתוך 6 קבוצות + השמיטה מקוואות לגמרי)
המשך לאותה מחלקת-פער שכל סבבי 03/09 היום כבר תיעדו שוב ושוב (נתון שקיים
במקור-אמת ומגיע למסך+למייל, אבל `Presentation.tsx`'s `buildSlides()` בונה
שקופיות ידנית שדה-שדה ומציג רק חלק): `PlacesPanel.tsx` (המסך) מציג שש קבוצות
מלאות — מוסדות חינוך/גנים ומעונות/בתי כנסת ומוסדות דת/מסחר וקניות/בריאות/
פארקים ופנאי — ועוד `MikvePanel.tsx` נפרד למקוואות; `notes.surroundings`
ב-`buildreport.ts` (שורה 3374, ההערה שם אומרת במפורש: "יוצאות כ-notes כדי
שגם ה-PDF, המצגת והמייל יקבלו אותן") כבר מזין את אותן שש הקבוצות + מקוואות
גם למייל, דרך מנגנון ה-`categories`/`notes` הגנרי שהמייל כן צורך. אבל שקופית
"מה יש מסביב" הקיימת ב-`Presentation.tsx` בנתה `rows` רק מ-`d.places.education`
+`d.places.transport`+`d.places.religion` (top-2 כל אחת) — שלוש קבוצות
בלבד, כש-`transport` עצמו הוא Google Places, לא הקבוצות שהמסך/המייל מציגים
בכלל. גנים-ומעונות, מסחר-וקניות, בריאות, פארקים-ופנאי ומקוואות היו נעדרים
לגמרי מהדק/PDF — מי שרואה רק את המצגת מול לקוח (בדיוק השימוש שלה) לא ידע
בכלל שיש גנים/מרפאות/פארקים/מקוואות בסביבת הנכס, גם כשהמידע כבר קיים בדוח.

**התיקון:** בלוק בניית ה-`rows` הוחלף כך שהוא עובר על כל שש קבוצות
ה-`PlacesPanel` (אותם כותרות עברית מדויקות) + מקוואות כשורה שביעית — לכל
קבוצה לא-ריקה: כמות ("N בסביבה"/"N ביישוב") + הקרוב ביותר (שם+מרחק, `[0]`
של המערך — `mergePlaces`/`mikvaotNear` כבר ממיינים לפי מרחק, אותה הנחה
ש-`PlacesPanel`/`MikvePanel` עצמם כבר סומכים עליה בלי מיון-מחדש). אין קריאת-
מקור/API חדשה בכלל: `d.places`/`d.mikvaot` כבר נמשכים בכל הפקת דוח כולל
PDF (בניגוד ל-`listings` שיש לו guard ייעודי `skipListings` בשביל עלות-
Apify, ראו session 10 למעלה — אין guard מקביל כאן כי אין עלות-לפי-קריאה
על הנתונים האלה). שקופית "מיקום"/המפה עם הסמנים (מיד אחרי) לא נגעה — היא
עדיין משתמשת ב-`d.places.education`/`transport`/`religion` הגולמיים לצורך
סימוני-מפה, לא לרשימת-מלל, אז אין כפילות/סתירה בין שתי השקופיות.

אומת: `npx esbuild --bundle` (רשת זמינה הסבב הזה) תרגם את הקובץ המלא נקי
(עם `--external` לכל ה-imports החיצוניים, בדיוק כדי לתפוס שגיאות-syntax
אמיתיות ולא רק איזון-סוגריים); בדיקת איזון-סוגריים על הקובץ המלא עברה
(178/178 מסולסלים, 255/255 עגולים, 43/43 מרובעים). לוגיקת בניית ה-`rows`
שוכפלה עצמאית ב-Node טהור מול 5 תרחישים — שש הקבוצות+מקוואות מלאים (7
שורות, כל שדה נכון כולל "הקרוב ביותר"), הכול ריק (0 שורות, אין שקופית),
מקוואות-בלבד-בלי-אף-קבוצת-מקומות (שורה אחת), קבוצה בודדת (שורה אחת),
ומקווה בלי `meters` (נופל ל-שם-בלבד בלי "· null") — כל 5 תואמים במדויק.
`walkText` (יובא בקובץ במקור רק בשביל הבלוק הישן) הוסר מה-import אחרי
שנבדק שאין עוד קורא לו בקובץ — אחרת esbuild/eslint היו מסמנים import לא-
בשימוש. אין דפדפן חי בסנדבוקס הזה לגלול בפועל בדק/PDF, אותה מגבלה מוצהרת
כמו כל סבב `apps/32` קודם היום.

אפס רגרסיה: בלוק אחד הוחלף (שקופית "מה יש מסביב" בלבד), שקופית המפה/הסמנים
מיד אחריה לא נגעה, ושום שקופית/פאנל/API אחרים לא נגעו — המסך
(`PlacesPanel.tsx`/`MikvePanel.tsx`) והמייל (`notes.surroundings` ב-
`buildreport.ts`) לא נגעו כי הם כבר מציגים את הנתון נכון. נדחף לענף
`fix/32-nadlan-berega-surroundings-deck-slide-0903` — לא מוזג. System 35
KioskFleet לא נגע, לפי ה-HARD STEERING.

## עדכון — 03/09/2026, session 11 (Loop A — 32: נתוני פשיעה קיימים ועובדים, אך מעולם לא הגיעו לדוח המודרני — רק לזרם הישן)
בדיקה שיטתית של `grep -rn "crimeProfile\|from '\./crime'"` בכל הריפו לפני
הסבב הזה מצאה שהקורא היחיד ל-`lib/crime.ts`'s `crimeProfile()` (מאגר תיקי-
פשיעה של משטרת ישראל ב-data.gov.il, ברמת יישוב — ראה ההערה המפורטת בראש
הקובץ) הוא `app/api/profile/route.ts` בלבד — זרם ה"כרטיס זיהוי" הישן
(`PropertyIdCard.tsx`). הרישום ב-CLAUDE.md מ-25/08/2026 ("שדה 'פשיעה' היה
`pending` קבוע... חובר אמיתית") מתאר את החיבור **לזרם הזה בלבד**, ולא
לזרם הדוח המודרני (`lib/buildreport.ts` → `/report`, PDF, מייל, מצגת) שהאתר
הציבורי בפועל מציג היום מאז המעבר ל-"7 קטגוריות · 3 רמות" ב-29/07/2026 —
`grep` על `crime`/`Crime` בכל 3,500+ שורות `buildreport.ts` (לפני הסבב הזה)
החזיר אפס תוצאות, ו-`CategoryKey` ב-`lib/report.ts` אין בו אפילו קטגוריית
"סביבה וסיכונים" נפרדת (רק `surroundings` — "סביבה ומוסדות"). כלומר כל לקוח
שעובר דרך המסלול האמיתי היום (טופס→`/report?q=`→PDF/מייל/מצגת) לא רואה
נתוני פשיעה בכלל — לא כ-`pending`, אלא כשדה שלא קיים שם בכלל. אותה מחלקת-
פער בדיוק שכל שאר sessions 1-10 היום כבר תיעדו (יכולת-שרת קיימת ועובדת,
מעולם לא חוברה למסלול שהלקוח בפועל רואה) — רק שכאן זו לא "משטח אחד מתוך
ארבעה" אלא "אפס מתוך ארבעה" (מסך/PDF/מייל/מצגת כולם חסרים).

התיקון: `buildReport` קורא עכשיו ל-`findCityCode(city)` (מ-`lib/placenames.ts`,
כבר יוצא — אותה פונקציה בדיוק שהזרם הישן משתמש בה) ואז `crimeProfile(cityCode)`
מיד אחרי שליפת המקוואות (אותו שער `if (city)`, אותו מקום יחסי בשלב-האיסוף
האסינכרוני), ומוסיף עובדה חדשה ל-`facts.surroundings` מיד אחרי "מקוואות
ביישוב" — אותו נוסח מילה-במילה כמו הזרם הישן (כותרת, "בעיקר: <קטגוריה>
(<מספר>)", ההסבר על רמת-יישוב-לא-רדיוס, `sourceKey: 'datagov'`, `tier:
'premium'`). כיוון ש-`categories`/`facts.surroundings` הוא המנגנון הגנרי
שכבר מזין את המסך (`ReportView.tsx`) ואת המייל (`reporthtml.ts`'s
`categories` loop) לכל שדה קיים — השדה החדש מגיע לשני המשטחים האלה **בלי
שום שינוי נוסף**, בדיוק כמו שסעיף "מדיניות רמ"י"/"היתר בנייה" כבר עשו
בסבבים קודמים היום. המצגת (`Presentation.tsx`) בונה שקופיות ידנית שדה-שדה
ולא צורכת `categories`/`facts` בכלל (אותה מגבלה מתועדת לגבי כל שדה גנרי
אחר) — לא נוספה שקופיה ייעודית הסבב הזה כדי לא להרחיב את ההיקף מעבר לפער
שנמצא בפועל (המצגת ממילא לא מציגה את רוב שדות ה-`surroundings`, לא רק
פשיעה — זה עקבי, לא רגרסיה).

אומת: `npx esbuild` (רשת זמינה הסבב הזה) תרגם את `buildreport.ts` המלא נקי
(183.7kb, אפס שגיאות) ואת `lib/crime.ts`/`lib/placenames.ts` (ללא שינוי,
רק ווידוא תאימות-ייבוא) נקי; בדיקת איזון-סוגריים על הקובץ המלא עברה
(582/582 מסולסלים, 1193/1193 עגולים, 274/274 מרובעים). `crimeProfile`
עצמו נבדק **חי** מול data.gov.il האמיתי (לא רק קריאת-קוד): תל אביב-יפו
(קוד 5000) → 37,064 תיקים ל-2025, ירושלים (3000) → 29,910, חיפה (4000) →
12,478 — תואם בדיוק למספרים שכבר תועדו ב-25/08/2026 כשהזרם הישן חובר,
מוכיח שהמקור עדיין חי ומחזיר את אותם נתונים; קוד-יישוב `null` → `null`
מיידי בלי קריאת רשת. לוגיקת בניית ה-`fact()` (כותרת עם/בלי שנה, `topCategories`
ריק/מלא, `crime=null` עם/בלי `city` ידוע) שוכפלה עצמאית ב-Node טהור מול 4
תרחישים — כולם רינדרו נכון, כולל שני מסלולי ה-`missingReason` השונים (עיר
לא ידועה מול עיר ידועה שהמאגר לא החזיר לה נתון). אין `node_modules`/`tsc`/
דפדפן חי בסנדבוקס הזה להריץ `next build`/לגלול בפועל ב-`/report`, אותה
מגבלה מוצהרת כמו כל סבב `apps/32` קודם.

אפס רגרסיה: שתי שורות ייבוא + משתנה חדש + בלוק-איסוף חדש (שער `if (city)`,
נכשל בשקט ל-`warnings` כמו כל שאר המקורות בסביבה הזו) + עובדה חדשה אחת
ב-`facts.surroundings` — שום שדה/פונקציה/API/זרם ישן קיימים לא נגעו
(`app/api/profile/route.ts` וההתנהגות שלו זהות לפני/אחרי). נדחף לענף
`fix/32-nadlan-berega-crime-modern-report-0903` — לא מוזג. System 35
KioskFleet לא נגע, לפי ה-HARD STEERING.
