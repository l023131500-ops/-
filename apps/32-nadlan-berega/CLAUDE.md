...
## עדכון — 03/09/2026, session 10 (Loop A — 32: "דירות מוצעות כרגע" (§4 listings) הגיעו למסך ולמייל אך מעולם לא למצגת/PDF)
המשך לאותה מחלקת-פער שכבר תועדה שוב ושוב היום (session 5-9 למעלה): נתון-דוח
שקיים במקור-אמת ומגיע לשני משטחים אך לא לשלישי, כי `Presentation.tsx`'s
`buildSlides()` בוחר שקופיות ידנית שדה-שדה. הפעם: `report.listings`
(`lib/apify.ts`, מודעות יד2/מדלן פעילות באזור) כבר מוצג במסך
(`Listings.tsx`, מחובר פעמיים ב-`ReportView.tsx`) ובמייל (`listingsBlock()`,
`lib/reporthtml.ts`, קרוי בפועל בכל שליחת דוח מ-`app/api/admin/requests`) —
אבל `grep` על `d.listings`/`Listing` ב-`Presentation.tsx` החזיר אפס תוצאות
לפני הסבב הזה (מלבד `skipListings`, פרמטר-URL לא-קשור).

**גילוי חשוב תוך כדי הבדיקה:** ה-fetch של `Presentation.tsx` עצמו כבר מוסיף
`&skipListings=1` **רק** כש-`print=true` (הפקת PDF/דק — כדי לא לשלם על
Apify בכל קובץ שמופק, לפי הערה מפורשת בראש הקובץ: "אף שקופית אינה מציגה
מודעות, ולכן במסלול הקובץ לא משלמים עליהן"). כלומר זו לא "עוד לא נבנה" סתם —
מישהו כבר חשב על עלות-Apify בהקשר הזה, ובכל זאת השאיר את הנתון בחוץ גם
במצגת האינטראקטיבית החיה (`print=false`), ששם `d.listings` **כן** נמשך
ומשולם עליו אך אף פעם לא הוצג. תיקון נאיבי (שקופית ללא guard) היה מנוגד
לכוונה המתועדת; התיקון בפועל משתמש באותו guard בדיוק כמו `listingsBlock()`
של המייל (`listingsStatus?.configured && listings?.length`) — כך שההתנהגות
נשמרת בשני הכיוונים: אין שקופית ריקה בהפקת-PDF (מכבד את חיסכון-העלות הקיים
בכוונה), אבל יש שקופית אמיתית במצגת החיה שבה הנתון כבר נמשך ושולם עליו.

**התיקון:** שקופית חדשה ב-`buildSlides()` (בין שקופית "הערכת שווי" לשקופית
"מה יש מסביב", אותו מיקום יחסי כמו במסך — `Listings` יושב מיד אחרי
`ValuationPanel` ב-`ReportView.tsx`), + קבוע `LISTINGS_NOUN` חדש (עותק מדויק
של זה שכבר קיים ב-`lib/reporthtml.ts`, כי אין import משותף בין שני הקבצים
לקבועים כאלה — אותה שיטה בדיוק כמו קבועי-תווית אחרים בקובץ הזה). כותרת "N
<שם-לפי-סוג-נכס> כרגע באזור", עד 4 שורות (כתובת, מחיר, ₪-למ"ר), נופל בבטחה
ל"כתובת לא צוינה"/"—" על שדות חסרים — בדיוק כמו `listingsBlock()`. אותו
`Slide` type הקיים בדיוק (`kicker/title/subtitle/rows`), שום טיפוס/שקופית
קיימים לא נגעו.

אומת: `npx esbuild` (רשת זמינה הסבב הזה) תרגם את הקובץ המלא נקי; בדיקת
איזון-סוגריים על הקובץ המלא עברה (172/172, 248/248, 33/33). לוגיקת בניית
השקופית שוכפלה עצמאית ב-Node טהור מול 6 תרחישים — מצב-`skipListings`/הפקת-
PDF (`listings=[]` → 0 שקופיות, מכבד את חיסכון-העלות הקיים), רמת-`basic`/
דוח-קרקע (`configured=false` → 0 שקופיות), דוח שמור ישן בלי `listingsStatus`
בכלל (לא קורס, 0 שקופיות), נתונים עשירים במגורים (כותרת+ספירה נכונים, 4
שורות מתוך 5, שדות-חסרים נופלים ל"—"/"כתובת לא צוינה"/בלי `note`), סוג-נכס
שכירות (תווית נכונה "דירות שמוצעות להשכרה"), וסוג-נכס לא-מזוהה (נופל לתווית
מגורים כברירת מחדל) — כל 6 תואמים במדויק. אין דפדפן חי בסנדבוקס הזה לגלול
בפועל במצגת, אותה מגבלה מוצהרת כמו כל סבב `apps/32` קודם.

אפס רגרסיה: שקופית חדשה אחת + קבוע חדש אחד, שום שקופית/טיפוס/פונקציה
קיימים לא נגעו — המסך (`Listings.tsx`) והמייל (`listingsBlock()`) לא נגעו
כי הם כבר מציגים את הנתון נכון. נדחף לענף
`fix/32-nadlan-berega-listings-deck-slide-0903` — לא מוזג. System 35
KioskFleet לא נגע, לפי ה-HARD STEERING.

## עדכון — 03/09/2026, session 9 (Loop A — 32: מגמת מחירי הדירות (מדד הלמ"ס) מעולם לא הגיעה למייל או למצגת/PDF)
לפני הסבב הזה נבדק מחדש מצב הדיפלוי (`OWNER ORDER 2026-09-02b`): `core.build_tasks`
עדיין 0 שורות `todo` שאינן "merge to main + deploy" עבור 35/32/36/01 (15 בלי
שורת-דיפלוי כלל), וההוכחה שהחסימה סביבתית ולא ספציפית-למערכת התחזקה עוד יותר
הסבב הזה: נבדק בפועל ה-repo האמיתי של קיוסק (`/home/m30/zol-work`, remote
`l023131500-ops/zol`) — `origin/main` שם מכיל **רק** `README.md` וקובץ zip, לא
את אפליקציית הקיוסק בכלל (כל הבנייה בפועל יושבת אך ורק על ענפי `feat/`/`fix/`
שלא מוזגו), ואין `railway.json`/`Procfile`/`nixpacks.toml` בעץ בשום ענף שמאשרים
איזה ענף Railway בכלל צופה בו. כלומר merge+push ל-main כאן לא רק "חסום בגלל
חוסר-CLI" (כמו שתועד ב-`apps/35-kioskfleet/STATUS.md` session 7) — התוצאה של
push כזה בפועל **לא ידועה** (יכול להיות no-op, יכול לשבור שירות-פרודקשן אם
Railway בכל זאת צופה ב-main מסיבה לא-מתועדת). זה חיזוק קונקרטי וטרי לממצא
שכבר תועד שלוש פעמים היום (35/32+36/01) שהחסימה היא תכונה של הסביבה הזו, לא
החלטת-הימנעות — ולכן, כמו session 8 למעלה, הסבב הזה המשיך בביקורת-איכות
מבוססת-verification במקום לנסות push לא-בטוח או לחזור על אותה בדיקה פעם רביעית.

session 8 (למטה) השאיר פתוח: "`report.priceTrend` (מדד מחירי-דירות ארצי של
הלמ"ס) מוצג במסך אך אינו מגיע לא למייל ולא למצגת — מחלקת-פער דומה אך שונה
(מסך-בלבד, לא מסך+מייל), דורש תיקון בשני משטחים ולא רק אחד". אומת: `lib/
buildreport.ts` בונה `priceTrend: cbs?.points ?? []` בכל דוח **ללא תלות ברמה**
(לא כמו `valuation`/`listings` שנעדרים לגמרי ברמה חינמית) — השער היחיד הוא
ב-`ReportView.tsx` עצמו: `{tier !== 'basic' && <PriceTrend points={data.
priceTrend} />}`, מיד אחרי טבלת העסקאות שנסגרו (קטגוריית `sold`). `grep` על
`priceTrend` בכל הריפו החזיר רק את שלוש הנקודות האלה (הגדרת-טיפוס, בנייה
ב-buildreport, צריכה במסך) — אפס ב-`lib/reporthtml.ts` (מייל) ואפס ב-
`Presentation.tsx` (מצגת/PDF).

**התיקון:** `priceTrendBlock(report)` חדש ב-`lib/reporthtml.ts` — אותו שער
`tier==='basic'`/`points.length<3` בדיוק כמו `PriceTrend.tsx`, טבלת "תקופה ↔
ערך מדד" (עד 8 הנקודות האחרונות, כי מייל לא יכול להריץ `recharts`) + שורת
שינוי-אחוזי בצבע ירוק/אדום כמו הרכיב במסך, קרוי מיד אחרי טבלת העסקאות (אותו
מיקום יחסי כמו במסך) גם ב-`reportEmailHtml` וגם ב-`reportEmailText` (גרסת
טקסט תואמת). ב-`Presentation.tsx`: שקופית חדשה ב-`buildSlides()` מיד אחרי
שקופית "עסקאות שנסגרו" (אותו מיקום יחסי), עם אותו שער `d.tier !== 'basic'`
נבדק במפורש (בניגוד ל-`valuation`/`nearbyPlans` שם השער כבר מגולם בהיעדר-
נתונים — כאן `priceTrend` תמיד קיים בנתונים ללא תלות ברמה, אז חובה לבדוק
`tier` בפירוש כדי לא לחשוף ל-`basic`). אותו `Slide`/`section()` הקיימים
בדיוק — שום טיפוס/שקופית/סעיף קיימים לא נגעו.

אומת: `npx esbuild` (רשת זמינה הסבב הזה) תרגם את שני הקבצים המלאים נקי;
בדיקת איזון-סוגריים על שניהם עברה (`reporthtml.ts`: 580/580,584/584,59/59;
`Presentation.tsx`: 165/165,232/232,32/32). לוגיקת בניית הבלוק/השקופית
(HTML+טקסט+דק, שלושתם) שוכפלה עצמאית ב-Node טהור מול 8 תרחישים — רמה
`basic` (שלושתם ריקים/null גם עם נתונים עשירים), פחות מ-3 נקודות (ריק, אותו
שער כמו הרכיב במסך), `priceTrend` חסר/`null` (ריק, לא קורס), מגמת-עלייה
עשירה (12 נקודות → אחוז חיובי נכון, צבע טורקיז, "8 מתוך 12", 4 השורות
האחרונות בשקופית), מגמת-ירידה (אחוז שלילי בלי "+-" כפול, צבע אדום), נקודה
ראשונה בערך 0 (בלי `NaN`/קריסה, בלי שורת-שינוי בטקסט), וסט קטן (≤8, בלי
הערת-קיצוץ) — כל 8 עברו (כולל תיקון טעות-חישוב בבדיקה עצמה, לא בקוד: הציפייה
הראשונית לתרחיש-העלייה חושבה ב-27.5% במקום 55% הנכונים — תוקנה לפני שהבדיקה
נחשבה עוברת). אין דפדפן חי בסנדבוקס הזה לגלול בפועל במייל/מצגת, אותה מגבלה
מוצהרת כמו כל סבב `apps/32` קודם.

אפס רגרסיה: פונקציה חדשה + קריאה נוספת אחת בכל אחד משלושת המשטחים (HTML/
טקסט/דק), שום בלוק/שקופית/טיפוס קיימים לא נגעו — המסך עצמו (`PriceTrend.tsx`)
גם הוא לא נגע כי הוא כבר מציג את הנתון נכון. נדחף לענף
`fix/32-nadlan-berega-price-trend-email-deck-0903` — לא מוזג. System 35
KioskFleet לא נגע, לפי ה-HARD STEERING.

## עדכון — 03/09/2026, session 8 (Loop A — 32: מדיניות רמ"י לשינוי ייעוד קרקע (דוח קרקע) מעולם לא הגיעה למצגת/PDF)
המשך לאותה מחלקת-פער שכבר תועדה שוב ושוב היום (session 5-7 למעלה): נתון
שקיים במקור-אמת ומגיע למסך+למייל, אבל `Presentation.tsx`'s `buildSlides()`
עדיין בוחר שקופיות ידנית שדה-שדה ומעולם לא כלל אותו. הפעם: `lib/rami.ts`
שואל בכל דוח קרקע (`assetType==='land'`) את פרק "קרקע חקלאית" מקובץ החלטות
מועצת מקרקעי ישראל (`apps.land.gov.il/CouncilDecisions`, JSON פתוח בלי מפתח)
ומחזיר `report.ramiPolicy` — סעיפים רלוונטיים לשינוי ייעוד/השבת קרקע, עם
גרסה+תאריך+קישור למקור. בדקתי איפה זה בפועל מגיע ללקוח: מסך — `RamiPolicyPanel`
(`PermitsPanel.tsx` שורה 129, מחובר ב-`ReportView.tsx` שורה 681, `cat.key
==='land'`) כבר מרנדר את כל הסעיפים כ-`<details>`. מייל — `buildreport.ts`
(שורות 3320-3330) בונה `notes.land` (רשימת הסעיפים+קישור למקור) על קטגוריית
ה-`land`, וההערה בקוד עצמה אומרת במפורש "יוצאות כ-`notes` כדי שגם ה-PDF,
המצגת והמייל יקבלו אותן" — אבל `reporthtml.ts`'s `categories` loop (שורה 624,
`c.notes?.length ? bullets(...)`) הוא שמימש את החלק הזה בפועל; `Presentation.tsx`
לא צורך `categories`/`notes` בכלל, וגם לא בודק `assetType==='land'` באף
מקום — `grep` על `assetType` בקובץ מחזיר רק שימוש ב-fetch/exit, אף לוגיקת-
שקופית. תוך כדי הבדיקה נבדקו גם `priceTrend` (מדד למ"ס ארצי, מוצג במסך
כ-`PriceTrend.tsx` ל-`tier!=='basic'`, אך **גם הוא לא מגיע למייל בכלל** —
פער אמיתי נוסף אבל לא מאותה מחלקה המדויקת שהמשימה ביקשה, כי שם גם המייל
חסר, לא רק המצגת; נשאר פתוח לסבב הבא) ו-`mikvaot`/`transitStops`
(מגיעים למסך+מייל דרך העובדות הגנריות, ולכן לא רלוונטיים כאן).

**התיקון:** שקופית חדשה ב-`buildSlides()` (בין שקופית "היתכנות להיתר נוסף"
לשקופית "סיכום"), מוצגת רק כש-`d.ramiPolicy` קיים **וגם** נושא לפחות סעיף
אחד (כלומר רק בדוח קרקע חקלאית רלוונטי — בדיוק אותו תנאי כמו
`RamiPolicyPanel`): כותרת קבועה "מדיניות רמ"י — שינוי ייעוד של קרקע חקלאית",
תת-כותרת = `note`+גרסה+תאריך (פורמט זהה ל-`RamiPolicyPanel`), עד 4 שורות
(אחד לכל סעיף — מספר+כותרת, נתיב, ותקציר הטקסט עד 140 תו). אותו `Slide` type
הקיים בדיוק (`kicker/title/subtitle/rows`), שום טיפוס/שקופית קיימים לא נגעו.

אומת: `npx esbuild` (רשת זמינה הסבב הזה) תרגם את הקובץ המלא נקי; בדיקת
איזון-סוגריים על הקובץ המלא עברה (160/160 מסולסלים, 217/217 עגולים, 30/30
מרובעים). לוגיקת בניית השקופית שוכפלה עצמאית ב-Node טהור מול 4 תרחישים —
`ramiPolicy===null` (0 שקופיות, דוח מגורים/שכירות/מסחרי רגיל), `sections=[]`
(0 שקופיות, חלקה לא-חקלאית שדוח-הקרקע שלה עדיין נבנה), נתונים עשירים עם 5
סעיפים (בדיוק שקופית אחת, `rows` נחתך ל-4 כמצופה, `text=null`/`number=null`
לא קורסים ונופלים בחזרה נכון), ו-`versionDate=null` (תת-כותרת נשארת תקינה
בלי "Invalid Date" דולף) — כל 4 תואמים במדויק. אין דפדפן חי בסנדבוקס הזה
כדי לגלול בפועל במצגת/PDF, אותה מגבלה מוצהרת כמו כל סבב `apps/32` קודם.

אפס רגרסיה: שקופית חדשה אחת בלבד (27 שורות) לפני שקופית "סיכום" הקיימת,
שום שקופית/טיפוס/פונקציה קיימים לא נגעו — מסך (`RamiPolicyPanel`) ומייל
(`reporthtml.ts`'s `categories`/`notes` loop) לא שונו כי הם כבר מציגים את
הנתון הזה נכון. **נשאר פתוח לסבב הבא:** `report.priceTrend` (מדד מחירי-דירות
ארצי של הלמ"ס) מוצג במסך אך אינו מגיע לא למייל ולא למצגת — מחלקת-פער דומה
אך שונה (מסך-בלבד, לא מסך+מייל), דורש תיקון בשני משטחים ולא רק אחד. נדחף
לענף `fix/32-nadlan-berega-rami-policy-deck-slide-0903` — לא מוזג. System 35
KioskFleet לא נגע, לפי ה-HARD STEERING.

## עדכון — 03/09/2026, session 7 (Loop A — 32: הנחיית "איך משיגים היתר בנייה" קיימת מזמן אך מעולם לא הגיעה למצגת/PDF)
המשך לאותה מחלקת-פער שכבר תועדה שוב ושוב ב-32/36 (נתון שקיים במקור-אמת ולא
מגיע לכל משטח-תצוגה): `lib/permits.ts`'s `permitGuidanceText()` בונה משפט
מעשי לכל דוח — לאיזו ועדה מקומית לפנות כדי לבדוק תיק-בניין, ולמה אין מרשם
ארצי של היתרים שאפשר לשאול לפי כתובת — ונשמר על `permits.permitGuidance`.
בדקתי איפה הוא בפועל מגיע ללקוח: `buildreport.ts` (שורה 2976) מציב אותו
כ-`missingReason` על עובדה בשם "היתר בנייה על הנכס" (value תמיד `null`,
tier=premium) — כלומר הוא **כן** מגיע למסך (העמודה הגנרית של קטגוריות) ולמייל
(`reporthtml.ts`'s `categories` loop, אותו מנגנון גנרי בדיוק), כי שניהם
מרנדרים כל fact כולל `missingReason`. אבל `Presentation.tsx` (המצגת החיה +
`/api/deck`'s PDF, שהוא `/present?print=1` מודפס) **אינו** משתמש במנגנון
הגנרי הזה בכלל — `buildSlides()` בוחר שקופיות ידנית שדה-שדה (בדיוק כמו
`valuationBlock`/`streetBlock` שכבר תוקנו כך בסבבים קודמים), ומעולם לא כלל
שקופית לתכנון/היתרים. `PermitsPanel.tsx` (הרכיב הייעודי במסך) גם הוא לא
מרנדר את `permitGuidance` בכלל — רק את רשימת התוכניות המאושרות/בהליך/מדיניות
ואת `documentsAccessNote`. כך שלקוח שרואה רק את המצגת/ה-PDF (בדיוק השימוש
שלה מול לקוח) מעולם לא ראה את ההנחיה המעשית הזו, ולא ידע כמה תוכניות כבר
חלות על הנכס או מי הוועדה המוסמכת.

**התיקון:** שקופית חדשה ב-`buildSlides()` (בין שקופית "פוטנציאל"/התחדשות
עירונית לשקופית "סיכום"), מוצגת רק כש-`d.permits` קיים: כותרת קבועה "איך
משיגים היתר בנייה בפועל", תת-כותרת = `permits.permitGuidance` המלא, ושלוש
שורות (תוכניות מאושרות + הערה מותנית, תוכניות בהליך, ועדה מקומית — האחרונה
רק כשידועה). אותו `Slide` type הקיים בדיוק (`kicker/title/subtitle/rows`),
שום טיפוס/שקופית קיימים לא נגעו.

אומת: `npx esbuild` (רשת זמינה הסבב הזה) תרגם את הקובץ המלא נקי. לוגיקת
בניית השקופית שוכפלה עצמאית ב-Node טהור מול 3 תרחישים — `permits===null`
(0 שקופיות), נתונים מלאים עם ועדה+תוכניות (כל השדות כולל שורת-ועדה+הערה),
וחור-כיסוי בלי ועדה ובלי תוכניות (שורת-ועדה נעדרת, הערת-האישור נעדרת, בלי
קריסה) — כל 3 תואמים במדויק. אין דפדפן חי בסנדבוקס הזה כדי לגלול בפועל
במצגת/PDF, אותה מגבלה מוצהרת כמו כל סבב `apps/32` קודם.

אפס רגרסיה: שקופית חדשה אחת בלבד בסוף `buildSlides()`, שום שקופית/טיפוס/
פונקציה קיימים לא נגעו — מסך (`PermitsPanel.tsx`) ומייל (`reporthtml.ts`)
לא שונו כי הם כבר מציגים את הנתון הזה נכון דרך המנגנון הגנרי. נדחף לענף
`fix/32-nadlan-berega-permits-deck-slide-0903` — לא מוזג. System 35
KioskFleet לא נגע, לפי ה-HARD STEERING.

## עדכון — 03/09/2026, session 6 (Loop A — 32: שטח רשום בנסח (חלקה/תת-חלקה/משותף) מחולץ ב-AI מנסח טאבו — ומעולם לא מוצג, לא ללקוח ולא לצוות)
המשך לאותה מחלקת-פער שכבר תועדה כמה פעמים ב-32/36 (שדה שקיים במקור-אמת ולא
מגיע לתצוגה): בדקתי קודם את ההבטחה ב-`PRODUCT_TIERS.md` §רמה 2 "חשד לחריגות
בנייה (הצלבת שטח מדווח מול היתר)" — נמצא **לא ניתן למימוש כפי שנכתב**:
`lib/xplan.ts` (`PlanningResult.buildingRights`) מחזיר `null` תמיד עם הערה
מפורשת שאין מקור-API לזכויות-בנייה כמותיות לחלקה ספציפית, ו-`lib/feasibility.ts`
מצהיר במפורש "מה שלא ייכתב כאן לעולם: מספר" — אין צד "היתר" למספר להשוואה,
לא רק ב-32 אלא בשום מקור ממשלתי מחובר. זה נשאר לא-בנוי בכוונה (חוסר-מקור-נתונים
אמיתי, לא עצלות) — לא נגעתי בזה.

אבל אותה חקירה חשפה פער אחר, אמיתי וקונקרטי, מאותה משפחה בדיוק (צד אחד של
ההשוואה כן קיים): `lib/tabudoc.ts` (מודול ניתוח-AI של נסח טאבו, ה-prompt
בשורה 46 + סכימת `TabuAnalysis` ב-`lib/requests.ts` שורות 246-248) כבר מחלץ
`parcelArea`/`subParcelArea`/`sharedAreas` (שטח החלקה/תת-החלקה/השטחים
המשותפים כפי שרשום בנסח בפועל) מכל נסח שמנותח — אבל שני מקומות התצוגה
היחידים של תוצאת-ניתוח (`tabuBlock()` ב-`lib/reporthtml.ts`, מה שנשלח
בפועל במייל ללקוח; ו-`TabuPanel` ב-`components/admin/RequestsBoard.tsx`,
לוח-הניהול) רינדרו בעלות/משכנתאות/הערות-אזהרה/חכירות/הגבלות/לא-ניתן-לקרוא —
**ומעולם לא את שלושת שדות-השטח**. `grep` על שם שלושת השדות בכל הריפו החזיר
תוצאות רק בהגדרה/נורמליזציה (`tabudoc.ts`/`requests.ts`) — אפס בכל מקום
שמרנדר תצוגה. שדה שה-AI כבר קורא בהצלחה מכל נסח (או מדווח "לא ניתן לקרוא"
בכנות, לפי כלל-הברזל של המודול) פשוט נעלם בדרך למסך ולמייל — בדיוק אותו
דפוס "שרת+נתון שלם, קונסולה/UI מעולם לא חיברה" שחוזר שוב ושוב ב-CLAUDE.md
הזה (`valuationBlock`/`streetBlock`/`potentialHtml` וכו').

(תוך כדי אותה בדיקה נמצאו גם `identifiedGush`/`identifiedHelka`/
`identifiedTatHelka` כשדות-אימות פנימיים שגם הם לא מוצגים בשום מקום — אלה
נשארו בכוונה מחוץ לסקופ הסבב הזה: הם נועדו להצלבה-פנימית מול גוש/חלקה
שהמשתמש כבר הקליד בבקשה, לא נתון-לקוח בפני עצמו כמו שטח, ומימוש נכון שלהם
דורש לוגיקת-השוואה חדשה, לא רק שורת-תצוגה. הותיר לסבב עתידי אם יוחלט שיש
בהם ערך.)

**התיקון:** תוספת טהורה בת 19 שורות בשני מקומות. `tabuBlock()`: אותו דפוס
בדיוק כמו שאר השדות (`if (a.X) areaParts.push(...)`), שורת "שטח רשום בנסח:"
חדשה נוספת ל-`lines` רק כשלפחות שדה-שטח אחד לא-ריק קיים — אין שדות → אין
שורה, בלי קריסה. `TabuPanel`: אותה תבנית-בדיוק כמו הבלוקים הקיימים
(owners/mortgages/cautionNotes), `<div>` מותנה שמוצג רק כש-
`parcelArea || subParcelArea || sharedAreas`. שני המקומות משתמשים ב-`esc()`
הקיים על כל ערך (הגנת-XSS זהה לכל שדה אחר שכבר מוצג).

אומת: `npx esbuild` (רשת זמינה הסבב הזה) תרגם את שני הקבצים ששונו נקי;
בדיקת איזון-סוגריים על שניהם עברה (`reporthtml.ts`: 440/440,428/428,47/47;
`RequestsBoard.tsx`: 189/189,185/185,72/72). לוגיקת בניית שורת-השטח (בשני
המקומות בנפרד) שוכפלה עצמאית ב-Node טהור מול 10 תרחישים — כל שלושת השדות,
רק אחד, אף אחד (השורה נעדרת, לא קורסת), ובריחת-HTML נכונה על ערך עוין —
כולם עברו. הותקנו `react`/`react-dom` בתיקיית `/tmp` מבודדת (לא נגעו
ב-`node_modules`/`package.json` של האפליקציה) ובוצע חילוץ-מדויק של בלוק
ה-`TabuPanel` לרכיב עצמאי, רונדר עם `react-dom/server` מול 4 תרחישי-props
אמיתיים (ניתוח מלא עם שלושת שדות-השטח, בלי אף שדה-שטח, רק `subParcelArea`,
ו-`analysis=null`) — כולם רינדרו נכון, כולל שהשורה נעדרת בדיוק כשצריך.
`tabuBlock()` המלא (לא רק הלוגיקה המבודדת) חולץ עם `esc()`/`section()`
הפרטיים שלו והורץ מול 3 תרחישי-מסמך מלאים (נסח דירה עם שלושת השדות, מסמך
בלי ניתוח בכלל → מחרוזת ריקה, נסח-בניין-מרוכז עם `parcelArea` בלבד) —
כולם רינדרו את ה-HTML הצפוי במדויק, כולל בריחת המרכאות העבריות (`מ"ר` →
`מ&quot;ר`) התואמת לכל שדה אחר שכבר עובר דרך `esc()`.

אפס רגרסיה: שתי תוספות מותנות בלבד (שורה חדשה שמוצגת רק כשיש נתון), שום
שדה/רכיב/handler/signature קיימים לא נגעו — `d.analysis.owners`/
`mortgages`/`cautionNotes`/`unreadable`/`summary` והרינדור שלהם ב-שני
המקומות זהים למצב שלפני. נדחף לענף
`fix/32-nadlan-berega-tabu-area-visibility-0903` — לא מוזג. System 35
KioskFleet לא נגע, לפי ה-HARD STEERING.
...
`np_tabu_request_create`/`np_tikmeida_request_create` (0153/0155) רק הוסיפו
שורה, ו-`np_property_get` מחזירה את `tabu_requests`/`tik_meida_requests` רק
בתגובה של נכס בודד. גרפ מלא על `app.html` מצא ששני מקומות הקריאה היחידים
ל-`d.tabu_requests`/`d.tik_meida_requests` הם בתוך `openProperty()` בלבד —
במשרד עם יותר מכמה נכסים, אין שום דרך לגלות "יש בקשה ממתינה" בלי לפתוח כל
נכס בנפרד. אין תור/רשימת-משימות ברמת המשרד לזרימה הזו בכלל, בניגוד לעמלות
ומשימות (`tasks_open`/`tasks_overdue`) שכבר מוצגות בדשבורד.

נוספה מיגרציה 0161: RPC חדש `np_registry_requests_pending(p_office uuid)`
(SQL, security invoker, RLS-מוגן) שמאחד `tabu_requests`+`tik_meida_requests`
עם `status in ('pending','sent')` בלבד (לא `fulfilled`), מצטרף ל-`properties`
לכתובת/עיר/גוש/חלקה, וממיין דחוף-קודם ואז ותיק-קודם (`order by (grade =
'urgent') desc, created_at asc`) — כך שהבקשה הכי דחופה/הכי מוזנחת מופיעה
ראשונה. `renderDash()` קוראת לו במקביל ל-`np_dashboard` הקיים, מוסיפה אריח
KPI חדש ("בקשות נסח/תיק-מידע פתוחות", מודגש כשיש דחופות) וסקציית רשימה חדשה
(תווית סוג+סטטוס+דחיפות+כתובת+תאריך לכל בקשה) עם כפתור "פתיחת הנכס" שקורא
ל-`openProperty()` הקיימת ישירות (אין תלות בטאב פעיל — הדרואר הוא overlay).

אומת חי ב-MCP בתוך BEGIN/ROLLBACK על המשרד האמיתי `QA 18/08` (בעלים אמיתי,
`can_touch`/`manages_office` אומתו `true` תחילה): נכס+4 בקשות (2 דחופות
בתאריכים שונים, 1 רגילה, 1 `fulfilled`) — הפונקציה החזירה בדיוק 3 שורות
(הבקשה שהושלמה הוצאה נכון), סדר נכון (דחוף-ותיק-יותר קודם, אחריו הדחוף
השני, ואז הרגילה), ובידוד RLS חוצה-משרד אומת נפרד (משרד אחר קיבל `[]`).
אפס שיוריות אחרי כל סבב (connection נסגר בלי commit = rollback אוטומטי,
אומת ישירות מול הטבלה). `get_advisors` (security) לא סימן שום ממצא חדש על
הפונקציה. `node --check` נקי על בלוק ה-`<script type="module">` היחיד;
בדיקת איזון-סוגריים על הקובץ המלא עברה (937/937 מסולסלים, 3393/3393 רגילים,
271/271 מרובעים). לוגיקת הרינדור (תווית סוג/סטטוס, דחוף, נפילה-לגוש/חלקה
כשאין כותרת/כתובת, בריחת HTML) נבדקה בנפרד ב-Node מול 4 תרחישים — כולם
עברו. אין דפדפן חי בסנדבוקס הזה לקליק-דרך, אותה מגבלה כל סבב אחר שנגע
ב-app.html-only.

אפס רגרסיה: RPC חדש + אריח/סקציה חדשים בלבד, שום RPC/handler/עמודה קיימים
לא נגעו. נדחף לענף `fix/36-nadlan-pro-registry-requests-dashboard-0903` —
לא מוזג. System 35 KioskFleet לא נגע, לפי ה-HARD STEERING.

## עדכון — 03/09/2026, session 2 (Loop A — 36 nadlan-pro: בקשות נסח/תיק-מידע לא ניתנות לשיוך לעסקה, `sites/36-nadlan-pro/tivuch/app.html`)
המשך לאותה מחלקת-פער (שדה קיים ב-DB, לא נגיש ב-UI): `nadlan_pro.tabu_requests`/
`tik_meida_requests` (0153/0155) כוללות עמודת `deal_id` (FK ל-`nadlan_pro.deals`,
מאונדקסת, `on delete set null`) ו-`np_tabu_request_create`/
`np_tikmeida_request_create` כבר מקבלות `deal_id` כפרמטר אופציונלי — אבל טופסי
יצירת הבקשה ב-`app.html` (`wireTabu`/`wireTikMeida`, בקשה חדשה) מעולם לא הציעו
לבחור עסקה, ו-`tabuHtml`/`tikMeidaHtml` מעולם לא הציגו לאיזו עסקה בקשה קיימת
משויכת — למרות ש-`np_property_get` כבר מחזיר `deal_id` בכל שורה (אומת ב-grep,
מיגרציות 0153/0155). במשרד עם כמה עסקאות פתוחות על אותו נכס (בדיוק המצב
שהחלק "עסקאות על הנכס" בכרטיס הנכס כבר תומך בו) בקשת נסח/תיק-מידע לא הייתה
מקושרת לעסקה הספציפית שבשבילה היא נדרשת.

`tabuHtml`/`tikMeidaHtml` מקבלות עכשיו פרמטר חמישי `deals` (מערך `d.deals`,
כבר קיים בתגובת `np_property_get` ומוצג ב"עסקאות על הנכס"); שורה חדשה "קשור
לעסקה: <כותרת>" מתווספת לכרטיס בקשה כש-`r.deal_id` נפתר מול המערך — `uuid`
לא-נפתר (עסקה שנמחקה/`deal_id` זר) פשוט לא מציג שורה, לא קורס. `wireTabu`/
`wireTikMeida` מקבלות אותו פרמטר ומוסיפות `<select>` "קשור לעסקה" לטופס
בקשה-חדשה (רק כש-יש עסקאות על הנכס; ברירת מחדל "ללא עסקה") ששולח `deal_id`
ל-RPC (`null` כשלא נבחר).

אומת ע"י חילוץ `tabuHtml`+`tikMeidaHtml`+התלויות לקובץ עצמאי והרצה ב-Node מול
9 תרחישים (עסקה נפתרת בשני הלוחות, `deal_id` זר/לא-נפתר, בלי `deal_id` כלל,
קריאה בלי הפרמטר `deals` כלל — תאימות-לאחור, כותרת-עסקה חסרה → ברירת מחדל
"עסקה") — כולם עברו; ובדיקה נפרדת על לוגיקת בניית ה-`<select>` (מערך ריק/
`undefined` → אין select, בריחת HTML בכותרת עסקה, `id` השדה) — 6 תרחישים,
כולם עברו. `node --check` נקי על בלוק ה-`<script type="module">` היחיד
בקובץ. בדיקת איזון-סוגריים על הקובץ המלא עברה (933/933 מסולסלים, 3355/3355
רגילים, 263/263 מרובעים). אפס רגרסיה: פרמטר אופציונלי חדש בלבד בשתי פונקציות
תצוגה + טופס, שום handler/RPC/עמודה קיימים לא נגעו (אין מיגרציה חדשה — ה-API
כבר החזיר את השדה). נדחף לענף `fix/36-nadlan-pro-tabu-tikmeida-deal-link-0903`
— לא מוזג. System 35 KioskFleet לא נגע, לפי ה-HARD STEERING.
...
אבל **מעולם לא `sent_at`** כש-`status='sent'` — מצב-הביניים שבו המתווך כבר
סימן "נשלח לרשם המקרקעין" וממתין לנסח. `np_property_get` (מיגרציה 0153,
שורה 266) כבר כולל `'sent_at', r.sent_at` בכל שורת `tabu_requests` שמוחזרת;
`grep -n "sent_at"` בקובץ החזיר רק שימושים בטבלת עמלות לא-קשורה (שורות
3027/3072), אפס בתוך `tabuHtml`. השוואה ל-`RequestsBoard.tsx` של מערכת 32
(שורה 167) הראתה שב-32 `sent_at` כבר מוצג — הפער היה ספציפי ל-36, בדיוק כמו
פער ה-`fulfilled_at` הקודם (יישום עצמאי, אין Next.js ב-36).

נוסף `sentHtml` על אותו דפוס בדיוק כמו `fulfilledHtml`: שורה "נשלח לרשם
המקרקעין &lt;תאריך&gt;" כש-`status==='sent'` וקיים `r.sent_at`; חסר-תאריך
מדלג על השורה בלי קריסה (נבדק כתרחיש-קצה מפורש). `tikMeidaHtml` לא נבדקה
לאותו פער כי תיק-מידע (בניגוד לנסח) עובר ישר מ"בקשה" ל"הונפק" בהעלאת הקובץ
עצמה (0155) — אין שלב-ביניים "נשלח" מקביל, לפי ההערה שכבר קיימת בקובץ
מעל `tikMeidaHtml` (שורות 1496-1504).

אומת: חילוץ `tabuHtml`+`tabuAnalysisHtml`+התלויות (`esc`/`NA`/`dtm`/
`TABU_STATUS_HE`/`TABU_GRADE_HE`) לקובץ עצמאי, הרצה ב-Node מול 4 תרחישים
(pending, sent+`sent_at`, sent-בלי-`sent_at`, fulfilled+מסמך) — כולם רינדרו
נכון, תרחיש-הקצה לא קרס. `node --check` נקי על ה-`<script type="module">`
היחיד בקובץ (812 תווי-מקור אחרי חילוץ). בדיקת איזון-סוגריים על הקובץ המלא
עברה (812/812 סוגריים מסולסלים, 3166/3166 סוגריים רגילים, 248/248 מרובעים
בתוך בלוק ה-script). `nadlan_pro.tabu_requests` עדיין 0 שורות חיות (נבדק),
כמו הפער הקודם — תיאורטי-לעת-עתה, מוכן לרגע שהזרימה תשמש בפועל. אפס
רגרסיה: שינוי תצוגה טהור, אין RPC/עמודה/מיגרציה חדשים. נדחף לענף
`fix/36-nadlan-pro-tabu-sent-at-visibility-0902` — לא מוזג. System 35
KioskFleet לא נגע, לפי ה-HARD STEERING.
...
## עדכון — 03/09/2026 (Loop A, תזכורת חתימה — event שקיים בקוד אך אף פעם לא נורה)
ביקורת RPC-מול-UI על 36 nadlan-pro חשפה שהמאפיין `'reminded'` קיים בשלושה
מקומות — `np_sign_log` (הפונקציה היחידה שכותבת ל-`nadlan_pro.signature_events`)
מקבל אותו כ-event חוקי, ותיבת "יומן ראיות" בכרטיסיית חוזה (`openContract`,
`app.html`) כבר ממפה אותו לתווית "תזכורת" — אבל שום קוד בפועל לא היה יוצר
אותו: כפתור "שלח במייל" היחיד תמיד שלח את אותו מייל ראשוני ותמיד רשם
`emailed`, גם בפעם העשירית שנלחץ לאותו חותם שלא הגיב. לא היה למשרד שום דרך
להבחין בין "שליחה ראשונה" ל"תזכורת" — לא בממשק, לא במייל שהחותם מקבל, ולא
ביומן הראיות.

התיקון: ב-`app.html`, כפתור "שלח במייל" הופך ל"שלח תזכורת" ברגע שלחותם כבר
יש אירוע `emailed`/`reminded` קודם (בודק את `s.events` שכבר מגיע מ-
`np_contract_get`), ומעביר `reminder: true/false` ל-Edge Function
`np-send-signature`. ב-Edge Function: כש-`reminder=true` הנושא/הפתיח של
המייל משתנים לניסוח תזכורת ("תזכורת — מסמך ממתין לחתימתך") במקום ההודעה
המקורית, וה-event שנרשם דרך `np_sign_log` הוא `reminded` ולא `emailed`.
אפס שינוי בזרימה הקיימת (שליחה ראשונה מתנהגת בדיוק כמו קודם), אפס טבלה/
עמודה חדשה — `signature_events` וה-check ב-`np_sign_log` כבר תמכו ב-
`reminded` מהתחלה. אומת: `diff` מול הגרסה החיה שנשלפה מה-DB לפני העריכה
הראה רק את השינויים המכוונים; בדיקת איזון-סוגריים/backticks על ה-TS
עברה; חמישה תרחישי-Node על לוגיקת תווית-הכפתור (לא נשלח, sent-בלבד,
כבר-נשלח-פעם, כבר-נשלח-תזכורת, אין מייל) עברו כולם; `node --check` +
איזון-סוגריים על ה-`<script type="module">` המלא (195KB, 814/814/3230/
3230/262/262) עבר. הפונקציה נפרסה בפועל ל-Supabase (`np-send-signature`
v3) דרך `deploy_edge_function` ואומתה בחזרה מול הגרסה שנפרסה. לא בוצע
שליחת מייל אמיתית לבדיקה (אין חוזה/חותם ב-QA שממתין לחתימה בלי ליצור
תופעת-לוואי אמיתית) — לפי חוק מצב-הטסט-בלבד. נדחה+נדחף לענף
