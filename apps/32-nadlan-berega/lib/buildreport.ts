...
export async function buildReport(
    ...
    if (g) {
      ...
      if (lat == null || googleVerifies) {
        ...
        if (googleVerifies) {
          ...
          // התוצאה של גוגל היא היחידה שיש, אבל היא לא אומתה מול היישוב שהוזן,
          // ובלי האזהרה הזו הדוח היה ממשיך בשקט עם נקודה לא-מאומתת (בניגוד
          // למקרה המקביל של GovMap לא-מאומת בשורה 844-848 שכן מזהיר).
          warnings.push(
            `לא הצלחנו לאמת שהכתובת שנמצאה היא בדיוק זו שחיפשת. המערכת זיהתה "${g.formatted}". כדאי לבדוק שזו אכן הכתובת הנכונה.`,
          );
        }
      }
    }
  ...
  if (lat != null && lng != null && itmX != null && itmY != null && !tierMayUsePaidSources(tier)) {
    ...
    // ⚠️ ברמה החינמית religion נשאר [] תמיד — לא כי לא נמצאו בתי כנסת, אלא כי
    // המקור (Google Places) בתשלום ולא נשלף כלל כאן. בלי אזהרה מפורשת, [] נראה
    // זהה ל"נבדק ולא נמצא", וזה בדיוק סוג השתיקה שדוח בתשלום לא יכול להרשות
    // לעצמו (ראה ההערה על בית חולים למטה). מקוואות כן נשלפים גם ברמה החינמית
    // (מרשם ממשלתי חינמי, לא Google) ולכן לא נכללים באזהרה הזו.
    warnings.push(
      'רשימת בתי הכנסת ומוסדות הדת בסביבה אינה כלולה ברמה החינמית (המקור, Google Places, ' +
        'הוא בתשלום). רשימת מקוואות כן מופיעה למעלה, ממרשם ממשלתי חינמי. רשימה מלאה של בתי ' +
        'כנסת עם מרחקים אמיתיים מופיעה בדוח המקיף.',
    );
  ...
  // ⚠️ בלי radiusM: השדה המוצג הוא "מקוואות ביישוב" (עיר שלמה, לא סביבת
  // הנכס) — רדיוס 4 ק"מ שהיה כאן קודם סינן בשקט מקוואות אמיתיים ומגואקדים
  // בערים גדולות (ירושלים, באר שבע) שהיו מעבר ל-4 ק"מ מהנכס אך עדיין בתוך
  // אותה עיר, וגרם לספירה חסרה שסתרה את התווית. המיון עדיין שם את הקרוב
  // ביותר ראשון, כך ש"המקווה הקרוב ביותר" לא נפגע.
  if (city) {
    mikvaot = await mikvaotNear(city, lat, lng).catch(() => []);
  }
