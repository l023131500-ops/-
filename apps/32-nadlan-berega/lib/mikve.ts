...
export async function mikvaotNear(
  ...
  try {
    // ⚠️ סינון לפי `filters` על שם יישוב נכשל כשהכתיב שונה במקצת, ולכן
    // מסננים אצלנו על תוצאות חיפוש חופשי — אותו שיעור שנלמד בקובץ הבחירות.
    const res = await datastoreSearch(MIKVE_RESOURCE(), { q: want, limit: 200 });
