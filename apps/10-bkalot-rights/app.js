 ...
 *
 * עדכון: בשלב ראשון הופנה למרכז השליטה הכללי, וזה היה נכון יותר מהקישור
 * הקודם אבל לא מה ש-§5 מבקש — הוא מבקש ניהול **של המערכת הזאת**. מה שמערכת
 * 10 באמת מנהלת הוא מאגר הזכויות (888 שורות ב-rights.catalog, אותו מאגר
 * שהאתר מציג), ולכן זה היעד: מסך שמראה מה יש במאגר ואיפה הוא חסר.
 *
 * המסך מסנן בעצמו — מי שאינו סופר-אדמין מקבל שם הסבר ולא תוכן, כמו בשאר
 * מסכי הניהול ב-more30.
 *
 * לא נגענו ב-08/09 המוגנות — הן מערכות אחרות לגמרי, וההנחיה מפרשת שהשכפול
 * הפתוח הוא של מערכת 10 ושל rights.catalog בלבד.
 */
const ADMIN_URL = 'https://more30.com/admin/rights';
