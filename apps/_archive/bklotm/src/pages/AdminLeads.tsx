...
import { IntakeSubmissionsPanel } from "@/components/admin/IntakeSubmissionsPanel";
...
const AdminLeads = () => {
  ...
  const [premiumEmails, setPremiumEmails] = useState<Set<string>>(new Set());
  ...
  const fetchLeads = async () => {
    ...
    if (error) {
      setLeads([]);
      setAccessError("לא ניתן לקרוא את הפניות. ודאו שנכנסתם עם משתמש מנהל.");
    } else if (data) {
      setLeads(data as Lead[]);
      // Check premium status against Aura Financial
      try {
        const emails = (data as Lead[]).map(l => l.email).filter(Boolean) as string[];
        const phones = (data as Lead[]).map(l => l.phone).filter(Boolean) as string[];
        if (emails.length || phones.length) {
          const { data: pd } = await supabase.functions.invoke("check-premium-client", {
            body: { emails, phones },
          });
          if (pd?.premium) {
            setPremiumEmails(new Set((pd.premium as any[]).map(p => (p.email || "").toLowerCase())));
          }
        }
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (roleError || !roleData) {
        setLoading(false);
        setAccessError("המשתמש מחובר, אבל אין לו הרשאת מנהל ולכן הפניות מוסתרות.");
        return;
      }
      fetchLeads();
    };
      ...
      <header className="border-b border-border bg-card px-6 py-4">
        <div className="container mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Link to="/admin/podcasts"><Button variant="ghost" size="sm" className="gap-2">🎙️ פודקאסטים</Button></Link>
      ...
      <main className="container mx-auto px-6 py-6 space-y-4">
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-6 text-center space-y-3">
            <p className="font-bold text-destructive">אין גישה לפניות</p>
            <p className="text-sm text-muted-foreground">{accessError}</p>
            <Button variant="outline" size="sm" onClick={handleLogout}>כניסה מחדש</Button>
          </div>
        ) : filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtered.map((lead) => {
              ...
              const isPremium = (lead.email && premiumEmails.has(lead.email.toLowerCase())) ||
                (lead.phone && premiumEmails.has(`${lead.phone.replace(/\D/g, "")}@luxmanage.app`));
              return (
                <div key={lead.id} className={`rounded-xl border-2 ${isPremium ? "border-amber-400 ring-2 ring-amber-300/50" : st.border} ${st.bg} p-4 space-y-3 cursor-pointer hover:shadow-md transition-shadow relative`}
                  onClick={() => { setSelectedLead(lead); setEditNotes(lead.admin_notes || ""); setHandledDesc(lead.handled_description || ""); }}>
                  {isPremium && (
                    <div className="absolute -top-2 -right-2 px-2 py-0.5 rounded-full text-[10px] font-bold bg-gradient-to-r from-amber-400 to-yellow-500 text-black shadow-lg">
                      ⭐ לקוח פרימיום
                    </div>
                  )}
