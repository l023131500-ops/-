...
import { MapPin, Clock, Video, Radio, Globe, Building2, Mail, Phone, MessageSquare, FileText } from "lucide-react";
...
import PublicPrayerTimes from "@/components/portal/PublicPrayerTimes";
...
const PublicOrgPage = () => {
  ...
  const fetchData = async (token: string, isCancelled: () => boolean) => {
    ...
    const [lessonsRes, photosRes] = await Promise.all([
      rabbiNames.length > 0
        ? supabase.from("lessons").select("*").in("rabbi_name", rabbiNames).eq("is_approved", true).order("created_at", { ascending: false })
  ...
  const prayerTimesEnabled = (portal?.features_enabled as any)?.prayer_times === true;
    ...
    >
      ...
      <div className="relative z-10">
        ...
        {prayerTimesEnabled && portal && (
          <PublicPrayerTimes orgId={portal.id} />
        )}
        ...
        {/* Activity Photos - moved to after lessons and sections */}
        {photos.length > 0 && (
          <div className="bg-background/90 backdrop-blur-sm py-12 px-6">
        ...
        {/* Contact Form */}
        {portal && <PublicContactForm portalId={portal.id} portalType="org" portalName={portal.org_name} />}
      ...
      <AnimatePresence>
        {selectedLesson && (
          <LessonDetailModal lesson={selectedLesson} onClose={() => setSelectedLesson(null)} imageOptions={{ backgroundPreset: portal?.background_preset, portalName: portal?.org_name }} />
        )}
      </AnimatePresence>
    </div>
  );
