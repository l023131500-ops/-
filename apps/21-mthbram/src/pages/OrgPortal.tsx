          ...
          <div className="rounded-2xl border-2 border-amber-300 bg-amber-50 text-amber-900 px-5 py-4 shadow-sm space-y-2">
            ...
            <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl px-4 py-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="font-body text-xs text-green-800 truncate">{`${getPublicOrigin()}/view/org/${portal.public_token}`}</p>
              </div>
              <Button size="sm" variant="outline" className="border-green-300 text-green-700 hover:bg-green-100 gap-1 text-xs"
                onClick={() => { navigator.clipboard.writeText(`${getPublicOrigin()}/view/org/${portal.public_token}`); toast.success("קישור ההפצה הועתק!"); }}>
              ...
              {addingNew && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mb-6">
                  <div className="bg-card rounded-2xl border-2 border-gold/30 p-6 space-y-4">
                    ...
                    <div>
                      <label htmlFor="new-lesson-rabbi-name" className="font-body text-xs font-medium text-muted-foreground mb-1 block">שם הרב *</label>
                      <select
                        id="new-lesson-rabbi-name"
