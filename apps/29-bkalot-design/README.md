...
`bk-font-modern` / `bk-font-friendly` / `bk-font-alt-body` / `bk-font-narrow` /
`bk-font-serif-alt`) — ראו `style-guide.html` לדוגמה חיה של כל אחד ומתי להשתמש
בו. וגם 4 גרדיאנטים + 5 טקסטורות רקע (`bk-bg-brand` / `bk-bg-hero` /
`bk-bg-gold` / `bk-bg-subtle` / `bk-bg-dots` / `bk-bg-grid` / `bk-bg-diagonal` /
`bk-bg-wave` / `bk-bg-noise`), בנויים ממשתני הצבע הקיימים כך שמתאימים אוטומטית
גם למצב כהה.

---
מותג בקלות · Heebo + Frank Ruhl Libre + 7 פונטים נוספים · RTL
