...
/**
 * pdf-lib is 176 kB of the route's JS and only the page-selection tool needs it,
 * so it is fetched as its own chunk rather than bundled into /shelf/[id] — which
 * every image, doc, media and Google item there was paying for without ever
 * mounting this component. import() caches, so the later callers reuse it.
 */
...
export default function PdfViewer({
  ...
  useEffect(() => {
    ...
    // Nothing hidden — the overwhelming majority — means the file at `fileUrl`
    // *is* the preview, so hand the URL itself to the embedded viewer. It paints
    // the first page while the rest of the document is still arriving; a blob
    // built from fetch() cannot exist until the last byte has landed in JS, so
    // the reader used to sit on "טוען תצוגה…" for the whole download (the shelf
    // holds PDFs into the tens of MB). The buffer the page tool needs is still
    // fetched below, from the same same-origin URL the viewer is reading, served
    // `Cache-Control: public, max-age=86400`.
    const trimmed = hiddenSet.size > 0;
    if (!trimmed) setPreviewUrl(fileUrl);
    ...
    (async () => {
      try {
        ...
        if (trimmed) {
          ...
          if (!alive) return;
          const u = URL.createObjectURL(new Blob([bytes as unknown as BlobPart], { type: "application/pdf" }));
          urls.push(u);
          setPreviewUrl(u);
        }
        setStatus("ready");
        ...
        if (!alive) return;
        // The file's own bytes never arrived, and the embedded viewer is reading
        // the very same URL — it will fail the same way, so take the optimistic
        // preview back down and show why. A failure after the bytes landed
        // (pdf-lib chunk, trimming) leaves the preview alone: it is real.
        if (!gotBytes) setPreviewUrl("");
  ...
  return (
    <>
      ...
      <div style={{ marginTop: 20, borderRadius: 14, overflow: "hidden", border: "1px solid #ebe7de", background: "#f4f2ec" }}>
        {/* A preview already on screen outlives a later failure (pdf-lib chunk,
            trimming): the file itself loaded, so keep showing it. */}
