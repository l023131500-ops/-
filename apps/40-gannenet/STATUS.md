...
## 19/08/2026 (ערב) — 22 קבצי מקור חסרים לגמרי מהגיט שוחזרו (git add -f מעולם לא כלל אותם)

**התגלית:** למרות שהרשומות למעלה מתעדות "בנייה ✓, 180 עמודי `/lesson/[id]`"
ו"פריסה מהמקור" — עץ הגיט של הענף הזה **מעולם לא החזיק את הקבצים שמייצרים
את זה**. `git ls-files apps/40-gannenet` החזיר 33 קבצים בלבד; מקור ה-Next.js
המלא (ללא PDFs, שיושבים ב-bucket ולא בגיט בכוונה) מחזיק 49. 22 מהם הופעלו רק
בפריסה שבוצעה בסבב קודם ולא נכנסו מעולם לאף commit כאן — כולל `package.json`
עצמו (!), `tsconfig.json`, ארבעת קובצי `content/*.json` (מקור 180 המערכים +
קטלוג המדף), ושמונה עמודים/נתיבי API: `app/library`, `app/lesson/[id]`,
`app/calendar`, `app/newsletter`, `app/pricing`, `app/shelf/page.tsx`,
`app/shelf/[id]`, וגם `app/api/admin/list` + `app/api/admin/override` —
שני נתיבי ה-API שמסך `app/shelf/admin/page.tsx` (**כן** מחובר בגיט) קורא
להם בכל פעולה. כלומר: מסך הניהול היה חי ובנוי, ולחיצה על כל כפתור בו הייתה
נכשלת ב-404 בלי שנתפס — וארבעה קישורי ניווט מרכזיים בעמוד הבית ובתפריט
(`/library`, `/calendar`, `/newsletter`, `/pricing`) היו מובילים לאותו 404.
גם `public/manifest.webmanifest`, `public/icon.svg`, `public/sw.js` היו
חסרים — ה-PWA (מניפסט/אייקון) ורישום ה-service-worker לאופליין נכשלו בשקט
(`.catch(() => {})` ב-`RegisterSW.tsx` בלע את השגיאה).

**התיקון:** שוחזרו 22 הקבצים במדויק מה-branch המקורי
(`l023131500-ops/-` @ `claude/ganenet-full-system-gdrive-fdflfc`,
`f489e2c8`) דרך ה-Git Blob API של GitHub (לא clone מלא — `_deploy`/STATUS.md
כבר הזהירו שהסניף מכיל 166MB של PDFs). כל קובץ אומת בשתי רמות: גודל בבייטים
זהה לזה שברשומת ה-tree, ו-`git hash-object` על הקובץ שנכתב מקומית זהה בדיוק
ל-`sha` הרשום ב-tree — לא רק "משהו ירד", אלא בדיוק אותו בלוב. כל ארבעת קובצי
ה-JSON נבדקו גם ש-`json.load` מצליח עליהם. **אפס רגרסיה:** 11 קבצים
שנוספו בריפו הזה אחרי הייבוא המקורי (session/auth/rate-limit/quota/מדף-שלי —
תכונות שלא היו במקור) לא נגעו ולא נדרסו; נבדק שאף אחד מ-22 הקבצים המשוחזרים
לא מייבא מהם ולהפך, כך שאין התנגשות.

**מה שנשאר פתוח, ולא תוקן כאן בכוונה:** `app/shelf/admin/page.tsx` קורא גם
ל-`/api/admin/delete` (מחיקת קובץ שהועלה, דורש `SUPABASE_SERVICE_ROLE_KEY`
לפי הרשימה למעלה) — הנתיב הזה **אינו קיים במקור הווענדור בכלל** (תוסף
שנכתב כאן בסבב קודם, כמו `admin/usage`), ואין לו עותק לשחזר משום מקום נגיש
מהסשן הזה. לא נוחש/נכתב מחדש קוד שרת שנוגע ב-service_role בלי מקור אמין —
רשום ל-`core.issues` לסבב הבא. גם `/shelf` (השוחזר, גרסת המקור המקורית) אינו
קורא `cat`/`q` מה-URL — שיפור שהיה מתועד ב-STATUS.md קודם ("`/calendar`
מקשר ל-`/shelf?cat=…`") אבל מעולם לא נכנס לגיט; `lib/season.ts` (כן בגיט)
עדיין לא מחובר ל-`app/calendar/page.tsx` השוחזר (גרסת המקור, לפני השיפור
המתועד). לא הורחב בסבב הזה — שחזור מדויק מהמקור קודם על פני שחזור-חלקי
מהזיכרון.

עדות: Git Tree API (recursive) על הסניף המקורי, אימות hash מלא לכל 22
הקבצים (התאמה 100%). לא בוצע `npm install`/`npm run build` בסבב הזה (לפי
כלל הסשן — בלי תהליכים ארוכים/build מקומי); האימות היה קריאת קוד + בדיקת
ייבוא + התאמת hash, לא הרצה.
