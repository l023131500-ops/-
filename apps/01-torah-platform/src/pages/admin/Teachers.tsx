          ...
          <Dialog open={showCreate} onOpenChange={setShowCreate}>
            ...
            <DialogContent className="max-w-lg border-2 border-secondary/40 bg-gradient-to-br from-card via-card to-secondary/5 shadow-[0_20px_60px_-15px_hsl(var(--secondary)/0.4)]" dir="rtl">
              ...
              <div className="space-y-3 mt-3">
                <div>
                  <label htmlFor="teachers-portal-type" className="text-sm font-medium mb-1 block">סוג הפורטל *</label>
                  <Select value={form.portal_type} onValueChange={(v) => setForm(f => ({ ...f, portal_type: v }))}>
                    <SelectTrigger id="teachers-portal-type"><SelectValue /></SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  <label htmlFor="teachers-full-name" className="text-sm font-medium mb-1 block">שם מלא *</label>
                  <Input id="teachers-full-name" value={form.full_name} onChange={(e) => setForm(f => ({ ...f, full_name: e.target.value }))} placeholder="הרב ישראל ישראלי" />
                </div>
                {form.portal_type !== "rabbi" && (
                  <div>
                    <label htmlFor="teachers-org-name" className="text-sm font-medium mb-1 block">שם הארגון / בית הכנסת</label>
                    <Input id="teachers-org-name" value={form.organization_name} onChange={(e) => setForm(f => ({ ...f, organization_name: e.target.value }))} placeholder="בית כנסת אהל יעקב" />
                  </div>
                )}
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label htmlFor="teachers-email" className="text-sm font-medium mb-1 block">מייל *</label>
                    <Input id="teachers-email" type="email" inputMode="email" autoComplete="email" value={form.email} onChange={(e) => setForm(f => ({ ...f, email: e.target.value }))} placeholder="email@example.com" dir="ltr" />
                  </div>
                  <div>
                    <label htmlFor="teachers-phone" className="text-sm font-medium mb-1 block">טלפון</label>
                    <Input id="teachers-phone" value={form.phone} onChange={(e) => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="050-0000000" />
                  </div>
                </div>
                <div>
                  <label htmlFor="teachers-initial-password" className="text-sm font-semibold mb-1 flex items-center justify-between text-foreground">
                  ...
                  <Input id="teachers-initial-password" value={form.initial_password} onChange={(e) => setForm(f => ({ ...f, initial_password: e.target.value }))} dir="ltr" className="font-mono tracking-wider" />
                </div>
              </div>
