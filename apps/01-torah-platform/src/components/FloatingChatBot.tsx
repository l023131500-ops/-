    ...
    try {
      ...
      if (actionMatch) {
        try {
          ...
          // seeker_leads and teacher_leads were folded into `leads` with a
          ...
          if (actionType === "submit_lesson") {
            ({ error } = await supabase.from("lessons").insert({
              ...actionData, tenant_id: tenant.id, is_approved: false, is_active: true,
            }));
          } else {
            ({ error } = await supabase.from("leads").insert({
              ...actionData,
              tenant_id: tenant.id,
              kind: actionType === "submit_seeker" ? "seeker" : "teacher",
              source: "chatbot",
            }));
          }
          ...
          toast.error("לא הצלחנו לשמור את הפרטים — נסו שוב או צרו קשר טלפונית");
        }
            ...
            <div className="relative z-10 flex flex-col h-full bg-card rounded-3xl overflow-hidden">
              ...
              <div className="relative p-4 flex items-center gap-3">
                ...
                <div className="relative z-10 flex items-center gap-3 w-full">
                  ...
                  <button onClick={() => setIsOpen(false)} className="w-8 h-8 rounded-full bg-muted/50 flex items-center justify-center hover:bg-muted transition-colors" aria-label="סגירת הצ'אט">
              ...
              <div className="p-3 border-t border-border/50">
                <div className="flex gap-2">
                  <input value={input} onChange={(e) => setInput(e.target.value)}
                    ...
                    aria-label="חפשו שיעור, נושא, רב, עיר"
                    disabled={isTyping}
