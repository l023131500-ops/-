...
import {
  LayoutDashboard, Calendar, Users, FileText, Settings, LogOut, BookOpen, MessageSquare, ShieldCheck, Building2, Mail, ClipboardCheck, BadgeCheck, HeartHandshake, Heart, Newspaper, Megaphone
} from "lucide-react";
...
const PortalSidebar = () => {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("profiles").select("portal_type, display_name").eq("id", user.id).maybeSingle()
      .then(({ data }) => {
        if (data?.portal_type) setPortalType(data.portal_type);
        // organization_name was folded into display_name.
        if (data?.display_name) setOrgName(data.display_name);
      });
  }, [user]);
