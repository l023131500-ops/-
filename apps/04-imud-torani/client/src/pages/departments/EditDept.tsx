      ...
      {blocks.map((b, i) => {
        ...
        return (
          ...
          >
            <div className="mb-1.5 flex items-center justify-between">
              ...
              <div className="flex items-center gap-0.5">
                ...
                <button onClick={() => setOpenMeta(openMeta === b.id ? null : b.id)}
                  className="rounded p-1 text-muted-foreground hover-elevate" title="הגדרות בלוק" aria-label="הגדרות בלוק"
                  aria-expanded={openMeta === b.id} aria-controls={`block-meta-${b.id}`} data-testid={`button-meta-${i}`}>
            ...
            {/* הגדרות בלוק מתקדמות */}
            {openMeta === b.id && (
              <div id={`block-meta-${b.id}`} className="mt-2 space-y-2 rounded-md border border-border bg-background p-2.5">
                {isHeading(b.layer) && (
                  <>
