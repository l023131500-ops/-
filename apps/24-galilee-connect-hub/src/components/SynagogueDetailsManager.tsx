...
type EditableGabai = { clientId: string; name: string; phone: string; isDemo?: boolean };
type EditablePrayer = { clientId: string; name: string; time: string; day: PrayerDay; no_minyan: boolean; isDemo?: boolean };
type EditableLesson = { clientId: string; subject: string; teacher: string; day: string; time: string; audience: string; location: string; isDemo?: boolean };
type EditableAnnouncement = { clientId: string; content: string; image_url: string | null; imageFile: File | null; previewUrl: string | null; is_active: boolean; isDemo?: boolean };
type EditableAccess = { id?: string; username: string; password: string; display_name: string; is_active: boolean };

const makeId = () => Math.random().toString(36).slice(2, 10);

// isDemo rows are examples only — handleSave() strips them unless the admin/gabai actually edits
// one, so an unattended "שמור הכל" can never publish fabricated data as if it were real.
const createDemoGabbaim = (): EditableGabai[] => [{ clientId: makeId(), name: 'גבאי ראשי לדוגמה', phone: '050-0000000', isDemo: true }];
const createDemoPrayers = (): EditablePrayer[] => [
  { clientId: makeId(), name: 'שחרית', time: '06:15', day: 'weekday', no_minyan: false, isDemo: true },
  { clientId: makeId(), name: 'מנחה', time: '17:30', day: 'weekday', no_minyan: false, isDemo: true },
  { clientId: makeId(), name: 'ערבית', time: '19:45', day: 'weekday', no_minyan: false, isDemo: true },
  { clientId: makeId(), name: 'שחרית שבת', time: '08:00', day: 'shabbat', no_minyan: false, isDemo: true },
];
const createDemoLessons = (): EditableLesson[] => [{ clientId: makeId(), subject: 'הלכה יומית', teacher: 'הרב לדוגמה', day: 'ראשון', time: '20:00', audience: 'כללי', location: 'בית הכנסת', isDemo: true }];
const createDemoAnnouncements = (name?: string): EditableAnnouncement[] => [{ clientId: makeId(), content: `מודעה לדוגמה עבור ${name || 'בית הכנסת'} — אפשר לערוך או למחוק.`, image_url: null, imageFile: null, previewUrl: null, is_active: true, isDemo: true }];
...
const SynagogueDetailsManager = ({ synagogueId, mode = 'admin', synagogueName, onSaved }: SynagogueDetailsManagerProps) => {
  ...
  const handleSave = async () => {
    ...
    const cleanGabbaim = gabbaim.filter((item) => !item.isDemo).map((item) => ({ name: item.name.trim(), phone: item.phone.trim() })).filter((item) => item.name && item.phone);
    const cleanPrayers = prayerTimes.filter((item) => !item.isDemo).map((item) => ({ name: item.name.trim(), time: item.time.trim(), day: item.day, no_minyan: item.no_minyan })).filter((item) => item.name && (item.time || item.no_minyan));
    const cleanLessons = lessons.filter((item) => !item.isDemo).map((item) => ({ subject: item.subject.trim(), teacher: item.teacher.trim(), day: item.day.trim(), time: item.time.trim(), audience: item.audience.trim() || null, location: item.location.trim() || null })).filter((item) => item.subject && item.teacher && item.day && item.time);
    const cleanAnnouncements = (await Promise.all(announcements.filter((item) => !item.isDemo).map(async (item) => {
  ...
  return (
    <div className="space-y-6 rounded-2xl border border-border/40 bg-card p-6 shadow-card">
      ...
      <div className="space-y-3 rounded-2xl border border-border/40 bg-muted/20 p-4">
        ...
        {gabbaim.map((item) => <div key={item.clientId} className="grid grid-cols-1 gap-2 md:grid-cols-[1fr_1fr_auto]"><Input value={item.name} onChange={(e) => setGabbaim((prev) => prev.map((row) => row.clientId === item.clientId ? { ...row, name: e.target.value, isDemo: false } : row))} placeholder="שם הגבאי" /><Input dir="ltr" value={item.phone} onChange={(e) => setGabbaim((prev) => prev.map((row) => row.clientId === item.clientId ? { ...row, phone: e.target.value, isDemo: false } : row))} placeholder="050-0000000" /><Button variant="ghost" size="icon" className="text-destructive" onClick={() => setGabbaim((prev) => prev.filter((row) => row.clientId !== item.clientId))}><Trash2 className="w-4 h-4" /></Button></div>)}
      </div>
      ...
      <div className="space-y-3 rounded-2xl border border-border/40 bg-muted/20 p-4">
        ...
        {lessons.map((item) => <div key={item.clientId} className="rounded-xl border border-border/40 bg-background/80 p-3 space-y-2"><div className="grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-3"><Input value={item.subject} onChange={(e) => setLessons((prev) => prev.map((row) => row.clientId === item.clientId ? { ...row, subject: e.target.value, isDemo: false } : row))} placeholder="נושא" /><Input value={item.teacher} onChange={(e) => setLessons((prev) => prev.map((row) => row.clientId === item.clientId ? { ...row, teacher: e.target.value, isDemo: false } : row))} placeholder="שם הרב" /><Input value={item.day} onChange={(e) => setLessons((prev) => prev.map((row) => row.clientId === item.clientId ? { ...row, day: e.target.value, isDemo: false } : row))} placeholder="יום" /><Input dir="ltr" value={item.time} onChange={(e) => setLessons((prev) => prev.map((row) => row.clientId === item.clientId ? { ...row, time: e.target.value, isDemo: false } : row))} placeholder="20:00" /><Input value={item.audience} onChange={(e) => setLessons((prev) => prev.map((row) => row.clientId === item.clientId ? { ...row, audience: e.target.value, isDemo: false } : row))} placeholder="קהל יעד" /><Input value={item.location} onChange={(e) => setLessons((prev) => prev.map((row) => row.clientId === item.clientId ? { ...row, location: e.target.value, isDemo: false } : row))} placeholder="מיקום" /></div><div className="flex justify-end"><Button variant="ghost" size="sm" className="gap-2 text-destructive" onClick={() => setLessons((prev) => prev.filter((row) => row.clientId !== item.clientId))}><Trash2 className="w-4 h-4" /> מחק</Button></div></div>)}
      </div>
      ...
      <div className="space-y-3 rounded-2xl border border-border/40 bg-muted/20 p-4">
        ...
        {announcements.map((item) => <div key={item.clientId} className="rounded-xl border border-border/40 bg-background/80 p-3 space-y-3"><textarea value={item.content} onChange={(e) => setAnnouncements((prev) => prev.map((row) => row.clientId === item.clientId ? { ...row, content: e.target.value, isDemo: false } : row))} rows={3} placeholder="תוכן המודעה" className="w-full rounded-lg border border-input bg-background px-4 py-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring resize-none" /><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div className="flex items-center gap-3">{(item.previewUrl || item.image_url) && <img src={item.previewUrl || item.image_url || ''} alt="מודעה" className="h-20 w-28 rounded-xl object-cover shadow-card" />}<label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-border bg-card px-3 py-2 text-sm font-bold text-foreground hover:border-primary/30"><Camera className="w-4 h-4 text-primary" /> העלה תמונה<input type="file" accept="image/*" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (!file) return; setAnnouncements((prev) => prev.map((row) => row.clientId === item.clientId ? { ...row, imageFile: file, previewUrl: URL.createObjectURL(file), isDemo: false } : row)); }} /></label></div><div className="flex items-center gap-3"><label className="flex items-center gap-2 text-sm font-bold text-foreground"><span>פעילה</span><input type="checkbox" checked={item.is_active} onChange={(e) => setAnnouncements((prev) => prev.map((row) => row.clientId === item.clientId ? { ...row, is_active: e.target.checked, isDemo: false } : row))} /></label><Button variant="ghost" size="sm" className="gap-2 text-destructive" onClick={() => setAnnouncements((prev) => prev.filter((row) => row.clientId !== item.clientId))}><Trash2 className="w-4 h-4" /> מחק</Button></div></div></div>)}
      </div>
