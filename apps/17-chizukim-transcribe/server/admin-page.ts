...
// ומוגשים ישירות מ-Express תחת /api/admin, אותה שיטה בדיוק כמו
// apps/28-kupot-health-funds/server/admin-page.ts. נתוני ההקלטות מוצגים דרך
// textContent בלבד (לא innerHTML עם מחרוזות שהורכבו).
// ---------------------------------------------------------------------------
...
<title>ניהול — תמלול חיזוקים</title>
<style>
  ...
  #login-box label { display:block; font-size:.9rem; margin-bottom:.4rem; margin-top:.8rem; }
  ...
  #login-box button, #logout-btn { margin-top:1rem; width:100%; padding:.6rem; border:0; border-radius:8px; background:#3a6b52; color:#fff; font-size:1rem; cursor:pointer; }
  ...
  th, td { padding:.5rem .7rem; text-align:right; border-bottom:1px solid #e6e9ec; font-size:.85rem; white-space:nowrap; max-width:22rem; overflow:hidden; text-overflow:ellipsis; }
  ...
  #search { padding:.4rem .6rem; border:1px solid #ccd3da; border-radius:8px; font-size:.9rem; min-width:14rem; }
  #table-wrap { overflow-x:auto; }
  .status-badge { display:inline-block; padding:.1rem .5rem; border-radius:999px; font-size:.75rem; }
  .status-ready { background:#e3f3e8; color:#1f7a3f; }
  .status-error { background:#fbe4e4; color:#a92626; }
  .status-other { background:#eef1f4; color:#5a6672; }
</style>
</head>
<body>

<div id="login-box">
  <h1>ניהול — תמלול חיזוקים</h1>
...
<div id="app">
  <div id="topbar">
    <h1>הקלטות — <span id="count"></span></h1>
    <input id="search" type="search" placeholder="חיפוש לפי נושא/שם קובץ..." />
...
(function () {
  ...
  var COLUMNS = [
    { key: "seq", label: "#" },
    { key: "original_name", label: "קובץ" },
    { key: "topic", label: "נושא" },
    { key: "parsha_or_date", label: "פרשה/תאריך" },
    { key: "status", label: "סטטוס" },
    { key: "duration_seconds", label: "משך (שנ')" },
  ...
  function renderTable(rows) {
    ...
    wrap.textContent = "";
    document.getElementById("count").textContent = rows.length + " מתוך " + allRows.length + " הקלטות";
    if (!rows.length) {
      var p = document.createElement("p");
      p.textContent = "אין הקלטות תואמות.";
      wrap.appendChild(p);
      return;
    }
  ...
  function applyFilter() {
    ...
    renderTable(
      allRows.filter(function (r) {
        return (
          (r.topic || "").toLowerCase().indexOf(q) !== -1 ||
          (r.original_name || "").toLowerCase().indexOf(q) !== -1 ||
          (r.parsha_or_date || "").toLowerCase().indexOf(q) !== -1
        );
      })
    );
  }
  ...
  function loadRecordings() {
    fetch("admin/recordings", { credentials: "include" })
      .then(function (r) {
        if (!r.ok) throw new Error("לא ניתן לטעון הקלטות");
        return r.json();
      })
      .then(function (rows) {
  ...
  document.getElementById("login-btn").addEventListener("click", function () {
    ...
    fetch("admin/login", {
      ...
      credentials: "include",
      body: JSON.stringify({ username: userInput.value, password: pwInput.value }),
    })
