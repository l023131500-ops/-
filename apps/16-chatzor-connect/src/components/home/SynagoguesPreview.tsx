...
import { SAMPLE_SYNAGOGUES } from "@/data/seed";
import { Section } from "@/components/ui/Section";

function SynagogueCard({ s, index }: { s: Synagogue; index: number }) {
  return (
    <motion.article
    ...
    >
      <div className="relative h-24" style={{ background: s.brandGradient }}>
        ...
        <div className="absolute -bottom-6 right-5 grid h-12 w-12 place-items-center rounded-xl border border-border bg-card font-display text-lg font-bold text-primary shadow-soft">
          {s.name.replace(/^בית (ה)?כנסת |^בית המדרש /, "").charAt(0)}
        </div>
      </div>
...
export function SynagoguesPreview() {
  const synagogues = SAMPLE_SYNAGOGUES;
  return (
    <Section
      id="synagogues"
      eyebrow="בתי הכנסת"
