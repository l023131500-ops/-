...
## 🔴 0. Google SSO — הדבר היחיד שחסר לכניסה האחידה (31/07)

הכניסה האחידה **עובדת**: `eueu1234`/`eueu1234` נכנס בפועל, הסשן עובר בין
מערכות, יש יציאה, ויש כניסת ניהול. ראיות ב-`QA/auth.md`. נשאר סעיף אחד.

**מה חסר:** ספק Google כבוי בפרויקט ההאב. נמדד:
`GET /auth/v1/settings` → `"google": false`.

**מה צריך ממך (שני שלבים, ~5 דקות):**
1. `console.cloud.google.com` → APIs & Services → Credentials → **OAuth client ID**
   (Web application). Authorized redirect URI:
   `https://uhnrgujbdxhhmoxcjria.supabase.co/auth/v1/callback`
2. Supabase → פרויקט `uhnrgujbdxhhmoxcjria` → Authentication → Providers →
   **Google** → להדליק ולהדביק `CLIENT_ID` + `CLIENT_SECRET`.

**ואז — כלום.** דף הכניסה בודק את `settings` בטעינה, וכפתור "המשך עם Google"
**יופיע מעצמו** בלי בנייה ובלי פריסה מחדש. (זו הסיבה שהוא מוסתר ולא מוצג-
ומושבת: כפתור שמוביל ל-400 גרוע יותר מכפתור שאינו קיים.)

**באותו מסך, בזמן שאתה שם:** Authentication → URL Configuration →
Redirect URLs → להוסיף `https://more30.com/**` (זה גם הפריט מסעיף 5 הישן).
