...
**ארבעה** דברים שלא היו ברורים לפני, ומשנים את התכנון:

0. **תור ההודעות היוצא כבר קיים** — עם ערוצים (email · whatsapp · voice ·
   n8n), מצב, תזמון, ניסיונות חוזרים ותיעוד תשובה (§4א). **אין צורך לבנות
   מנוע שליחה.** צריך רק להזין אותו.
...
## 4א. תור ההודעות היוצא — **קיים, וזה שלד האוטומציה**

זה מה שהחקירה הראשונה החמיצה, והוא משנה את היקף מה שצריך להיבנות.

ל-27 יש **תור הודעות יוצאות מלא** עם ערוצים:
`server/routes.ts` — `/api/admin/delivery` (יצירה · עריכה · מחיקה · שליחה).

```
if (!["email", "whatsapp", "voice", "n8n"].includes(channel)) { … }
```

**כל הודעה מחזיקה:** `channel`, `recipientLabel`, `toAddress`, `subject`,
`body`, `callbackUrl`, `status`, `statusDetail`, `scheduledAt`, `attempts`,
`endpointUsed`, `responseText`, `sentAt`. כלומר יש כאן **תור עם מצב,
תזמון, ניסיונות חוזרים ותיעוד תשובה** — לא רק "שלח".

**איך נשלח (`/api/admin/delivery/:id/send`):**

| ערוץ | מסלול |
|---|---|
| `voice` + `YEMOT_API_KEY` | **עוקף את הקונקטור** והולך ישר ל-`sendYemotVoice` |
| `email` · `whatsapp` · `n8n` | דרך `automation_configs[<channel>]` — `endpointUrl` + `enabled` |

**ומה שקורה כשאין קונקטור מוגדר הוא בדיוק ההתנהגות הנכונה:** ההודעה
מסומנת `skipped` עם `"הקונקטור לא מופעל או חסר endpoint. ההודעה נשמרה
בתור טיוטה."` — היא לא נעלמת ולא מתחזה לנשלחת.

> **המסקנה לתכנון:** אין צורך לבנות מנוע שליחה. צריך **להזין** את התור
> הזה: מי הלקוח, מה מגיע לו, ובאיזה ערוץ. זה כל הפער.
...
| **תור הודעות יוצא, עם ערוצים ומצב** | ✅ ממומש (§4א) — email · whatsapp · voice · n8n |
| **טופס → זיהוי מצב אישי → בחירת זכויות → הזנת התור** | ❌ **החוליה החסרה, וזו היחידה** |
...
לקוח (טופס / נדרים / שיחה)
  ...
  [מערכת חדשה בהאב]  schema: bkalot_auto
   · customers · requests · consents
        │
        ├─► זכויות:   rights.catalog (האב, 888 שורות) — קריאה ישירה
        ├─► מחירים:   pc-supabase-read (csjekrvu) — קריאה בלבד
        ├─► קופות:    hf (28, בהאב)
        │
        ▼  בונה הודעה ו**מזין את התור הקיים**
  delivery_messages  (§4א — כבר קיים, עם מצב/תזמון/ניסיונות)
        │
        ├─ voice ──► sendYemotVoice()  ← **מחולץ ל-packages/**
        ├─ email ──► automation_configs['email'].endpointUrl
        └─ n8n   ──► הקונקטור הקיים
```

**שים לב מה אין כאן:** אין מנוע שליחה חדש, אין תור חדש, ואין אינטגרציית
ימות חדשה. שלושתם קיימים ועובדים. המערכת החדשה היא **מי שמחליט מה לשלוח
למי** — וזה כל מה שחסר.
...
## 8א. ממצא מהסבב שאחרי — `/bkalot` רץ בלי JavaScript בכלל

בזמן חקירת הסגנון של `/bkalot` התברר משהו שמשנה את הערכת המצב של מערכת 10:
**האתר הוגש בפרודקשן בלי גיליון סגנון ובלי אף סקריפט.** `style.css`,
`engine.js`, `repo.js` ו-`app.js` כולם החזירו את ה-HTML של הפורטל
(נתיבים יחסיים + אין `<base>` → נפתרו לשורש). הכול 200, ולכן שום בדיקה
לא צעקה.

**המשמעות לחקירה הזו:** מנוע הזכויות והשאלון של מערכת 10 היו **מתים**,
וגם הטעינה החיה של הקטלוג מההאב (§4א) לא רצה מעולם — לא כי לא נפרסה,
אלא כי `repo.js` לא הורץ. **תוקן ואומת** (`<base href="/bkalot/">`).
מכאן `rights.catalog` באמת מזין את האתר.
