...
> ## 🟡 18/08/2026 (לילה, מאוחר עוד יותר) — **32 נדל"ן: §12 — מיקום התוכניות על המפה האינטראקטיבית (עדיין לא נפרס)**

> המשך ישיר ל"**נשאר**" שברשומה מתחת (מיקום על המפה, marker לכל תוכנית).
> `InteractiveMap.tsx` מקבל עכשיו `plans?: NearbyPlan[] | null` אופציונלי —
> כל תוכנית מסומנת כריבוע זהוב קטן (שונה מהעיגולים הטורקיז של נקודות עניין
> ומסיכת הנכס), לחיצה פותחת popup עם מספר/שם/מצב/מרחק. `PropertyImagery.tsx`
> מעביר `report.nearbyPlans` (כבר קיים, `null` בחינמי לפי השער הקיים). הרשימה
> הטקסטואלית הקיימת ב-`NearbyPlansPanel` לא שונתה — זו תוספת חזותית בלבד.
>
> **אומת מקומית:** `tsc --noEmit` + `next build` נקיים (15/15 routes).
> Playwright מול `next dev`, `/report?tier=premium&q=דיזנגוף 100 תל אביב`:
> 6 ריבועים זהובים (תואם בדיוק ל-6 התוכניות שנמדדו בפריסת הפרודקשן), popup
> אמיתי (`507-0880112`, מצב אישור, 35 מ'). אותה כתובת ב-`tier=basic`: 0
> ריבועים — שער הפרימיום נשמר, שאר המפה (נכס + נקודות עניין) ללא שינוי.
> ראיות: `QA/platform/nadlan-nearby-plans-map-markers-0818/_results.md`.
>
> **נשאר:** פריסה לפרודקשן (דרך `sync-nadlan.ps1` + `vercel deploy --prod`
> מהריפו הנפרד, כמו בפריסת §12 הקודמת) — לא בוצעה בצעד הזה.
