...
> ## 🟢 19/08/2026 (+44) — **10 בקלות: כפתור "סמן כמומש" לכל המלצה — audit_gaps #10 סעיף 4 נסגר**
>
> `core.projects.audit_gaps` ל-#10 (בקלות, עדיפות #1 של המשתמש) תיעד פער אמיתי:
> "אין מעקב מה כבר מימשתי" — שום דרך לסמן אילו מהזכויות שהומלצו כבר טופלו.
> האתר הוא static site וניל-JS ללא build וללא שום חשבון משתמש בכלל (רק מפתח
> anon לקריאת קטלוג + לכידת ליד עם INSERT בלבד) — מעקב בצד שרת לא רלוונטי.
> נוסף פתרון מקומי לדפדפן, אותו דפוס בדיוק כמו מתג המצב-הכהה הקיים
> (`localStorage['bkalot-theme']`): כפתור חדש "סמן כמומש" בכל כרטיס המלצה
> (`recCard()`, ליד "פרטים מלאים" הקיים) שומר/מסיר את מזהה הכרטיס ב-
> `localStorage['bkalot-done']`, מסמן את הכרטיס בפס ירוק (`.rec-done`,
> אותו טוקן `--tip` ואותה שיטת פס-פנימי כמו קטגוריות העדיפות הקיימות), ומעדכן
> שורת סיכום חדשה ("סימנתם X מתוך Y..."). קריאה בלבד מה-server — אין
> endpoint/סכמה/הרשאה חדשים, ואין שינוי לכפתורי הדפסה/JSON/פרטים/ליד הקיימים.
>
> אומת פעמיים: מקומית (שרת סטטי זמני תחת `/bkalot/`, נמחק אחרי) ואז חי ב-
> `more30.com/bkalot` (Playwright, cache-buster): 7 כרטיסים מתוך שאלון אמיתי,
> קליק על "סמן כמומש" → כרטיס ירוק + `localStorage` מעודכן + שורת סיכום
> 1/7, רענון מלא + הרצה חוזרת של השאלון הראו את אותו כרטיס עדיין מסומן,
> ביטול הסימון נקי, 0 שגיאות קונסולה. כל הכפתורים הקיימים (7×"פרטים מלאים",
> הדפסה, JSON, 3×"קבלו את הפרטים") נשארו ללא שינוי — אפס רגרסיה.
> `more30.com/` ו-`more30.com/bkalot/` עדיין 200. צילום:
> `bkalot-mark-done-card-0819.png`. פריסה: אין build לאתר הזה — `app.js`/
> `style.css` הועתקו bit-for-bit ל-`_deploy/bkalot-more30/bkalot/` (אומת
> זהה לפני פריסה) → `vercel deploy --prod`, `dpl_GWAYZzsY9S26fLpan1oQNh12bUZF`,
> READY. commit `e439cc1`.

> ## 🟢 19/08/2026 (+43) — **26 studio: דיאלוג "ביקורת AI" מציג עכשיו את הערת הלקוח עצמה — CHECKLIST/graphics.md #14, פרוסה בטוחה נוספת**
>
> עד עכשיו דיאלוג "ביקורת AI" רק ציין במשפט קבוע ("הביקורת הביאה בחשבון
> את הערת הלקוח השמורה") בלי להראות את הטקסט עצמו — צריך היה לפתוח את
> דיאלוג "הערות לקוח" הנפרד כדי לראות מה נכתב שם. נוסף בלוק קריאה-בלבד
> בראש דיאלוג הביקורת (`Editor.tsx`) שמציג את `clientNotes` בפועל כשקיימת
> הערה שמורה — כדי שהמעצב יראה מיד מול מה הביקורת הגיבה, בלי לעבור בין
> שני דיאלוגים. תוספת UI טהורה בלבד — אין שינוי ל-endpoint/סכמה/state.
>
> `tsc --noEmit` נקי, `vite build --base=/studio/` נקי (2169 מודולים, זהה
> לבסיס — 0 קוד מת). אומת חי ב-`more30.com/studio` (Playwright,
> cache-buster, 1280×900): נפתחה "שיעור — חסידי מלכותי" → הערות לקוח →
> הוקלדה הערה → "ביקורת AI" → ציון 68/100, הבלוק החדש מציג בדיוק את טקסט
> ההערה שנכתב (לא ריק, לא גנרי). 0 שגיאות קונסולה. שאר הדיאלוג (חוזקות/
> הערות/כפתורי "החל תיקון"/הצעות/"בדוק שוב אחרי התיקונים") ושאר הכפתורים
> בעורך ללא שינוי — אפס רגרסיה; `/studio`, `more30.com/` עדיין 200. צילום:
> `studio-critique-client-notes-shown-0819.png`. פריסה: `vite build` →
> `_deploy/studio-more30/public/studio` (build חדש, `api/` ללא שינוי) →
> `vercel deploy --prod`, `dpl_HWg6mYuZjH9W3jFZ5B5eT2LCaKxf`, READY.
> **נשאר לסבב הבא:** עדיין הלולאה האוטומטית המלאה עצמה (פריטים 13/14) —
> במכוון לא נבנתה, אותה סיבה כמו הפעמים הקודמות.
...
## 19/08/2026 (ערב) — 06 briut / 12 smel / 15 egod: תיקון `audit_gaps` מיושן ("אין קישור מחירון")

סבב הביקורת האחרון (19/08) רשם על שלוש המערכות "אין קישור מחירון" — אבל
`SYSTEMS_STATUS.md` כבר תיעד למעלה (בוקר +4, אותו יום) ש-12 smel פרס ואימת
בדיוק את הקישור הזה. נבדק מחדש: שתי הסיבות שהמדידה נראתה שלילית הן אותה
מלכודת בדיוק — smel ו-egod הן אפליקציות SPA (React, מרונדרות ב-JS), אז
`fetch`/`Invoke-WebRequest` על ה-HTML הגולמי לא רואה טקסט/קישורים שה-JS
מוסיף רק אחרי טעינה. `Header.tsx`/`Navbar.tsx` כבר מכילים
`<a href="/subscribe?app=smel|egod">מחירון</a>`; briut הוא אתר סטטי (בלי
build) ולכן ה-HTML הגולמי שלו כן מראה את הקישור.

**אומת חי (Playwright, DOM מרונדר, לא HTML גולמי):**
- `more30.com/smel` — `a[href*="subscribe?app=smel"]` קיים, טקסט "מחירון".
- `more30.com/egod` — `a[href*="subscribe?app=egod"]` קיים, טקסט "מחירון".
- `more30.com/briut` — `Invoke-WebRequest` על ה-HTML הגולמי כבר מראה
  `subscribe?app=briut` (200).

**תוקן:** `core.projects.audit_gaps`/`audited_at` לשלוש השורות (06/12/15)
עודכנו כדי שלא ירשמו כפער אמיתי בסבב הבא. אין שינוי קוד — שום דבר לא היה
שבור בפרודקשן. **לקח שיטתי:** כל סקריפט ביקורת שבודק "יש קישור X" מול
HTML גולמי (`fetch`, לא דפדפן) ידווח false-negative על כל מערכת SPA
(React/Vite ללא SSR) — אותה מלכודת בדיוק כמו
[[mounted-spa-returns-200-for-missing-routes]], כיוון שונה: כאן ה-route
כן קיים, רק שהתוכן לא ברינדור השרת. לא נבדקו שאר המערכות ל-false-negative
דומה בסבב הזה — אם מופיע עוד "אין קישור מחירון"/"אין X" על מערכת SPA,
לבדוק קודם ב-Playwright לפני שמאמינים לו.
FCP 3.2s → 2.0s.
