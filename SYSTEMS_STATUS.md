...
| ~~**30 CRM זכויות**~~ | ✅ **נבדק מחדש 17/08 — אינו פער פעיל.** `/crm` ו-`/crm/` מגישים מסך כניסה מלא, 0 שגיאות קונסולה. dashboard אחרי כניסה לא נבדק. ראה הרשומה בראש הקובץ. | — |
...
| **01 torah** | **FCP נפתר: 3.2s → 1.6s**, מתחת לסף. מה שנשאר הוא **LCP 5.3s** ו-TBT 510ms — בעיה אחרת מזו שהרינדור מראש נועד לפתור, ודורשת SSR מלא. ראה `QA/torah.md`. ~~🔴 וגם: CLS 0.578 חי כרגע — רגרסיה מדגל seed שנשמט~~ ✅ **נבדק מחדש 17/08 — אינו פער פעיל, CLS 0.001.** ראה הרשומה בראש הקובץ. | גדול, ארכיטקטוני |
