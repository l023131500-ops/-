...
| **יעדי מגע < 24px** | נותרו **9 קישורי טקסט** בגובה 17-21px, פזורים: `tamlul` 1 · `modaot` 3 · `smachot` 1 · `mthbram` 1 · `kesef` 1 · `kiosk` 1 · `tivuch` 1. כולם מאותה משפחה — קישור inline בלי padding אנכי. `node scripts/qa/small-targets.mjs` מרכז אותם. **kesef ו-tivuch כבר מתוקנים בקוד וממתינים לפריסה** (§0ג). | קטן |
