...
| **יעדי מגע < 24px** | **תוקן ואומת ב-0:** `tamlul` · `modaot` · `kesef` · `tivuch` · `egod`. **נותרו 4:** `mthbram` 2 (תוקן ונבנה, **לא נפרס** — מכסה), `smachot` 1, `kiosk` 1 (שתיהן לא vendored כאן). `node scripts/qa/small-targets.mjs` מרכז. | קטן |
