...
| **יעדי מגע < 24px** | **תוקן ואומת ב-0:** `egod` (18) · `modaot` (3) · `mthbram` (2) · `tamlul` · `kesef` · `tivuch`. **נותרו 2:** `smachot` 1 — תוקן ב-`style.css`, **מוכן ולא נפרס** (מכסה); `kiosk` 1 — הקישור לא אותר במקור ה-vendored (המערכת על Railway). | קטן |
