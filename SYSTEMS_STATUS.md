...
## 23/08/2026 (Loop C · Fable) — 38 אירועים ומתנות: תקציב האירוע — הוצאות מול הכנסות המתנות

סבב 8 של הבנייה. המודול הבא מ-EVENTS_BUILD.md §4: "תקציב אירוע לבעל האירוע
(הכנסות מתנות מול הוצאות)". צד ההכנסות קיים מ-0131 (פיצול הארנקים — נתח
`event_owner` הוא בדיוק ההכנסה נטו של בעל האירוע אחרי עמלות); עד עכשיו לא
היה לאן לרשום את הצד השני של המשוואה.

מה נוסף:
- **מיגרציה 0138** (הוחלה חיה על ה-hub): טבלת `events.expenses` — קטגוריה
  (רשימה קבועה נבדקת-DB: אולם/קייטרינג/מוזיקה/צילום/לבוש/עיצוב/הסעות/טקס/אחר),
  שם סעיף, ספק+טלפון, **מחיר משוער מול מחיר סגור** (העלות האפקטיבית היא
  `coalesce(actual, planned)`), סימון שולם, תאריך יעד, הערות. RLS זהה
  למוזמנים/שולחנות (בעל-אירוע-או-אדמין); **אפס גישת anon** — התקציב פרטי,
  מוזמנים לא רואים אותו. 3 RPC-ים `security invoker`: `evg_expense_add`,
  `evg_expense_update` (קונבנציית jsonb-patch של `evg_guest_update` — null
  מפורש על מחיר-סגור מחזיר למשוער), `evg_expense_delete`.
  `evg_event_dashboard` הוחלף בסופרסט קפדני של צורת 0137 + מערך `expenses`.
- **index.html — מדור "תקציב האירוע"** בלוח האירוע: 4 מדדים (סה"כ עלות /
  שולם עד כה / הכנסות ממתנות נטו עם תג טסט / מאזן צבוע ירוק-אדום), בר
  "כמה מההוצאות המתנות מכסות" באחוזים, טופס הוספת סעיף (קטגוריה, משוער,
  ספק, טלפון-ספק כקישור חיוג, תאריך יעד), טבלה עם צ'קבוקס שולם, כפתור ✎
  למחיר-סגור (ריק=חזרה למשוער), ✎ לשם+משוער, ✕ מחיקה, והדגשת ⚠ אדומה על
  תאריך יעד שעבר בסעיף שטרם שולם. ייצוא CSV עם BOM (נפתח נכון באקסל) כולל
  שורות סיכום עלות/הכנסות/מאזן. כרטיס פיצ'ר חדש בדף הנחיתה.

אימות: בדיקת עשן חיה בטרנזקציה שגולגלה לאחור — הוספת הוצאה, עדכון מחיר-סגור
+שולם, וקריאת `evg_event_dashboard` שהחזירה את הצורה החדשה (actual=480000
אומת ב-jsonb) — עברה, 0 שורות נשארו. תחביר ה-script אומת ב-`node --check`.
עדיין לא מנותב על more30.com (אין פרויקט Vercel — כמו 39); `core.projects`
#38 נשאר `live=false` בכנות.

## 24/08/2026 (Loop C · Fable) — 38 אירועים ומתנות: מדריך הספקים — דירקטורי אצור עם ערוץ לידים

סבב 9 של הבנייה. המודול מ-EVENTS_BUILD.md §4: "דירקטורי ספקים (צלם/תזמורת/
קייטרינג) — הכנסה נוספת לפלטפורמה". עד עכשיו התקציב (0138) נתן לרשום ספק
ביד; לפלטפורמה עצמה לא היה מה להציע. הקטגוריות הן בדיוק רשימת הקטגוריות של
`events.expenses` — כך ספק נכנס לתקציב בלי שום מיפוי.

מה נוסף:
- **מיגרציה 0139** (הוחלה חיה על ה-hub): `events.vendors` (קטגוריה נבדקת-DB,
  שם, עיר, טלפון, תיאור, "מחיר החל מ-", אתר, `published`) — אצירה בידי
  סופר-אדמין בלבד (RLS `more30_is_super_admin`); ו-`events.vendor_leads` —
  פנייה לספק דרך הפלטפורמה (שם+טלפון+הערה, סטטוס חדש/נוצר-קשר/סגור, קישור
  אופציונלי לאירוע + מזהה המבקש). 7 RPC-ים: `evg_vendors_list` (ציבורי —
  ספקים מפורסמים בלבד, אותה עמדה כמו `evg_hall_public`), `evg_vendor_lead_create`
  (ציבורי; אירוע נצמד לליד **רק אם המתקשר הוא בעליו** — אחרת מזהה-אירוע מזויף
  היה מדליף "מי מתכנן מה" לתצוגת האדמין), ו-5 `evg_admin_vendor_*` (definer +
  שער סופר-אדמין: list עם ספירת לידים, upsert, delete, leads_list עם אימייל
  המבקש+שם האירוע, lead_status).
- **index.html — "📒 מדריך הספקים"** בתוך מדור התקציב של בעל האירוע: סינון
  קטגוריה, כרטיסי ספקים (תיאור, עיר, החל מ-, אתר), **"➕ לתקציב"** שיוצר סעיף
  הוצאה עם פרטי הספק והמחיר-החל-מ- כמשוער, "✉ השארת פנייה" (ליד דרך
  הפלטפורמה, נצמד לאירוע הפתוח) וקישור וואטסאפ ישיר כשיש טלפון.
- **קונסולת הסופר-אדמין:** מדור אצירה מלא — טופס הוספה/עריכה (✎ ממלא את
  הטופס, ביטול עריכה), הסתרה/הצגה בלחיצה, מחיקה עם אישור, ספירת לידים
  לכל ספק; וטבלת "פניות לספקים" עם סטטוס עביד. כרטיס פיצ'ר בדף הנחיתה.
- **תיקון באג רדום מסבב 6 (מנדט האודיט):** בלוק ה-boot של המודול ישב באמצע
  הקובץ וקרא `await refreshMe()` → `renderAdmin()` שנוגע ב-`const`-ים
  (`EV_STATUS_LABEL`/`bpsPct`) המוצהרים אחריו — עם top-level await זה TDZ:
  **כניסת סופר-אדמין קרסה ברינדור הראשון**. ה-boot הועבר לסוף המודול אחרי
  כל ההצהרות (להאזנרים אין תלות בסדר — הם נרשמים לפני שה-boot מציג את ה-UI).

אימות: מיגרציה הוחלה חיה + בדיקת עשן חיה (upsert→update→lead→שלוש הרשימות
החזירו את הצורות הנכונות כולל ספירות לידים ואימייל מבקש; קריאה נפרדת כי
פונקציות stable לא רואות כתיבות של אותו statement), בדיקת שלילה (משתמש
לא-אדמין קיבל exception מ-`evg_admin_vendors_list`), וניקוי דרך
`evg_admin_vendor_delete` (cascade מחק גם את הליד; vendors=0, leads=0).
תחביר המודול אומת ב-`node --check`; כל 18 ה-ids החדשים קיימים בדיוק פעם
אחת. עדיין לא מנותב על more30.com (אין פרויקט Vercel — כמו 39);
`core.projects` #38 נשאר `live=false` בכנות.

## 24/08/2026 (Loop C · Fable) — 38 אירועים ומתנות: חלון הראווה של האולם נפתח — עורך דף נחיתה, מיתוג לבן ואנליטיקה לבעל האולם

סבב 10 של הבנייה, ותיקון ממצא אודיט (המנדט של הלולאה): `events.halls` נושאת
מאז 0131 את `cover_url`/`gallery`/`brand`, ודף הנחיתה הציבורי (hall.html)
מרנדר את שלושתם — אבל **אף RPC לא יכל אי-פעם למלא אותם**. `evg_hall_create`
מקבל רק שם/slug/עיר/טלפון/תיאור, ופונקציית עדכון לא הייתה קיימת בכלל: בעל
אולם לא יכל אפילו לתקן שגיאת כתיב בתיאור — קל וחומר לבנות את "חלון הראווה"
ש-EVENTS_BUILD §0/§7 מגדיר כדבר שמוכר את הפלטפורמה לבעלי אולמות. §1 תפקיד 2
כולל במפורש "ניהול המיתוג שלו", ו-§4 מונה מיתוג לבן + אנליטיקה לבעל אולם.

מה נוסף:
- **מיגרציה 0140** (הוחלה חיה על ה-hub): עמודות `capacity_guests` (בדיקת
  1–10000), `amenities` (jsonb), `video_url` (https בלבד, נבדק-DB);
  `evg_hall_update` — invoker (RLS `halls_owner_or_admin` תוחם לבעלים;
  טריגר-השמירה מ-0131 על `published`/`hall_share_bps` לא נגוע) עם סניטציה
  מלאה: גלריה = רק מחרוזות https, תקרה 12; שירותים = מחרוזות ≤40 תווים,
  תקרה 16; `brand` = רשימה לבנה של שלושה מפתחות בלבד (accent בפורמט צבע hex,
  logo_url https, tagline ≤140) — כל מפתח אחר נזרק; `evg_hall_stats` —
  definer עם בדיקת בעלות מפורשת (RLS של `events.events` היא בעלים-בלבד, אז
  invoker לא היה יכול לספור אירועים של אחרים שנקבעו באולם — רק ספירות יוצאות
  מהפונקציה, אף שורת אירוע): משפך לידים (סה"כ/חדש/נוצר-קשר/נסגר), דופק 30
  יום, פילוח לפי סוג ולפי חודש (6 חודשים), אירועים שנקבעו + קרובים;
  `evg_hall_public` ו-`evg_me` הוגדרו מחדש עם השדות החדשים (evg_me מזין את
  טופס העריכה, אז הוא מחזיר את כל מה שעביר).
- **index.html — עורך חלון הראווה:** כפתורי "עריכת דף האולם" ו"סטטיסטיקות"
  על כל כרטיס אולם; פאנל עריכה מלא (שם/עיר/כתובת/טלפון/קיבולת, תמונת שער,
  לוגו, סרטון, בוחר צבע מותג, סלוגן, תיאור, שירותים מופרדי-פסיקים, גלריה
  שורה-לכל-קישור) עם ולידציית https בצד לקוח לפני השליחה; פאנל סטטיסטיקות
  עם 6 KPI (כולל שיעור סגירה), ברים לפי חודש ופילוח לפי סוג אירוע; כרטיסי
  האולמות מציגים תמונת-שער ממוזערת, סלוגן, ו"חסרה תמונת שער" כדחיפה לאיכות.
- **hall.html — מיתוג לבן בפועל:** `brand.accent` צובע את כל הדף (משתני
  CSS), עם חישוב luminance שמחליף את צבע טקסט ה-CTA לבהיר כשהמבטא כהה —
  הכפתור נשאר קריא; לוגו מעל שם האולם, סלוגן מתחתיו; קיבולת בשורת הפרטים,
  צ'יפים של שירותים, וכפתור "▶ צפייה בסרטון האולם" (https בלבד, `rel=noopener`).

אימות: בדיקת עשן חיה בטרנזקציה שגולגלה לאחור (0 שורות נשארו) — יצירת אולם
כבעלים אמיתי, `evg_hall_update` עם קלט עוין (קישור http בגלריה + ערך לא-מחרוזת
+ מפתחות `evil`/`extra` ב-brand + שירות ריק) — הכל סונן והשאר נשמר בדיוק;
`evg_hall_stats` החזיר מצב-אפס נקי; `evg_me` החזיר את הרשומה המלאה; ובדיקות
שלילה — משתמש מחובר אחר קיבל exception גם על עדכון וגם על סטטיסטיקות (זקיף
'SECURITY HOLE' היה מפיל את כל ה-batch אילו RLS נכשל). תחביר שני המודולים
אומת ב-`node --check`; אפס ids כפולים בשני הקבצים. עדיין לא מנותב על
more30.com (אין פרויקט Vercel — כמו 39); `core.projects` #38 נשאר
`live=false` בכנות.

## 24/08/2026 (Loop C · Fable) — 38 אירועים ומתנות: סוכן ה-AI של האירוע — ניסוח מקורקע בנתוני אמת, סיכום RSVP ומי-טרם-ענה

סבב 11: הפריט המתקדם האחרון מ-EVENTS_BUILD §4 שבעיר לבנייה עכשיו — "סוכן
AI: ניסוח הזמנות/תזכורות, סיכום אישורי הגעה, זיהוי מי לא ענה" (וואטסאפ
דו-כיווני נשאר החלטת-ספק-בתשלום; סליקה חיה נשארת NEEDS_USER).

מה נוסף:
- **מיגרציה 0141** (הוחלה חיה על ה-hub): `events.ai_usage` (RLS — בעלים
  קורא את עצמו, אין שום מדיניות כתיבה ל-authenticated), מכסה יומית 20 לפי
  יום-לוח ישראל (Asia/Jerusalem, כמו maatefet 0130), `evg_ai_consume`
  (service_role בלבד, נעילת advisory פר-בעלים נגד מרוץ מכסה, **ובדיקת
  בעלות בתוך ה-DB**: ל-events.events יש מדיניות `events_hall_owner_read`
  מ-0131, כך ש"רואה את הדשבורד" ≠ "בעל האירוע" — בעל אולם שיקרא ישירות
  לפונקציה יידחה), ו-`evg_ai_my_usage` לתצוגת "נוצלו X מתוך Y".
- **Edge Function `events-ai`** (נפרסה, verify_jwt ON, אומת 401 בלי
  טוקן): מזהה את הקורא מול `auth/v1/user`, מושך את האירוע האמיתי דרך
  `evg_event_dashboard` **בטוקן של הקורא עצמו** (RLS חל), צורך מכסה דרך
  ה-RPC הנעול, קורא ל-claude-sonnet-5 עם מפתח מ-core.secrets
  (`more30_secrets_fetch`, סטריפ-BOM כמו בתקרית מערכת 26). ארבע פעולות:
  ניסוח הזמנה/תזכורת/תודה — נוסחים שמשלבים את סימוני-המקום של מרכז
  ההודעות ({שם}/{קישור}) כפי שהם; וסיכום RSVP שבו **הצבירה דטרמיניסטית
  בקוד** (ספירות לפי סטטוס/קבוצה, מי בלי טלפון, מי קיבל תזכורת, רשימת
  ממתינים עד 100 שמות) והמודל רק מנסח — בלי להמציא שם או מספר.
- **index.html:** מקטע "🪄 עוזר ה-AI של האירוע" אחרי מרכז ההודעות — בחירת
  פעולה + טון (מוסתר בסיכום) + בקשה חופשית (≤300), תצוגת מכסה, פלט עם
  העתקה ו"שימוש כנוסח בהודעות" שמזרים את הטיוטה ישירות לתיבת הנוסח
  המתאימה (ואז "שמירת נוסחים" הקיים). החלפת אירוע מסתירה טיוטה של האירוע
  הקודם. כרטיס פיצ'ר חדש בדף הנחיתה.

אימות: מיגרציה הוחלה חיה; `has_function_privilege` — anon בלי EXECUTE על
consume/my_usage, authenticated רק על my_usage, service_role על consume;
PostgREST אנונימי החזיר 42501 על שתיהן (401); הפונקציה שנפרסה הושוותה
בייט-לבייט לקובץ שבריפו; `node --check` נקי על מודול הדף; כל 10 ה-ids
החדשים ייחודיים. לא בוצעה קריאת AI אמיתית מהסשן (אין משתמש-בדיקה מחובר
כאן) — נתיב ה-Anthropic זהה אחד-לאחד ל-maatefet-ai שאומת חי ב-23/08.
עדיין לא מנותב על more30.com (אין פרויקט Vercel — כמו 39); `core.projects`
#38 נשאר `live=false` בכנות.
